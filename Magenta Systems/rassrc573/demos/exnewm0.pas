...
// see if RAS has been installed
    ...
    // entDeviceName has sorted property set
		...
		// try and find device set for default connection
            ...
            if S <> '' then
            begin
			    errcode := MagRasEdt.GetAllEntryProps (AnsiString (S)) ;
			    if errcode = 0 then
                begin
                	I := entDeviceName.Items.IndexOf (String (MagRasEdt.DeviceName)) ;
...
// set specific properties
    ...
    // device stuff - required, must match precisely name and type from lists
        ...
        DeviceName := AnsiString (MagRasCon.DeviceNameList [I]) ;
        DeviceType := AnsiString (MagRasCon.DeviceTypeList [I]) ;
    end ;
