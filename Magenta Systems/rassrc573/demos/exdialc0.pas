...
uses
  ...
  ExtCtrls, StdCtrls, magrascon, magrasent, magrasapi;
...
// if dialling need to see what's happened
	...
	// online OK, get IP addresses and restart timer
        ...
        begin
            ...
            begin
              	ConnLog.Lines.Add ('Protocol: ' + String (MagRasCon.ConnProtocol)) ;
              	ConnLog.Lines.Add ('IP Address: ' +  String (MagRasCon.ClientIP) +	' > ' + String (MagRasCon.ServerIP)) ;
        	  	ConnLog.Lines.Add ('PPP Reply Message : ' + String (MagRasCon.PPPReplyMessage)) ;
			end ;
...
// entry to dial
    ...
    DialName := String (MagRasEntryRecs [I].EntryName) ;
...
// set phonebook entry to access
	MagRasCon.EntryName := AnsiString (DialName) ;
...
begin
	...
	entCanonNumber.Text := String (MagRasCon.GetCanonical
