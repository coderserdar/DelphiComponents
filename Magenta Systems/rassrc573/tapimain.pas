...
{
DELPHI RAS COMPONENT - Testing TAPI
delphi@magsys.co.uk, http://www.magsys.co.uk/delphi/
Copyright 2012, Magenta Systems Ltd
...
uses
  ...
  StdCtrls, MagTapi, magtapiapi, ComCtrls ;
...
begin
	...
	begin
    	temp := String (TAPI.Modems [devid].Name) ;
...
begin
    ...
    for I := 0 to (TAPI.Count - 1) do
                            DevList.Items.Add (String (TAPI.Modems [I].Name)) ;
end;
...
begin
    ...
    if DevList.ItemIndex >= 0 then
          result := TAPI.GetModemDevice (AnsiString (DevList.Items [DevList.ItemIndex])) ;
end ;
...
begin
	...
	NumDial.Text := '' ;
    NumDisp.Text := '' ;
	if TAPI.TranslateAddr (devnr, AnsiString (NumUser.Text), disnum, dialnum) then
    begin
		NumDial.Text := String (dialnum) ;
    	NumDisp.Text := String (disnum) ;
    end ;
end;
