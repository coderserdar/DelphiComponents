...
Updated by Angus Robertson, Magenta Systems Ltd, England, 3rd December 2020
...
3 Dec 2020  - Encrypt/Decrypt revert to strings for base64 versions.
              ReverseEmail revert to strings.
...
// Dec 2020 revert to strings for base64 versions
FUNCTION codeb64 (cnt: integer; t: tripel_at): quadrupel_at;
FUNCTION decodeb64 (inp: quadrupel_at; VAR cnt: integer): tripel_at ;
function MagBase64decode (const S: String): AnsiString ;
function MagBase64encode (const instr: AnsiString): String ;
function strREncrypt(const S: AnsiString; Key: Word): AnsiString;
function strRDecrypt(const S: AnsiString; Key: Word): AnsiString;
function strBEncrypt(const S: String; Key: Word): String;
function strBDecrypt(const S: String; Key: Word): String;
function strBXEncrypt(const S: String; Key: Word): String;
function strBXDecrypt(const S: String; Key: Word): String;
function strBXDecryptEx(const S: String; Key: Word): String;
...
function ReverseEmail (const AString: String): String ;
...
begin
   result := '' ;
    try
        ...
        result := String (strRDecrypt (temp, Key)) ;
    except ;
        result := '' ;
    end ;
end ;
...
function ReverseEmail (const AString: String): String ;  // took 1.1 secs for 35K emails
var
    Ch: Char;
    I, J, L, M: Integer;
    Source, OldSrc, Dest: PChar;
begin
    L := Length (AString) ;
    result := AString ;
    if L <= 0 then exit ;
