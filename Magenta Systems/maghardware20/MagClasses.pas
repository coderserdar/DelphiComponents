...
{ various classes
Updated by Angus Robertson, Magenta Systems Ltd, England, 18th June 2021
...
18 Jun 2021   - Unicode fixes in TExRegistry
...
var
  ...
  begin
    ...
    begin
      if AnsiChar(st [i]) in ['0'..'9', 'a'..'f', 'A'..'F'] then     // June 2021
        val := val + st [i]
      else
      begin
        if val <> '' then
  ...
  begin
    ...
    begin
      p := 1;
      while (p <= Length (st)) and not (AnsiChar(st [p]) in [':', '(', ' ']) do    // June 2021
        Inc (p);
