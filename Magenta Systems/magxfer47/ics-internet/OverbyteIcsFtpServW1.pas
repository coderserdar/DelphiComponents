...
Nov 19, 2008 V7.02W wide version, fully unicode
             server component created in code to avoid installing package
             use the TNT Unicode controls to display Unicode with Delphi 2007, (Freeware:
             http://mh-nexus.de/en/tntunicodecontrols.php), undefine USE_TNT is not installed
Jan 7, 2009  V7.05 removed Display Raw, fixed HOST not tested due to event not set
Aug 8, 2011  V7.17 Angus constant FtpBuffSize = 32768 to increase FTP internal and data socket buffer sizes
...
uses
  ...
  OverbyteIcsTypes, OverbyteIcsFtpSrvWT,
  OverbyteIcsWSocket,    OverbyteIcsWinsock,
  OverbyteIcsWndControl,  OverbyteIcsFtpSrvW,
  OverbyteIcsAvlTrees,   OverbyteIcsOneTimePw,
  OverbyteIcsUtils;

const
  FtpServVersion      = 717;
  CopyRight : String  = ' FtpServW (c) 1998-2011 F. Piette V7.17 ';
  WM_APPSTARTUP       = WM_USER + 1;
  FtpBuffSize         = 32768;  // V7.17
...
{$ENDIF}
  ...
  public
     procedure Text(Prefix : Char; Msg : TFtpString);
  end;

  TCacheData = packed record
    MD5Sum    : String;
    CRC32B    : String;
    ZFileName : TFtpString;
    CFileUDT  : TDateTime;
    CFSize    : Int64;
    CReadCount: Integer;
    CLastTick : LongWord;
  ...
  TMyClient  = class(TFtpCtrlSocketW)
  private
  ...
  TFtpServerForm = class(TForm)
    ...
    procedure FtpServer1ClientConnect(Sender: TObject;
      Client: TFtpCtrlSocketW; Error: Word);
    ...
    procedure FtpServer1ClientCommand(Sender: TObject;
      Client: TFtpCtrlSocketW; var Keyword, Params, Answer: TFtpString);
    procedure FtpServer1StorSessionConnected(Sender: TObject;
      Client: TFtpCtrlSocketW; Data: TWSocket; Error: Word);
    ...
    procedure FtpServer1AnswerToClient(Sender: TObject;
      Client: TFtpCtrlSocketW; var Answer: TFtpString);
    procedure FtpServer1Authenticate(Sender: TObject;
      Client: TFtpCtrlSocketW; UserName, Password: TFtpString;
      var Authenticated: Boolean);
    procedure FtpServer1ChangeDirectory(Sender: TObject;
      Client: TFtpCtrlSocketW; Directory: TFtpString; var Allowed: Boolean);
    ...
    procedure FtpServer1BuildDirectory(Sender: TObject;
      Client: TFtpCtrlSocketW; var Directory: TFtpString; Detailed: Boolean);
    ...
    procedure FtpServer1GetProcessing(Sender: TObject;
      Client: TFtpCtrlSocketW; var DelayedSend : Boolean);
    procedure FtpServer1Md5Calculated(Sender: TObject; Client: TFtpCtrlSocketW;
      const FilePath, Md5Sum: TFtpString);
    procedure FtpServer1CalculateMd5(Sender: TObject; Client: TFtpCtrlSocketW;
    ...
    procedure FtpServer1OtpMethodEvent(Sender: TObject; Client: TFtpCtrlSocketW;
                UserName: TFtpString; var OtpMethod: TOtpMethod);
    procedure FtpServer1OtpGetPasswordEvent(Sender: TObject; Client: TFtpCtrlSocketW;
                UserName: TFtpString; var UserPassword: TFtpString);
    ...
    procedure FtpServer1CalculateCrc(Sender: TObject;
      Client: TFtpCtrlSocketW; var FilePath, Md5Sum: TFtpString;
      var Allowed: Boolean);
    procedure FtpServer1Combine(Sender: TObject; Client: TFtpCtrlSocketW;
      var Params, Answer: TFtpString);
    procedure FtpServer1SiteExec(Sender: TObject; Client: TFtpCtrlSocketW;
      var Params, Answer: TFtpString);
    procedure FtpServer1SiteMsg(Sender: TObject; Client: TFtpCtrlSocketW;
      var Params, Answer: TFtpString);
    procedure FtpServer1SitePaswd(Sender: TObject; Client: TFtpCtrlSocketW;
      var Params, Answer: TFtpString);
    procedure FtpServer1ValidateAllo(Sender: TObject;
      Client: TFtpCtrlSocketW; var Params, Answer: TFtpString);
    procedure FtpServer1Display(Sender: TObject; Client: TFtpCtrlSocketW;
      Msg: TFtpString);
    procedure FtpServer1UpCompressedFile(Sender: TObject;
      Client: TFtpCtrlSocketW);
    procedure FtpServer1UpCompressFile(Sender: TObject;
      Client: TFtpCtrlSocketW; var Done: Boolean);
    procedure FtpServer1Timeout(Sender: TObject; Client: TFtpCtrlSocketW;
      Duration: Integer; var Abort: Boolean);
    procedure FtpServer1ValidatePut(Sender: TObject; Client: TFtpCtrlSocketW; var FilePath: TFtpString;
      var Allowed: Boolean);
    procedure FtpServer1ValidateRnFr(Sender: TObject; Client: TFtpCtrlSocketW; var FilePath: TFtpString;
      var Allowed: Boolean);
    procedure FtpServer1ValidateDele(Sender: TObject; Client: TFtpCtrlSocketW; var FilePath: TFtpString;
      var Allowed: Boolean);
    procedure FtpServer1MakeDirectory(Sender: TObject; Client: TFtpCtrlSocketW; Directory: TFtpString;
      var Allowed: Boolean);
    procedure FtpServer1ValidateMfmt(Sender: TObject; Client: TFtpCtrlSocketW; var FilePath: TFtpString;
      var Allowed: Boolean);
    procedure FtpServer1Rein(Sender: TObject; Client: TFtpCtrlSocketW; var Allowed: Boolean);
    procedure FtpServer1Host(Sender: TObject; Client: TFtpCtrlSocketW; Host: TFtpString; var Allowed: Boolean);
    procedure DisplayDirectories1Click(Sender: TObject);
    procedure AddInfoMemo (S: TFtpString);
  private
    FtpServer1        : TFtpServerW;
    FInitialized      : Boolean;
    FIniFileName      : TFtpString;
    FPort             : String;
    FXTop             : Integer;
    FXLeft            : Integer;
    FXWidth           : Integer;
...
begin
    ...
    RcvSize := FtpBuffSize ;     // V7.17 increase FTP local xmit/receive buffer size
    RcvBufSize := FtpBuffSize ;  // V7.17 increase data socket receive buffer size
    SndBufSize := FtpBuffSize ;  // V7.17 increase data socket send buffer size
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TLogMsg.Text(Prefix : Char; Msg : TFtpString);
begin
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.AddInfoMemo (S: TFtpString);
begin
    if Application.Terminated then exit ;
    CurrentInfoMemo.Lines.Add (S) ;
end ;
...
begin
    ...
    InfoMemo.Font := Screen.IconFont;
    { Create the Log object }
    Log                  := TLogMsg.Create(Self);
    FMD5Cache            := TCacheTree.Create(FALSE);
    FMD5Cache.OnFreeData := CacheFreeData;

    FtpServer1 := TFtpServerW.Create(Self);
    with FtpServer1 do begin
        Options := [ftpsCwdCheck, ftpsCalcMD5OnTheFly, ftpsModeZCompress,
                             ftpsSiteXmlsd, ftpsThreadRecurDirs, ftpsEnableUtf8];
...
{$IFDEF USE_TNT}
    ...
    CurrentInfoMemo := TTntMemo.Create(self);
    CurrentInfoMemo.ParentFont  := InfoMemo.ParentFont ;
    CurrentInfoMemo.Parent      := InfoMemo.Parent ;
    CurrentInfoMemo.Align       := InfoMemo.Align ;
    CurrentInfoMemo.ScrollBars  := InfoMemo.ScrollBars ;
    CurrentInfoMemo.WordWrap    := InfoMemo.WordWrap ;
    CurrentInfoMemo.Font        := InfoMemo.Font ;
    CurrentInfoMemo.Left        := InfoMemo.Left ;
    CurrentInfoMemo.Top         := InfoMemo.Top ;
    CurrentInfoMemo.Width       := InfoMemo.Width ;
    CurrentInfoMemo.Height      := InfoMemo.Height ;
{$ELSE}
    CurrentInfoMemo := InfoMemo ;
{$ENDIF}

{    BigConsole(80, 100); }
    CurrentInfoMemo.Clear;
...
begin
    ...
    wsi := WinsockInfo;
    AddInfoMemo(Trim(CopyRight));
    AddInfoMemo('Using:');
    AddInfoMemo('   ' + OverbyteIcsWSocket.CopyRight);
    AddInfoMemo('   ' + OverbyteIcsFtpSrvW.CopyRight);
    AddInfoMemo('    Winsock:');
    AddInfoMemo('        Version ' +
            Format('%d.%d', [wsi.wHighVersion shr 8,
                             wsi.wHighVersion and 15]));
    AddInfoMemo('        ' + String(wsi.szDescription));
    AddInfoMemo('        ' + String(wsi.szSystemStatus));
{$IFNDEF VER100}
    { A bug in Delphi 3 makes lpVendorInfo invalid }
    if wsi.lpVendorInfo <> nil then
        AddInfoMemo('        ' + String(wsi.lpVendorInfo));
{$ENDIF}
...
begin
    ...
    AddInfoMemo('! ' + Client.SessIdInfo + ' connected');
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.FtpServer1Host(Sender: TObject; Client: TFtpCtrlSocketW; Host: TFtpString;
  var Allowed: Boolean);
var
    fname: string ;
begin
{ HOST might be ftp.domain.com or [123.123.123.123]   }
    ...
    if NOT FileExists (fname) then begin
        AddInfoMemo('! Could not find Accounts File: ' + fname);
        Allowed := false;
        exit;
    end ;
    AddInfoMemo('! Set Accounts File: ' + fname);
    Client.AccountIniName := fname;
    Allowed := true;
end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.FtpServer1Rein(Sender: TObject; Client: TFtpCtrlSocketW; var Allowed: Boolean);
begin
    Allowed := true;
    AddInfoMemo('! Reinitialise client accepted');
    Client.SessIdInfo := Client.GetPeerAddr + '=(Not Logged On)';
    AddInfoMemo('! ' + Client.SessIdInfo + ' connected');
    Client.AccountIniName := FtpServerForm.FIniRoot + 'ftpaccounts-default.ini';
    Client.AccountReadOnly := true;
...
procedure TFtpServerForm.FtpServer1ClientDisconnect(Sender: TObject;
  Client: TFtpCtrlSocketW; Error: Word);
begin
    AddInfoMemo('! ' + Client.SessIdInfo + ' disconnected after ' +
...
procedure TFtpServerForm.FtpServer1ClientCommand(Sender: TObject;
  Client: TFtpCtrlSocketW; var Keyword, Params, Answer: TFtpString);
begin
    AddInfoMemo('< ' + Client.SessIdInfo + ' ' + Keyword + ' ' + Params);           // Unicode version
end;
...
begin
    ...
    if NOT FileExists (Client.AccountIniName) then begin
        AddInfoMemo('! Could not find Accounts File: ' + Client.AccountIniName);
        exit;
    end;
    AddInfoMemo('! Opening Accounts File: ' + Client.AccountIniName);
  ...
  { One Time Passwords - keep sequence and seed for next login attempt }
    if Client.OtpMethod > OtpKeyNone then begin
        ...
        if ((Client.UserName = UserName) and (Password <> '')) and
            ...
            AddInfoMemo('! ' + Client.SessIdInfo + ' is authenticated')
        else begin
            AddInfoMemo('! ' + Client.SessIdInfo + ' failed authentication');
            Authenticated := FALSE;
        end;
        ...
        Authenticated := LogonUserW(PWideChar(UserName),
                                   nil,
                                   PWideChar(Password),
...
//    Client.Directory := Client.HomeDir;
    AddInfoMemo('! ' + Client.SessIdInfo + ' Home Directory: ' + Client.HomeDir);
...
begin
    if FtpServer1.ClientCount <= 0 then begin
        AddInfoMemo('No client');
        Exit;
    end;

    for I := 0 to FtpServer1.ClientCount - 1 do begin
        AddInfoMemo('Client ' + IntToStr(I + 1) + ': ' +
...
var
    MyServer : TFtpServerW;
    MyClient : TMyClient;
begin
    MyServer := Sender as TFtpServerW;
    MyClient := Client as TMyClient;
    ...
    if (Node <> nil) and (PCacheData(Node.Data)^.CFileUDT = FileUDT) and
       ...
       { only return cache md5sum if getting it for entire file }
        if (Client.HashEndPos = 0) or (Client.HashEndPos = FSize) then begin
            ...
            AddInfoMemo('Read MD5sum from cache: ' + Md5Sum);
        end;
    end;
end;
...
procedure TFtpServerForm.FtpServer1Md5Calculated(Sender: TObject;
  Client: TFtpCtrlSocketW; const FilePath, Md5Sum: TFtpString);
var
    PData : PCacheData;
    Node  : TCacheNode;
    FileUDT : TDateTime;
...
begin
    ...
    if (Client.CurCmdType = ftpcRNTO) then begin
        ...
        if (Node <> nil) then begin
            ...
            AddInfoMemo('Renamed cache for ' + Client.FromFileName +
                               ' to ' + FilePath);
        end;
        exit;
    end;
    ...
    if GetUAgeSizeFile (FilePath, FileUDT, FSize) then begin
        if Md5Sum <> '' then begin
          { only update cache md5sum if got it for entire file }
              ...
              { if cached file has changed, clear old details }
                if (PCacheData(Node.Data)^.CFileUDT <> FileUDT) OR
                    ...
                    if PCacheData(Node.Data)^.ZFileName <> '' then begin
                        AddInfoMemo('Deleted Old cached compressed file ' +
                                           PCacheData(Node.Data)^.ZFileName);
                        DeleteFile (PCacheData(Node.Data)^.ZFileName);
                    end;
                    PCacheData(Node.Data)^.ZFileName := '';
                end;
                AddInfoMemo('Old cached MD5sum updated for file ' +
                                   FilePath + '=' + Md5Sum);
            end
            else begin
            New(PData);
            ...
            AddInfoMemo('Cached MD5sum for new file ' +
                               FilePath + '=' + Md5Sum);
        end
        else
...
procedure TFtpServerForm.FtpServer1Combine(Sender: TObject;
  Client: TFtpCtrlSocketW; var Params, Answer: TFtpString);
var
    File1, File2, File3, File4: TFtpString;
    Offset: Integer;
begin
    Offset := 1 ;
    File1 := IcsScanGetNextArg (Params, Offset);
    File2 := IcsScanGetNextArg (Params, Offset);
    File3 := IcsScanGetNextArg (Params, Offset);
    File4 := IcsScanGetNextArg (Params, Offset);
    AddInfoMemo('Client wants to combine several files: ' + Params);
...
procedure TFtpServerForm.FtpServer1SiteExec(Sender: TObject;
  Client: TFtpCtrlSocketW; var Params, Answer: TFtpString);
begin
    AddInfoMemo('Client wants to run program: ' + Params);
...
procedure TFtpServerForm.FtpServer1SiteMsg(Sender: TObject;
  Client: TFtpCtrlSocketW; var Params, Answer: TFtpString);
begin
    AddInfoMemo('Log client message: ' + Params);
    Answer := '200 Noted OK';
end;
...
procedure TFtpServerForm.FtpServer1SitePaswd(Sender: TObject;
  Client: TFtpCtrlSocketW; var Params, Answer: TFtpString);
var
    OldPw, NewPw: TFtpString;
    Offset: Integer;
begin
    Offset := 1 ;
    OldPw := IcsScanGetNextArg (Params, Offset);
    NewPw := IcsScanGetNextArg (Params, Offset);
    AddInfoMemo('Client want to change password from: ' +
                       OldPw + ' to ' + NewPw);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.FtpServer1ValidateDele(Sender: TObject; Client: TFtpCtrlSocketW; var FilePath: TFtpString;
  var Allowed: Boolean);
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.FtpServer1ValidateMfmt(Sender: TObject; Client: TFtpCtrlSocketW; var FilePath: TFtpString;
  var Allowed: Boolean);
begin
    Allowed := NOT Client.AccountReadOnly;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.FtpServer1ValidatePut(Sender: TObject; Client: TFtpCtrlSocketW; var FilePath: TFtpString;
  var Allowed: Boolean);
begin
    Allowed := NOT Client.AccountReadOnly;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.FtpServer1ValidateRnFr(Sender: TObject; Client: TFtpCtrlSocketW; var FilePath: TFtpString;
  var Allowed: Boolean);
begin
    Allowed := NOT Client.AccountReadOnly;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.FtpServer1MakeDirectory(Sender: TObject; Client: TFtpCtrlSocketW; Directory: TFtpString;
  var Allowed: Boolean);
begin
    Allowed := NOT Client.AccountReadOnly;
end;
...
procedure TFtpServerForm.FtpServer1Display(Sender: TObject;
  Client: TFtpCtrlSocketW; Msg: TFtpString);
begin
    AddInfoMemo('! ' + Client.SessIdInfo + ' ' + Msg);
end;
...
// !!! TEMP should use proper cache stuff
    ...
    try
       ...
       Done := True ;
       AddInfoMemo('Opened cached compressed file: ' +
                          Client.ZCompFileName);
    except
       AddInfoMemo('Failed to open file: ' + Client.ZCompFileName);
    end;
end;
...
procedure TFtpServerForm.FtpServer1UpCompressedFile(Sender: TObject;
  Client: TFtpCtrlSocketW);
begin
   AddInfoMemo('Save cache file?: ' + Client.ZCompFileName);

end;

{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpServerForm.FtpServer1Timeout(Sender: TObject;
  Client: TFtpCtrlSocketW; Duration: Integer; var Abort: Boolean);
begin
    AddInfoMemo('! ' + Client.SessIdInfo +
