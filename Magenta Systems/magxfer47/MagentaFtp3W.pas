...
11 Aug 2011  - 3.7 - ICS changes, new throttling, zlib 1.2.5
               ...
               added TMagFtpThreadW component which runs TMagFtpW in a thread, tested with 250 threads running together
...
// 23 Sept 2010 threaded version of TMagFtp
  ...
  TThreadEventW = Procedure (LogLevel: TLogLevel ; const Id, Info: UnicodeString ;
                                              var Cancel: boolean) of object ;

  TMagFtpThreadW = class(TThread)
  private
...
{$ENDIF}
  ...
  public
    ...
    procedure LogEvent (LogLevel: TLogLevel ; Info: UnicodeString ; var Cancel: boolean) ;
...
procedure TMagFtpW.sysDelayX (aMs: longword);
var
    Trg: longword;
begin
    Trg := GetTrgMsecs (aMs) ;
...
function TMagFtpW.FtpDir (var FtpFiles: TFileRecsW; var FtpFileList: TFindList; const ListDirs: boolean = false): TTaskResult ;
var
...
begin
    ...
    try
        ...
        if result in [TaskResOKNew, TaskResOKNone] then
                    dirlisting := 'Remote Directory: ' + fSrcDir + CRLF_ + FmtFileDirListW (DirFileList, false) ;
    finally
...
begin
    TMagFtpW (Obj).MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    TMagFtpW (Obj).onFtpClientProg64 (Nil, Count, Cancel) ;
end ;
...
procedure TMagFtpW.onMagCopyEvent (LogLevel: TLogLevel ; Info: UnicodeString ; var Cancel: boolean) ;
begin
...
procedure TMagFtpW.onFtpClientProg64 (Sender: TObject; Count: Int64; var Abort: Boolean);
var
    temp: UnicodeString ;
begin
...
// build list of source files on FTP server
    ...
    if fDispRDir then doCopyEvent (LogLevelInfo, 'Source Files on FTP Server' + CRLF_ + FmtFileDirListW (SrcFileList, false)) ;
...
// build list of target files, so we don't download unnecessary stuff
    ...
    if fDispLDir then doCopyEvent (LogLevelInfo, 'Target Files on PC' + CRLF_ + FmtFileDirListW (TarFileList, false)) ;
    MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    if fCancelFlag then exit ;
...
// find size of stuff to copy
    ...
    begin
        listing.Capacity (TotSrcFiles * 50) ;   // no real idea yet
        listing.AppendLineW (CRLF_ + 'Files Selected for Downloading are: ') ;
    end;
    newsize := 0 ;
    for I := 0 to Pred (TotSrcFiles) do
    begin
            ...
            // 14 Sept 2008 skip Unicode names with substitution characters
                    ...
                    begin
                     ...
                     // add directory to list we'll try and delete later, 24 Apr 2003
                        ...
                        if newtardir <> '' then StrArrayAddSorted (DelDirList, newtardir, DelDirTot) ;
                    end
                    else
                    begin
    ...
    begin
      ...
      // 21 Feb 2011 - if a directory, special handling
            ...
            // create target directory
                ...
                if NOT IcsForceDirectoriesW (fnametar) then
...
var
    ...
    I, {donenr,} nodeltot, loop, retval, CopyOnlyTot, duration: integer ;
...
// don't delete target files unless processing full directories
    ...
    if fDispLDir then doCopyEvent (LogLevelInfo, 'Source Files on PC' + CRLF_ + FmtFileDirListW (SrcFileList, false)) ;
...
// 15 Sept 2008 don't stop, root may not exist
    ...
    fReqResponse := '' ;
    if fDispRDir then doCopyEvent (LogLevelInfo, 'Target Files on FTP Server' + CRLF_ + FmtFileDirListW (TarFileList, false)) ;
    MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    if fCancelFlag then exit ;
...
// find size of stuff to copy
    ...
    begin
       ...
       begin
            listing.Capacity (TotSrcFiles * 50) ;   // no real idea yet
            listing.AppendLineW (CRLF_ + 'Files Selected for Uploading are: ') ;
       end;
        for I := 0 to Pred (TotSrcFiles) do
        begin
            SrcFileRec := SrcFileList [I] ;
       ...
       if CheckFiles then listing.AppendLineW (CRLF_ + 'No Source Files Selected to Upload') ;
    if CheckFiles then
    begin
        info := listing.ToStringW ;
        doCopyEvent (LogLevelInfo, info) ;
...
constructor TMagFtpThreadW.CreateThread;
begin
...
procedure TMagFtpThreadW.CallThreadEvent ;  // called by Synchronise for main thread
var
    Cancel: boolean ;
begin
...
procedure TMagFtpThreadW.LogEvent (LogLevel: TLogLevel ; Info: UnicodeString ; var Cancel: boolean) ;
begin
...
procedure TMagFtpThreadW.IcsLogEvent (Sender: TObject; LogOption: TLogOption; const Msg : String) ;
begin
    if FLogmaskName <> '' then
        FBuffLogStream.WriteLine (FormatDateTime (LongTimeMask, Now) + space + FId + ': ' + Msg) ;
end ;

procedure TMagFtpThreadW.Execute;
var
    Cancel: boolean ;
begin
    MagFTPClient := TMagFTPW.Create (Nil) ;
    ...
    MagFTPClient.Utf8DiagFlag := Utf8DiagFlag ;
