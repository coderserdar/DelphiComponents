...
// supports Unicode for Delphi 2009 and later
...
TMagFtp descends from ICS TFtpClient, and publishes all it's logon and proxy
...
TMagFtp is copyrighted software, but may be used freely.
...
24 Jul 2013 - 4.1 - default to allowing IPv4 or IPv6 host names
              ...
              using TMagStringBuild to build listings
...
uses
  ...
  MagClasses, MagentaCopy,
  OverbyteIcsFtpcli, OverbyteIcsFtpSrvT, OverbyteIcsWSocket,
  OverbyteIcsMd5, OverbyteIcsCRC, 
  OverbyteIcsUtils, OverbyteIcsLogger,
  MagSubs1, MagSubs4
...
{$IFDEF USE_SSL}
  TMagFtp = class(TSslFtpClient)
{$ELSE}
  TMagFtp = class(TFtpClient)
{$ENDIF}
  private
...
//        fSslCertCheck: TSslCertCheck ;  never used
...
//      fTotProcFiles: integer ;   // 22 May 2013 gone 
...
//      CurDelFiles: integer ;  22 May 2013
        ...
        function FtpDir (var FtpFiles: TFileRecs;
                            var FtpFileList: TFindList; const ListDirs: boolean = false): TTaskResult ;  // 17 Feb 2011
        ...
        function UnpackFtpFDir (DirStream: TStream; RemDir, BaseDir: string;
                  Level: integer ; var DCodePage: Cardinal ; var HostType: THostType;
                            var TotFiles: integer; var RemFiles: TFileRecs): integer ;
        procedure FtpLogoff ;
...
// 23 Sept 2010 threaded version of TMagFtp
  ...
  TMagFtpThread = class(TThread)
  private
        FFtpThreadOpt: TFtpThreadOpt ;
        FTaskRes: TTaskResult ;
        FDirListing: String ;
        FLogLevel: TLogLevel ;
        FInfo: string ;
        FId: string ;
...
{$ENDIF}
  ...
  public
    ...
    procedure LogEvent (LogLevel: TLogLevel ; Info: String ; var Cancel: boolean) ;
    ...
    property FtpThreadOpt         : TFtpThreadOpt        read FFtpThreadOpt
                                                         write FFtpThreadOpt;
    property TaskRes              : TTaskResult          read FTaskRes;
...
{procedure Register;
begin
  RegisterComponents('Magenta Systems', [TMagFtp]);
end;}
...
constructor TMagFtp.Create(Aowner:TComponent);
begin
 ...
 // winsock bug fix for fast connections
    ...
    fMagFileCopy.MultiThreaded := FMultiThreaded ; // 16 Sept 2010
...
{$IFDEF USE_SSL}
procedure TMagFtp.OnFTPSslVerifyPeer(Sender: TObject; var Ok: Integer; Cert : TX509Base);
var
    info: string ;
begin
...
procedure TMagFtp.OnFTPSslCliNewSession(Sender: TObject; SslSession: Pointer;
                                            WasReused: Boolean; var IncRefCount : Boolean);
var
...
procedure TMagFtp.OnFTPSslCliGetSession(Sender: TObject; var SslSession: Pointer;
                                                        var FreeSession : Boolean);
var
    FtpCli: TSslFtpClient ;
begin
...
procedure TMagFtp.OnFTPSslHandshakeDone(Sender: TObject; ErrCode: Word;
                                  PeerCert: TX509Base; var Disconnect: Boolean);
var
    CertChain: TX509List;
...
procedure TMagFtp.OnFTPSslCliCertRequest(Sender: TObject; var Cert: TX509Base);
begin
    doCopyEvent (LogLevelDiag, 'Certificate Request Ignored') ;
end;
{$ENDIF}

procedure TMagFtp.SetSrcDir (S: string) ;
begin
    fSrcDir := Trim (S) ;
end ;

procedure TMagFtp.SetTarDir (S: string) ;
begin
    fTarDir := Trim (S) ;
end ;


function TMagFtp.WaitUntilReady : Boolean;
var
    DummyHandle     : THandle;
begin
...
procedure TMagFtp.sysDelayX (aMs: longword);
var
    Trg: longword;
begin
    Trg := GetTrgMsecs (aMs) ;
...
Procedure TMagFtp.SetupVMS (const BaseFolder: string; var HostType: THostType);
Begin
...
function TMagFtp.UnpackFtpFDir (DirStream: TStream; RemDir, BaseDir: string; Level: integer ;
...
// read each line in the FTP filelist
    ...
    Begin
        ...
        while (RawLineEnd <= RawTotLen) and (RawLines [RawLineEnd] <> LF) do inc (RawLineEnd) ; // find next line end
    ...
    {$ENDIF}
        ...
        // IBM MVS line
            ...
            Begin
                ...
                Begin
                    while (RawLines [RawLineEnd] <> LF) and  (RawLineEnd <= RawTotLen) do inc (RawLineEnd) ; // find next line end
                    I := RawLineEnd - RawLineStart ;
                    if (RawLineEnd > 1) and (RawLines [RawLineEnd - 1] < space) then dec (I) ; // remove CR
                    RawCurLine := Copy (RawLines, RawLineStart, I) ;  // get next line
                    inc (RawLineEnd) ;
                ...
                Begin
                    MyDir:=sDirLit;
                    MyName:=ExtractNameOnly(Copy(MyName,1,Pos(';',MyName)-1));
                End
                Else
                Begin
...
//                 ((Pos(S1[1],'cldwrx-s')<>0) And (Pos(S1[2],'cldwrx-s')<>0) And
...
function TMagFtp.FtpLogon: TTaskResult ;
var
    remdir, fname: string ;
    attemptnr: integer ;
//    waittick: DWORD ;
...
{$ENDIF}
    ...
    // toggle between two FTP servers
        ...
        fHostName := StripIpAddr (fHostName) ; // v4.0 strip [] off ipv6 address
...
{$ENDIF}
        doCopyEvent (LogLevelInfo, 'Connect/Logon to FTP Server: ' +
                                    FormatIpAddrPort (fHostName, fPort)) ; // v4.0
        fFtpErrFlag := false ;
        HostFileName := '' ;
        LocalFileName := '' ;
        ...
        begin
            ...
            // keep PWD response as root
                ...
                DosToUnixPath (fServRootDir) ;  // 6 Jan 2006 change to Unix, 11 Sept 2008 was not changing directory!!
...
function TMagFtp.FtpDir (var FtpFiles: TFileRecs; var FtpFileList: TFindList;
                                        const ListDirs: boolean = false): TTaskResult ;  // 17 Feb 2011
var
...
begin
    ...
    CopyProgClearCur (fCopyProg) ;  // 22 May 2013 clear current
...
// build list from array, omitting directories and rubbish, then sort
    ...
    begin
        ...
        FtpFileList.Sort (MagentaCopy.CompareFNext) ;
...
function TMagFtp.DispFtpDir (var dirlisting: string): TTaskResult ;
var
    DirFiles: TFileRecs ;
...
begin
    ...
    try
        ...
        if result in [TaskResOKNew, TaskResOKNone] then
                        dirlisting := 'Remote Directory: ' + fSrcDir + CRLF_ + FmtFileDirList (DirFileList, false) ;
    finally
...
procedure TMagFtp.doCopyEvent (const LogLevel: TLogLevel; const Info: string) ;
var
    oldflag: boolean ;
begin
...
begin
    TMagFtp (Obj).MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    TMagFtp (Obj).onFtpClientProg64 (Nil, Count, Cancel) ;
end ;

procedure TMagFtp.onZlibProg (Sender: TObject; Count: Int64; var Cancel: Boolean); // 9 Dec 2007
begin
    MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    Cancel := fCancelFlag ;
...
procedure TMagFtp.onMagCopyEvent (LogLevel: TLogLevel ; Info: string ; var Cancel: boolean) ;
begin
...
procedure TMagFtp.onFtpClientProg64(Sender: TObject; Count: Int64; var Abort: Boolean);
var
    temp: String ;
begin
    ...
    begin
        ...
        if Assigned (fProgressEvent) then   // 20 May 2013
            CopyProgDuration (fCopyProg, AppTicksPerFtp) ;
        doCopyEvent (LogLevelProg, temp) ;
    end;
end;

procedure TMagFtp.onFtpClientDisplay(Sender: TObject; var Msg: String);
var
    temp: string ;
begin
    temp := Msg ;
...
procedure TMagFtp.onFtpError(Sender: TObject; var Msg: String);
begin
    fFtpErrFlag := true ;
...
procedure TMagFtp.OnFtpResponse (Sender: TObject) ;
begin
...
procedure TMagFtp.OnFtpSessConn (Sender: TObject; Error: word) ;
var
    temp: string ;
begin
    temp := 'FTP Session Connected to ' + FormatIpAddrPort (DnsResult, Port) ;
    if Error <> 0 then temp := temp + ' - ' + GetWinsockErr (Error) ;
    doCopyEvent (LogLevelDiag, temp) ;
end;

procedure TMagFtp.OnFTPSocksConnected (Sender: TObject; Error: word) ;
var
    temp: string ;
begin
...
procedure TMagFtp.OnFtpSessClosed (Sender: TObject; Error: word) ;
var
    temp: string ;
begin
...
procedure TMagFtp.OnFtpRequestDone (Sender: TObject; RqType: TFtpRequest; Error: Word) ;
begin
...
procedure TMagFtp.OnFtpStateChange (Sender: TObject) ;
begin
// doCopyEvent (LogLevelDiag, '!FTP State Change=' + LookupFtpState (State)) ;  // TEMP !!!!
end;

procedure TMagFtp.EndUnZipEvent (Sender: TObject; FileIndex: Integer; FName: String) ;
var
    newsize: integer ;
begin
    if FName = '' then exit ;
...
procedure TMagFtp.UnZipHandleMessage(Sender: TObject;
          const MessageID: Integer; const Msg1, Msg2: String;
          const flags: Cardinal; var Return: Integer);
begin
...
function TMagFtp.FtpCheckFile (const RemDir, RemFile: string ; var FSize: Int64; var FileUDT: TDateTime): boolean;
var
    DirStream: TStream ;
    FtpFiles: TFileRecs;
    ret: boolean ;
...
function TMagFtp.IntDownOne (const RemDir, RemFile, RemFull, LocFileFull: string; const RFSize: Int64; RFileUDT: TDateTime): integer ;
var
...
//            doCopyEvent (LogLevelDiag, 'Old Time Stamp (local) ' + Date2Packed (LocFileUDT)) ;
    retval := MagDeleteFile (LocFileFull, fReplRO) ;
    if retval > 0 then
    begin
        if retval = 1 then
        begin
...
// tell user what we are doing
    ...
    fCopyProg.ProgMessBase := fCopyProg.ProgMessBase + CRLF_ + RemFull ;
...
// see if need to resume previous download
    ...
    begin
        fnametmp := AnsiLowerCase (RemFile);
        StringTranCh (fnametmp, '.', '_') ; // don't mess with directories
   ...
   // see if resume info file exists, check file on server has same info
        ...
        begin
            try
                ...
                partfsize := GetSize64File (fnametmp) ;
                if partfsize > 0 then
                begin
                    if FileInUse (fnametmp) then  // 31 Dec 2007 ensure file not open
                    begin
                        MagDeleteFile (fnameftp, true) ;
                        partfsize := 0 ;
                        ...
                        // reduce file size in case content is corrupted near end
                            ...
                            begin
                                ...
                                doCopyEvent (LogLevelFile, 'Resuming Partial File Download from: ' +
                                                       Int64ToCStr (partfsize) + ', with Overlap ' + IntToCStr (fMinResSize)) ;
                            end ;
                        end
                        else
...
//        onFtpClientProg64 (Self, 0, fCancelFlag) ;
    ...
    CopyProgClearCur (fCopyProg) ;  // 22 May 2013 clear current done file
    CopyProgDuration (fCopyProg, AppTicksPerFtp) ;

// FTP download failed, give up
    newsize := GetSize64File (fnametmp) ;
...
//    doCopyEvent (LogLevelProg, info) ;
    ...
    begin
        ...
        begin
            ...
            begin
                MagDeleteFile (fnameftp, true) ;  // kill restart info
                MagDeleteFile (fnametmp, true) ;
...
// check size of downloaded file matches server size, will repeat if allowed
    ...
    MagDeleteFile (fnameftp, true) ;  // kill restart info, got file OK
    ...
    begin
        ...
        begin
            ...
            doCopyEvent (LogLevelFile, 'MD5SUM Check OK: ' + fnametmp + ' took ' +    // 20 Sept 2010 was LevelInfo
                                    IntToStr (ElapsedSecs (fCopyProg.CurStartTick)) + ' secs; Result ' + fMD5Result) ;
        end
        else
     ...
     // PosEnd := newsize ;
        ...
        begin
            ...
            doCopyEvent (LogLevelFile, 'CRC32 Check OK: ' + fnametmp + ' took ' +       // 20 Sept 2010 was LevelInfo
                                IntToStr (ElapsedSecs (fCopyProg.CurStartTick)) + ' secs; Result ' + fCrcResult) ;
        end ;
    end ;
...
// replace old file, removing read only if necessary
    ...
    begin
        retval := MagRenameFile (fnametmp, LocFileFull, true, fReplRO) ;
        if retval <> 0 then
        begin
            if (retval = 1) then
            begin
...
// set file time stamp,
    if RFileUDT > 10 then
    begin
        doCopyEvent (LogLevelDiag, 'Updating Time Stamp: ' + LocFileFull + ' to (UTC)=' + Date2Packed (RFileUDT)) ;
...
// tell user we did it OK
    ...
    doCopyEvent (LogLevelFile, 'Download OK: ' + LocFileFull + ', size: ' + IntToKByte (newsize, true) + ', duration ' +
                                     TimerToStr (SecsToTime (duration div 1000)) + ', average speed ' +
                                                      IntToKByte (CalcSpeed (duration, actualbytes)) + '/sec') ;
...
function TMagFtp.FtpDownOneFile (const FdirSrc, Fnamesrc, Fnametar: string ; Replopt: TFileCopyRepl) : TTaskResult ;
var
...
// check if remote file available, get size and UTC time stamp
    ...
    // see if replacing existing file
        ...
        begin
            flag := MagCheckReplace (replopt, true, OneSecond * 2, RFSize, TarFSize, RFileUDT, TarFileDT) ;
...
function TMagFtp.FtpDownload (const CheckFiles: boolean): TTaskResult ;
begin
    fCancelFlag := false ;
// logon to FTP server
    result := FtpLogon ;
...
function TMagFtp.FtpDownFiles (const CheckFiles: boolean): TTaskResult ;
var
    ...
    OldWow64: BOOL ;            // 22 May 2013
    listing: TMagStringBuild ;  // 22 May 2013
    {$IFDEF Zipping}
    VCLUnZip: TVCLUnZip ;
    {$ENDIF}
begin
...
{$ENDIF}
    ...
    OldWow64 := false ; // 22 May 2013
    if fWow64RedirDisable then DisableWow64Redir (OldWow64) ; // 22 May 2013
    listing := TMagStringBuild.Create ; // 22 May 2013 make it faster
    try   // finally
//    CurDelFiles := 0 ;
...
// build list of source files on FTP server
    ...
    if fDispRDir then doCopyEvent (LogLevelInfo, 'Source Files on FTP Server' + CRLF_ + FmtFileDirList (SrcFileList, false)) ;
...
// build list of target files, so we don't copy unnecessary stuff
    ...
    if NOT ForceDirs (basetardir) then
    begin
        result := TaskResFail ;
    ...
    if fCancelFlag then exit ;
    fMagFileCopy.Wow64RedirDisable := fWow64RedirDisable ;  // 22 May 2013
    TotTarFiles := fMagFileCopy.GetDirList (basetardir, newfname, fCopyType, fSubDirs, 0, 0, TarFiles, TarFileList, fEmptyDirs) ; // 17 Feb 2011) ;
    if fDispLDir then doCopyEvent (LogLevelInfo, 'Target Files on PC' + CRLF_ + FmtFileDirList (TarFileList, false)) ;
    MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    if fCancelFlag then exit ;

// compare source and target files, see what to download
    fCopyProg.TotProcFiles := fMagFileCopy.SelectCopyFileList (SrcFileList, TarFileList, newfname, fCopyType,
...
// see if deleting old target files no longer in source directories
    ...
    begin
        fCopyProg.TotDelFiles := fMagFileCopy.SelectCopyFileList (TarFileList, SrcFileList, '*.*',
...
// find size of stuff to copy
    ...
    begin
        listing.Capacity (TotSrcFiles * 50) ;   // no real idea yet
        listing.AppendLine (CRLF_ + 'Files Selected for Downloading are: ') ;
    end;
    newsize := 0 ;
    for I := 0 to Pred (TotSrcFiles) do
    begin
    ...
    if fCopyProg.TotDelFiles <> 0 then info := info + CRLF_ + 'Old Files to Delete ' +
                                    IntToCStr (fCopyProg.TotDelFiles) + ', Total size ' + IntToKByte (delsize, true) ;
    doCopyEvent (LogLevelInfo, info) ;
...
// FTP download
    doCopyEvent (LogLevelInfo, 'Started FTP Download from: ' + FormatIpAddr (fHostName)) ;
    MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    if fCancelFlag then exit ;
    ...
    begin
      ...
      // 21 Feb 2011 - if a directory, special handling
            ...
            // create target directory
                ...
                if NOT ForceDirs (fnametar) then
         ...
         // FTP download file, limit maximum attempts
                    ...
                    begin
                        ...
                        begin
                            doCopyEvent (LogLevelFile, 'File Size Changed: ' + FrFullName  + ' from ' +
                                                Int64ToCStr (FrFileBytes)  + ' to ' + Int64ToCStr (RFSize)) ;
                            FrFileBytes := RFSize ;
                        end ;
                    end ;
                end
          ...
          //      doCopyEvent (LogLevelProg, '') ;
            ...
            begin
                newsize := GetSize64File (fnametar) ;
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                if NOT CheckDirAny (newtardir) then
                begin
...
// finished, report stuff
    doCopyEvent (LogLevelProg, '') ;
    duration := ElapsedTicks (fCopyProg.SessStartTick) ;
    ...
    if fCancelFlag then  // 20 Sept 2010 report something before totals
        doCopyEvent (LogLevelInfo, 'Finished, cancelled downloading, done OK: ' + IntToCStr (fCopyProg.ProcOKFiles) +
             ', failed: ' + IntToCStr (fCopyProg.ProcFailFiles) + ', skipped: ' +  IntToCStr (fCopyProg.SkippedFiles))
    else
        doCopyEvent (LogLevelInfo, 'Finished, files downloaded OK: ' + IntToCStr (fCopyProg.ProcOKFiles) +
            ', failed: ' + IntToCStr (fCopyProg.ProcFailFiles) + ', skipped: ' +  IntToCStr (fCopyProg.SkippedFiles)) ;
    ...
    doCopyEvent (LogLevelInfo, 'Total size downloaded ' + IntToKByte (totsize, true) + ', duration ' +
          TimerToStr (SecsToTime (duration div 1000)) + ', average speed ' + IntToKByte (CalcSpeed (duration, totsize)) + '/sec') ;
...
function TMagFtp.IntUpOne (const LocFileFull, RemDir, RemFile: string; const RFSize: Int64; RFileUDT: TDateTime): integer ;
var
...
// create archive directory
    ...
    begin
        fDelDone := false ;
        if NOT ForceDirs (fUpArchDir) then
        begin
            inc (fCopyProg.ProcFailFiles) ;
            fCopyProg.ProcBytesDone := fCopyProg.ProcBytesLast + RFSize ;
...
// 22 May 2013 prepare current file progress info
    ...
    fCopyProg.ProgMessBase :=  fCopyProg.ProgMessBase + CRLF_ + RemFile ;
...
//    sysDelayX (200) ;  23 May 2013 why??
    ...
    begin
        ...
        StringTranCh (fnameftp, '.', '_') ;// don't mess with directories
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            ...
                            begin
                                resflag := true ;
                                doCopyEvent (LogLevelFile, 'Resuming Partial File Upload from: ' + Int64ToCStr (partfsize)) ;
                            end ;
                        end
                        else
 ...
 //       onFtpClientProg64 (Self, 0, fCancelFlag) ;
     ...
     // start resume FTP upload
        ...
        begin
            ...
            doCopyEvent (LogLevelFile, 'Resuming FTP Upload from Offset ' +
                                     Int64ToCStr (partfsize) + ', Total file size ' + IntToKByte (uploadsize, true)) ;
   ...
   // 22 Nov 2007 check if sufficient space ALLOcated for upload
    ...
    if NOT ret then
    begin
        if actualbytes < fMinResSize then MagDeleteFile (fnameftp, true) ;  // 15 Sept 2008 kill resume file if insufficient copied
        doCopyEvent (LogLevelFile, 'Upload Failed: ' + LastResponse) ;
...
// check file arrived OK
    ...
    if NOT Connected then exit ;
    if fCancelFlag then exit ;
    MagDeleteFile (fnameftp, true) ;  // kill resume file
...
// successful upload
    ...
    doCopyEvent (LogLevelFile, 'Upload OK: ' + LocFileFull + ', size: ' + IntToKByte (uploadsize, true) +
                         ', duration ' + TimerToStr (SecsToTime (duration div 1000)) + ', average speed ' +
                           IntToKByte (CalcSpeed (duration, actualbytes)) + '/sec') ; // 10 Oct 2011 added duration and speed
    doCopyEvent (LogLevelDelimFile, LocFileFull + '|' + newtardir + fnametar +
...
// try and set file time stamp on server, few support command
 ...
 // immediate delete or move
    ...
    begin
        ...
        MagDeleteFile (LocFileFull, true) ;
    ...
    begin
        ...
        fnametar := strAddSlash (fUpArchDir) + ExtractFileName (LocFileFull) ;
        ret := RenameFile (LocFileFull, fnametar) ;
        if NOT ret then
        begin
            doCopyEvent (LogLevelFile, 'Failed to Move File to Archive Directory, Using Unique Name') ;
            fnametar := strAddSlash (fUpArchDir) + FormatDateTime ('"FTP-at-"yyyymmdd"-"hhnnss-z', Now) + ExtractFileExt (LocFileFull) ;
            ret := RenameFile (LocFileFull, fnametar) ;
        end ;
...
function TMagFtp.FtpUpOneFile (const LocFileFull, RemTarDir, RemTarFile: string; Replopt: TFileCopyRepl) : TTaskResult ;
var
...
// check if remote file available, get size and UTC time stamp, see if replacing it
    ...
    if flag and (replopt <> FCReplAlways) then
    begin
        flag := MagCheckReplace (replopt, true, OneSecond * 2, SrcFSize, RFSize, SrcFileUDT, RFileUDT) ;
...
function TMagFtp.FtpUpload (const CheckFiles: boolean): TTaskResult ;
var
...
// build list of source files on PC - before accessing server
    ...
    fMagFileCopy.Wow64RedirDisable := fWow64RedirDisable ;  // 22 May 2013
    fMagFileCopy.GetDirList (basesrcdir, newfname, fCopyType, fSubDirs, 0, 0, SrcFiles, SrcFileList, fEmptyDirs) ; // 17 Feb 2011) ;
    TotSrcFiles := SrcFileList.Count ;
    if TotSrcFiles = 0  then
    begin
        result := TaskResOKNone ;
...
// don't delete target files unless processing full directories
    ...
    if fDispLDir then doCopyEvent (LogLevelInfo, 'Source Files on PC' + CRLF_ + FmtFileDirList (SrcFileList, false)) ;

// logon to FTP server
    doCopyEvent (LogLevelInfo, 'Connecting to FTP Server: ' + FormatIpAddr (fHostName)) ;
    result := FtpLogon ;
    if result <> TaskResOKNew then exit ;
    doCopyEvent (LogLevelInfo, 'Succesfully logged onto FTP Server: ' + fUserName) ;
...
// 15 Sept 2008 don't stop, root may not exist
    ...
    fReqResponse := '' ;
    if fDispRDir then doCopyEvent (LogLevelInfo, 'Target Files on FTP Server' + CRLF_ + FmtFileDirList (TarFileList, false)) ;
    MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    if fCancelFlag then exit ;
...
// find size of stuff to copy
    ...
    begin
        ...
        begin
            listing.Capacity (TotSrcFiles * 50) ;   // no real idea yet
            listing.AppendLine (CRLF_ + 'Files Selected for Uploading are: ') ;
        end;
        for I := 0 to Pred (TotSrcFiles) do
        begin
            SrcFileRec := SrcFileList [I] ;
       ...
       if CheckFiles then listing.AppendLine (CRLF_ + 'No Source Files Selected to Upload') ;
    if CheckFiles then
    begin
        info := listing.ToString ;
        doCopyEvent (LogLevelInfo, info) ;
...
// stop now if only checking what will be uploaded - test only
    ...
    doCopyEvent (LogLevelInfo, 'Started FTP Upload to: ' + FormatIpAddr (fHostName)) ;
    MessagePump ; // 15 Sept 2010 needed to support MultiThreaded
    if fCancelFlag then exit ;
...
// see if deleting old target files first before uploading files
    ...
    begin
     ...
     // 22 Feb 2008 report now instead of after uploads, stop if no files to upload
        doCopyEvent (LogLevelInfo, 'Old target files deleted OK: ' + IntToCStr (fCopyProg.DelOKFiles) +
...
//  if NOT fCancelFlag then  14 Oct 2011 still report failure
    ...
    doCopyEvent (LogLevelInfo, 'Finished, files uploaded OK: ' + IntToCStr (fCopyProg.ProcOKFiles) + ', failed: ' +
                        IntToCStr (fCopyProg.ProcFailFiles) + ', skipped: ' + IntToCStr (fCopyProg.SkippedFiles)) ;
    doCopyEvent (LogLevelDelimTot, fSrcDir + '|' + fTarDir + '|' + IntToStr (totsize) + '|' +
                        IntToStr (fCopyProg.ProcOKFiles) + '|' + IntToStr (fCopyProg.ProcFailFiles) + '|Totals|') ;
    doCopyEvent (LogLevelInfo, 'Total size uploaded ' + IntToKByte (totsize, true) + ', duration ' +
     ...
     {   if fDelOKFiles <> 0 then doCopyEvent (LogLevelInfo,         22 Feb 2008 done earlier
            'Old target files deleted OK: ' + IntToCStr (fDelOKFiles) +
                        ', Total bytes deleted ' + IntToKByte (fDelOKBytes)) ;   }
...
constructor TMagFtpThread.CreateThread;
begin
...
procedure TMagFtpThread.CallThreadEvent ;  // called by Synchronise for main thread
var
    Cancel: boolean ;
begin
...
procedure TMagFtpThread.LogEvent (LogLevel: TLogLevel ; Info: String ; var Cancel: boolean) ;
begin
    ...
    begin
        if FLogmaskName <> '' then
            FBuffLogStream.WriteLine (FormatDateTime (LongTimeMask, Now) + space + FId + ': ' + Info) ;
    end;
...
procedure TMagFtpThread.IcsLogEvent (Sender: TObject; LogOption: TLogOption; const Msg : String) ;
begin
    if FLogmaskName <> '' then
        FBuffLogStream.WriteLine (FormatDateTime (LongTimeMask, Now) + space + FId + ': ' + Msg) ;
end ;

procedure TMagFtpThread.Execute;
var
    Cancel: boolean ;
begin
    ...
    if FLogmaskName <> '' then
    begin
        FBuffLogStream := TBuffLogStream.Create (FLogmaskName, '') ;  // Format mask for log file name
...
{$ENDIF}
    ...
    FInfo := 'FTP Thread Done, Task Result: ' + GetTaskResName (FTaskRes) + ' - ' + MagFTPClient.ReqResponse + CRLF_ ;
