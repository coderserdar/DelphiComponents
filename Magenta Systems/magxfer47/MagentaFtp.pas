unit MagentaFtp;
{
Updated by Angus Robertson, Magenta Systems Ltd, England, 13th July 2015
...
18 Nov 2008  - 2.5 - fixed PASS argument not being sanitised
17 May 2009  - 2.6 - don't ignore failed MD5/XMD5 command but report error, prefer MD5 to XMD5
8 Aug 2011   - 2.7 - now registered in MagentaXferReg
24 Aug 2012  - 2.8 - updated to support ICS V8 with IPv6
...
uses
  ...
  MagSubs1, MagSubs4, MagClasses, MagentaCopy,
...
// host type, for directory listing
    ...
    TMagFtpOpt  = (magftpNoFeat, magftpNoZlib, magftpNoMd5Crc, magftpNoTmpFile,
                   magftpNoUtf8, magftpIgnoreUtf8, magftpNoHost);   // 20 Sept 2008
  ...
  public
    { Public declarations }
        ...
        function UnpackFtpFDir (DirStream: TStream; RemDir, BaseDir: string;
                Level: integer ; var HostType: THostType; var TotFiles: integer;
                                                var RemFiles: TFileRecs): integer ;
        procedure FtpLogoff ;
...
{$IFDEF USE_SSL}
procedure TMagFtp.OnFTPSslVerifyPeer(Sender: TObject; var Ok: Integer;
  Cert : TX509Base);
begin
...
function TMagFtp.UnpackFtpFDir (DirStream: TStream; RemDir, BaseDir: string; Level: integer ;
       var HostType: THostType; var TotFiles: integer; var RemFiles: TFileRecs): integer ;
var
    FtpFList: TStringList ;
    initdlen: integer ;
Type
...
{   
Function ExtractFileNameOnly(S1:String):String;
Var
    FExt:String[255];
Begin
    S1:=ExtractFileName(S1);
    FExt:=ExtractFileExt(S1);
    ExtractFileNameOnly:=Copy(S1,1,Length(S1)-Length(FExt));
End; }
...
// load FTP file directory, make sure it's not empty
    ...
    Begin
        Inc(X);
    End;
    S1:=Strunc(FtpFList [X]);
    If (S1[Pos(':',S1)+1]='[') And (S1[Length(S1)]=']') Then
                                             HostType := FTPTYPE_MVS;

// keep year, for UNIX listing without it
    MyDateTime:=Date;
    DecodeDate(MyDateTime,CurYear,Month,Day);
    doCopyEvent (LogLevelDiag, 'Server Returned ' + IntToCStr (MaxX) +
     ' Line(s), Bytes ' + IntToCStr (DirStream.Size) + ' for Directory: ' + RemDir) ;

// read each line in the FTP filelist
    For X:=0 To MaxX-1 Do
    Begin
      // see if logging raw list, do it for single file anyway
        if fDispRemList or (MaxX = 1) then doCopyEvent (LogLevelDiag, FtpFList[X]) ;
        MyDate:='';
        MyTime:='';
        MyName:='';
        MyDir:='';
        ...
        if HostType = FTPTYPE_MLSD then
            S1 := FtpFList[X]       // get full line for processing
        else
        begin
            S1:=Strunc(Filter(FtpFList[X]));    // get filtered full line for processing
            Fifth:=GetFifth(S1);
            Second:=GetSecond(S1);
        end ;
        If S1<>'' Then
...
{$ENDIF}
    ...
    // toggle between two FTP servers
        ...
        fHostName := StripIpAddr (fHostName) ; // V2.8 strip [] off ipv6 address
...
{$ENDIF}
        doCopyEvent (LogLevelInfo, 'Connect/Logon to FTP Server: ' +
                                          FormatIpAddrPort (fHostName, fPort)) ; // v2.8
        fFtpErrFlag := false ;
        HostFileName := '' ;
        LocalFileName := '' ;
        ...
        begin
            ...
            ret := Pwd ;  // get initial working directory
            if fCancelFlag then ret := false ;
            if NOT ret then
            begin
        ...
        // set base directory, combining root and remote dir
                ...
                doCopyEvent (LogLevelInfo, 'Current Directory: ' + fCurRemDir) ;
            end ;
         ...
         // 8 Nov 2007 tell server who we are - before UTF8 to keep Gene6 happy
            ...
            // 22 Sept 2008 Unicode/UTF8 - only turn it off - note some servers ignore ON/OFF and always turn on
...
function TMagFtp.FtpDir (var FtpFiles: TFileRecs;
                                    var FtpFileList: TFindList): TTaskResult ;
var
    ...
    begin
        ...
        begin
            ...
            UnpackFtpFDir (LocalStream, fullpath, fServBaseDir,
                                             level, fHostType, totfiles, FtpFiles) ;
    ...
    try
        ...
        if result in [TaskResOKNew, TaskResOKNone] then
                    dirlisting := 'Remote Directory: ' + fSrcDir +
                                    CRLF_ + FmtFileDirList (DirFileList, false) ;
    finally
...
procedure TMagFtp.onMagCopyEvent (LogLevel: TLogLevel ; Info: string ;
                                                  var Cancel: boolean) ;
begin
...
procedure TMagFtp.onFtpClientProg64(Sender: TObject; Count: Int64;
                                                     var Abort: Boolean);
var
    temp: string ;
begin
...
begin
    ...
    begin
        if (temp [1] = '<') then exit ;   // ignore response
    end ;
...
{
procedure TMagFtp.OnFtpCommand (Sender: TObject; var Cmd: String) ;
var
    temp: string ;
begin
    temp := Cmd ;
...
begin
// 25 Nov 2007 ignore continuation responsed for FEAT and SITE INDEX/CMLSD
...
function TMagFtp.FtpCheckFile (const RemDir, RemFile: string ;
                                 var FSize: Int64; var FileUDT: TDateTime): boolean;
var
    DirStream: TStream ;
    FtpFiles: TFileRecs;
 ...
 // get latest file size and time stamp
    ...
    // 17 Sept 2008 WS_FTP MLST with 501 Invalid number of arguments with spaces, so try quoting file name
     ...
     //  15 March 2006, check we got some facts - parse may have failed
        ...
        begin
            DecodeMlsResp64 (facts, RFname, RFType, RFAttr, FSize, FileUDT) ;
        end
        else
...
function TMagFtp.IntDownOne (const RemDir, RemFile, RemFull, LocFileFull: string;
                               const RFSize: Int64; RFileUDT: TDateTime): integer ;
var
   ...
   // see if resume info file exists, check file on server has same info
                ...
                begin
                    ...
                    begin
                        if (ResInfRecs [ResInfServer] = fHostName1) and
                           (ResInfRecs [ResInfFName] = RemFull) and
        ...
        begin
         // write current file into into .ftp file so we can resume if necessary
            ...
            ResInfRecs.Add (RemFull) ;                  // ResInfFName = 1
 ...
 // check MD5 or CRC if possible and repeat if allowed
    ...
    begin
        ...
        begin
            ...
            info := FileMD5 (fnametmp, Self, MD5Progress) ;
    ...
    begin
     ...
     // PosEnd := newsize ;
        ...
        begin
            ...
            doCopyEvent (LogLevelProg, fProgMessBase) ;
            fProgFileSize := newsize ;   // keep name and size for event handler
            info := FileCRC32B (fnametmp, Self, MD5Progress) ;
...
// check if remote file available, get size and UTC time stamp
    ...
    // see if replacing existing file
        flag := GetUAgeSizeFile (Fnametar, TarFileDT, TarFSize) ;
...
// build list of source files on FTP server
    ...
    result := FtpDir (SrcFiles, SrcFileList) ;
    if result in [TaskResFail, TaskResAbort] then exit ;
    if fDispRDir then doCopyEvent (LogLevelInfo,
           'Source Files on FTP Server' + CRLF + FmtFileDirList (SrcFileList, false)) ;
    TotSrcFiles := SrcFileList.Count ;
    doCopyEvent (LogLevelFile, 'Source Directory: ' + fSrcDir) ;
...
// build list of target files, so we don't copy unnecessary stuff
    doCopyEvent (LogLevelFile, 'Target Directory: ' + fTarDir) ;
    if NOT ForceDirs (fTarDir) then
    begin
        result := TaskResFail ;
    ...
    if fSpecificFiles then
        ...
        newfname := fSrcFName ;
    TotTarFiles := fMagFileCopy.GetDirList (fTarDir, newfname, fCopyType,
                                    fSubDirs, 0, 0, TarFiles, TarFileList) ;
    if fDispLDir then doCopyEvent (LogLevelInfo,
                'Target Files on PC' + CRLF + FmtFileDirList (TarFileList, false)) ;
    Application.ProcessMessages ;
    if fCancelFlag then exit ;
...
// find size of stuff to copy
    info := info + CRLF + 'Files Selected for Downloading are: ' + CRLF ;
    newsize := 0 ;
    for I := 0 to Pred (TotSrcFiles) do
    begin
        SrcFileRec := SrcFileList [I] ;
...
// see if deleting old target files first
    if fDelOldTar and (CurDelFiles <> 0) then
    begin
        doCopyEvent (LogLevelInfo, 'Deleting Old Local Files: ' + fTarDir) ;
        for I := 0 to Pred (TotTarFiles) do
        begin
            Application.ProcessMessages ;
            if fCancelFlag then exit ;
    ...
    begin
        ...
        //    fnamesrc := FrFullName ;
            ...
            begin
                ...
                if CaseFile = FileLowerCase then
                                        newtardir := AnsiLowerCase (newtardir) ;
            end ;
...
function TMagFtp.IntUpOne (const LocFileFull, RemDir, RemFile: string;
                                  const RFSize: Int64; RFileUDT: TDateTime): integer ;
var
...
// 22 Sept 2008 skip Unicode names with substitution characters
    if (Pos ('?', LocFileFull) > 0) then
    begin
        doCopyEvent (LogLevelDiag, 'Skipped Inaccessible Unicode Name: ' + LocFileFull) ;
...
// start upload, to temporary file, resuming if possible
    ...
    fProgMessBase :=  fProgMessBase + CRLF + RemFile ;
...
// check file arrived OK
 ...
 // check MD5 or CRC32 if possible (not from zip stream yet) and repeat if allowed
    ...
    begin
        ...
        PosStart := 0 ;  // force XMD5 to do entire file, in case size changed 
        PosEnd := 0 ;
        if (ftpFeatMD5 in FSupportedExtensions) then  // 22 Nov 2007 support XMD5
            ret := MD5
        else
        ...
        begin
            ...
            doCopyEvent (LogLevelProg, fProgMessBase) ;
            fProgFileSize := uploadsize ;   // keep name and size for event handler
            info := FileMD5 (LocFileFull, Self, MD5Progress) ;
    ...
    // PosEnd := uploadsize ;
        ...
        begin
            ...
            doCopyEvent (LogLevelProg, fProgMessBase) ;
            fProgFileSize := uploadsize ;   // keep name and size for event handler
            info := FileCRC32B (LocFileFull, Self, MD5Progress) ;
            if (Length (info) = 8) and (Pos (fCrcResult, info) = 0) then
            begin
                doCopyEvent (LogLevelInfo, 'CRC32 Compare Failed: ' +
 ...
 // immediate delete or move
    ...
    begin
        ...
        begin
            ...
            fnametar := strAddSlash (fUpArchDir) + FormatDateTime
                  ('"FTP-at-"yyyymmdd"-"hhnnss-z', Now) + ExtractFileExt (LocFileFull) ;
            ret := RenameFile (LocFileFull, fnametar) ;
        end ;
...
function TMagFtp.FtpUpOneFile (const LocFileFull, RemTarDir, RemTarFile: string;
                                            Replopt: TFileCopyRepl) : TTaskResult ;
var
...
// 22 Sept 2008 skip Unicode names with substitution characters
    ...
    if NOT GetUAgeSizeFile (LocFileFull, SrcFileUDT, SrcFSize) then
    begin
        result := TaskResFail ;
...
var
    ...
    ret: boolean ;
    tempdir, info: string ;
    I, J, donenr, CurDelFiles, nodeltot, loop, retval: integer ;
...
// build list of source files on PC - before accessing server
    ...
    if fSpecificFiles then
        ...
        newfname := fSrcFName ;
    fMagFileCopy.GetDirList (fSrcDir, newfname, fCopyType,
...
// don't delete target files unless processing full directories
    ...
    if fDispLDir then doCopyEvent (LogLevelInfo,
                'Source Files on PC' + CRLF + FmtFileDirList (SrcFileList, false)) ;
...
// ignore errors, there may be nothing on server, yet
    doCopyEvent (LogLevelInfo, 'Checking files already on FTP Server') ;
...
// 15 Sept 2008 don't stop, root may not exist
    ...
    fReqResponse := '' ;
    if fDispRDir then doCopyEvent (LogLevelInfo,
           'Target Files on FTP Server' + CRLF + FmtFileDirList (TarFileList, false)) ;
    Application.ProcessMessages ;
    if fCancelFlag then exit ;
...
// find size of stuff to copy
    ...
    if (fTotProcFiles > 0) then
    begin
        info := info + CRLF + 'Files Selected for Uploading are: ' + CRLF ;
        for I := 0 to Pred (TotSrcFiles) do
        begin
            SrcFileRec := SrcFileList [I] ;
            with SrcFileRec^ do
       ...
       info := info + CRLF + 'No Source Files Selected to Upload' + CRLF ;
    if CheckFiles then doCopyEvent (LogLevelInfo, info) ;
    doCopyEvent (LogLevelInfo, 'Source Files Skipped ' +
                                            IntToStr (fSkippedFiles)) ;
