...
// build sorted list of source records, then remove duplicates
    ...
    begin
        if PTFileRecW (SrcFileList [I]).FrFullName = PTFileRecW (SrcFileList [Pred (I)]).FrFullName then
...
// show user the list we found
    if fLogRDir then doCopyEvent (LogLevelInfo, CRLF + 'Source HTTP Files:' + CRLF_ + FmtFileDirListW (SrcFileList, false)) ;
    Application.ProcessMessages ;
    if fCancelFlag then exit ;
...
// build list of target files, so we don't copy unnecessary stuff
    ...
    if fLogLDir then doCopyEvent (LogLevelInfo, 'Target Files on PC' + CRLF_ + FmtFileDirListW (TarFileList, false)) ;
    Application.ProcessMessages ;
    if fCancelFlag then exit ;
    ...
    begin
        ...
        begin
            ...
            begin
                ...
                if CheckFiles then info := info + FrFullName + ', Size ' + IntToCStr (FrFileBytes) + CRLF_ ;
            end ;
        end ;
    end ;
          ...
          //      DownloadStream.SetSize (FileBytes) ;  // not sure if this is efficient
            ...
            begin
               ...
               // replace old file, removing read only if necessary
                        ...
                        doCopyEvent (LogLevelInfo, 'Error Saving File: ' + fnametar + ', ' + GetExceptMess (ExceptObject)) ;
                        inc (fProcFailFiles) ;
                        FrFileCopy := FCStateFailed ;
                      //  statcode := 0 ;
                    end ;
              ...
              // see if unzipping it
                    ...
                    begin
                        ...
                        // List filenames in zip file
                                ...
                                begin
                                   info := info + Format (sDirLine, [Filename [J], IntToCStr (UnCompressedSize [J]), ' ',
