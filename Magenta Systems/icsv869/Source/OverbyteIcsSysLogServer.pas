...
Apr 1, 2022 V8.69  Allow / in syslog messages.
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
// Accept alphanumeric, underline and dot characters  V8.69 allow /
    ...
    while (Index > 0) and
          ...
          (
           ...
           ((RawMessage[Index] >= '0') and (RawMessage[Index] <= '9')) or
            ...
            (RawMessage[Index]  = '_') or (RawMessage[Index]  = '/')         { V8.69 added / }
          ) do
        Inc(Index);
    if Index <= I then
        Result := ''
...
var
    RawMessage : array [0..3000] of AnsiChar;  { V8.69 fixed size buffer }
    Len        : Integer;
    Src        : TSockAddrIn;
    SrcLen     : Integer;
    SrcIP      : AnsiString;
...
//  SetLength(RawMessage, Len);
    Len    := FUdpSocket.ReceiveFrom(@RawMessage, Length(RawMessage),
                                     Src, SrcLen);
    if Len <= 0 then
        Exit;
//    SetLength(RawMessage, Len);
    ...
    if Assigned(FOnDataAvailable) then
        FOnDataAvailable(Self, SrcIP, SrcPort, Copy(AnsiString(RawMessage), 1, Len));
end;
