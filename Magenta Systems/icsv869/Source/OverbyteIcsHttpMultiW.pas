...
const
    HttpMultiCopyRight : String = ' TIcsHttpMultiW (c) 2022 V8.69 ';
type
...
procedure TIcsHttpMultiW.IcsProgEvent (Sender: TObject; LogOption: TLogOption; const Msg : String) ;   { V8.69 }
begin
    doCopyEvent (LogLevelInfo, Msg) ;
end ;


procedure TIcsHttpMultiW.onHttpBgException(Sender: TObject; E: Exception; var CanClose: Boolean);    { V8.68 }
begin
...
procedure TIcsHttpMultiW.onHttpStateChange(Sender : TObject);     { V8.68 }
begin
...
procedure TIcsHttpMultiW.onHttpSessionClosed(Sender: TObject);    { V8.68 }
begin
...
var
    ...
    ret: boolean ;
    tempdir, info, curURL, curresp, fnametemp: string ;
...
begin
    ...
    // start real HTTP downloading
        ...
        begin
            ...
            // V8.68 allocate memory for download stream before doing request, so it does not fail due to lack of memory
        //      DownloadStream.Clear ;  // don't free memory
      (*          newsize := FrFileBytes + 16;
                if newsize <= 0 then newsize := 320000;  // guess size if not available
//                newsize := 2000000000;  // !! 2GB TEMP
                try
                    if DownloadStream.Size < newsize then
                        DownloadStream.SetSize (newsize);  // allocate more memory for stream, if needed
          //          if DownloadStream.Size <> newsize then
          //              doCopyEvent (LogLevelInfo, 'Mismatch Memory for Download, Request=' + IntToKbyte(newsize) + ' Size=' + IntToKbyte(DownloadStream.Size)) ;
                except
                    doCopyEvent (LogLevelInfo, 'Insufficient Free Memory for Download: ' + IntToKbyte(newsize)) ;
                    inc (fProcFailFiles) ;
                    FrFileCopy := FCStateFailed ;
                    DownloadStream.Clear ;
                    continue ;
                end;
                DownloadStream.Position := 0;    // reset to write at start of stream
             *)
                RcvdStream := DownloadStream ;
                RequestVer := '1.1';          // V8.66
                Connection := 'keep-alive';   // V8.66
