...
Legal issues: Copyright (C) 2011-2020 by Arno Garrels, Berlin, Germany,
...
Dec 09, 2020 - V8.65 - Corrected bad $MESSAGES typo.
                       Use AtomicIncrement instead of InterlockedIncrement,
                       AtomicDecrement instead of InterlockedDecrement.
                       Use TInterlocked.Exchange and TInterlocked.Add.
                       Made AppKit Cocoa related code MACOS only, Linux
                         needs a new Message Pump implemented so this unit
                         is now broken for Linux.
                       Make sure ThreadID is always TThreadId.
                       Rename unit from Ics.Posix.Messages.pas to
                         Ics.Posix.PXMessages.pas to avoid conflicts with
                         projects loading it instead of WinApi.Messages,
      ...
      Pending - think FMXLinux TPlatformLinux class contains a message pump
                 and timer so don't need this unit, but all undocumented.
                 Note MACOS code was written for 32-bit and needs updating
                 for current 64-bit OS.

 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}

unit Ics.Posix.PXMessages;
...
//                                                    Pointer(LocalSyncList));
    Pointer(LocalSyncList) := TInterlocked.Exchange(Pointer(FSyncList),
                                                    Pointer(LocalSyncList));   { V8.65 }
    try
...
var
  Thread: PThreadInfo;
  HasReadLock: Boolean;
  ThreadID: TThreadID;    { V8.65 was cardinal }
  Test: Integer;
...
//      InterlockedIncrement(FSentinel);
      AtomicIncrement(FSentinel);  { V8.65 uses compiler magic if available }
...
//    while InterlockedExchangeAdd(FSentinel, -mrWriteRequest) <> mrWriteRequest do
    while TInterlocked.Add(FSentinel, -mrWriteRequest) <> mrWriteRequest do  { V8.65 }
    begin
{$IFDEF DEBUG_MREWS}
      Debug('Write loop');
...
//      Test := InterlockedExchangeAdd(FSentinel, mrWriteRequest);
      Test := TInterlocked.Add(FSentinel, mrWriteRequest);               { V8.65 }
...
//    InterlockedDecrement(FSentinel);
      AtomicDecrement(FSentinel);  { V8.65 uses compiler magic if available }

    FWriterID := ThreadID;

//    Result := Integer(OldRevisionLevel) = (InterlockedIncrement(Integer(FRevisionLevel)) - 1);
    Result := Integer(OldRevisionLevel) = (AtomicIncrement(Integer(FRevisionLevel)) - 1);     { V8.65 uses compiler magic if available }
  end;
...
//    InterlockedExchangeAdd(FSentinel, mrWriteRequest);
    TInterlocked.Add(FSentinel, mrWriteRequest);      { V8.65 }
    {$IFDEF DEBUG_MREWS}
...
{$ENDIF DEBUG_MREWS}
 ...
 //     while (InterlockedDecrement(FSentinel) <= 0) do
      while (AtomicDecrement(FSentinel) <= 0) do        { V8.65 }
      begin
  {$IFDEF DEBUG_MREWS}
        Debug('Read loop');
  {$ENDIF DEBUG_MREWS}
...
//      Test := InterlockedIncrement(FSentinel);
      Test := AtomicIncrement(FSentinel);   { V8.65 uses compiler magic if available }
