...
//  if ReadString (section, 'ShowDiagsUtf8_Checked', 'False') = 'True' then ShowDiagsUtf8.Checked := true else ShowDiagsUtf8.Checked := false ;
  ...
  CleanupDays.Text := ReadString (section, 'CleanupDays_Value', CleanupDays.Text) ;                                                         { V8.68 }
  CleanupDateFrom.Date := ReadFloat (section, 'CleanupDateFrom_Date', CleanupDateFrom.Date) ;                                                  { V8.68 }
  CleanupDateTo.Date := ReadFloat (section, 'CleanupDateTo_Date', CleanupDateTo.Date) ;                                                        { V8.68 }
  CleanupFileMask.Text := ReadString (section, 'CleanupFileMask_Text', CleanupFileMask.Text) ;                                                 { V8.68 }
  if ReadString (section, 'CleanupSubDirs_Checked', 'False') = 'True' then CleanupSubDirs.Checked := true else CleanupSubDirs.Checked := false ;  { V8.68 }
  CleanupPath.Text := ReadString (section, 'CleanupPath_Text', CleanupPath.Text) ;                                                             { V8.68 }
    end ;
    finally
        IniFile.Free ;
    end ;
...
//  if ShowDiagsUtf8.Checked then temp := 'True' else temp := 'False' ; WriteString (section, 'ShowDiagsUtf8_Checked', temp) ;
  ...
  WriteString (section, 'CleanupDays_Value', CleanupDays.Text) ;                                                             { V8.68 }
  WriteFloat (section, 'CleanupDateFrom_Date', CleanupDateFrom.Date) ;                                                         { V8.68 }
  WriteFloat (section, 'CleanupDateTo_Date', CleanupDateTo.Date) ;                                                             { V8.68 }
  WriteString (section, 'CleanupFileMask_Text', CleanupFileMask.Text) ;                                                        { V8.68 }
  if CleanupSubDirs.Checked then temp := 'True' else temp := 'False' ; WriteString (section, 'CleanupSubDirs_Checked', temp) ; { V8.68 }
