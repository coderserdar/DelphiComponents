...
begin
    ...
    if MagMailQueue.UnQueueMail (AscToInt (item)) then
...
begin
    ...
    if MagMailQueue.MailTotItems = 0 then
    begin
        LabelState.Caption := 'Queue State: Empty' ;
        exit ;
    end;
    LabelState.Caption := 'Queue State: ' + IntToStr (MagMailQueue.MailTotItems) + ' Waiting Mail Items' ;
    for I := 0 to MagMailQueue.MailTotItems - 1 do
    begin
        MailQuItem := PMailQuItem (MagMailQueue.MailQuIdx [I])^ ;
        ...
        begin
            ...
            SubItems.Add (Int2Kbytes (BodySize)) ;
