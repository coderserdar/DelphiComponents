...
uses
  ...
  Dialogs, StdCtrls, magrasedtw, magrasconw, MagRasEntw, Spin, ExtCtrls, magrasedt, magrascon;
...
begin
    ...
    for I := 0 to Pred (MagRasNumEntryRec) do
            Entries.Items.Add (MagRasEntryRecs [I].EntryName) ;
end;
...
begin
    ...
    // silly telephone number
        MagRasEdt.LocalPhoneNumber := '1111' + IntToStr (I) ;

	// device stuff - required, must match precisely name and type from lists
    	MagRasEdt.DeviceName := MagRasCon.DeviceNameList [Device.ItemIndex] ;
    	MagRasEdt.DeviceType := MagRasCon.DeviceTypeList [Device.ItemIndex] ;

    // create new entry
        ret := MagRasEdt.PutAllEntryProps (newname) ;
        if ret <> 0 then
        begin
...
begin
    ...
    begin
        ...
        begin
            ...
            Application.ProcessMessages ;
            MagRasCon.EntryName := Entries.Items [I] ;
