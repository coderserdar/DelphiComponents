...
connections.  TMagRas supports Windows 2000 and later, allowing an
application to support all the various RAS extensions in NT and W2K,
including sub entries for multi-channel ISDN connections. Performance
statistics are returned separately for each connection.
...
Changed wDevicePort from WideString to UnicodeString
...
Changes in 6.33
Recognise Windows 8, 10, 2012 and 2016

Changes in 6.40
Windows 10 version 1703 (March 2017) broke RAS modem dialling, add FixWin10Ras
  method to detect this and fix the registry, a reboot will be required .
Beware Microsoft could fix this bug in a different way so this change
  may need reviewing in a few months.
TransAddr and TranslateAddress now check for an IP address or host name and
  don't attempt to generate a dialling string
Corrected Windows 10/2016 version display 
...
uses
  ...
  MagRasApiW, MagTapiApiW, MagRasStr, MagRasEntW,
  MagSubs1, MagClasses ;

const
  MagVersion = 'TMagRasCon, 6th June 2017 - Release 6.40, Copyright 2017, Magenta Systems Ltd' ;
...
{ A common buffer used for RasEntryAll }
  EntryInfo: array [0..8190] of byte ;          // 4.62 Angus
...
{ Used for reading and writing phonebook sub entries, followed by a variable number
 ...
 normally necessary to access this information, it is public primarily for
 debugging purposes.  Used by GetEntryProperties and PutEntryProperties.  }
  RasSubEntryNT4: TRasSubEntryNT4W absolute SubEntryInfo ;  // 3.2 Angus
...
//DeviceInfo: array [0..255] of byte ;      // 3.1 Angus
...
{ Array used to temporarily read active connections for all OSs, do
  ...
  RasConnectAll: array[1..MaxConnections] of TRasConnXPW ; // Angus 4.62

{ Array used to temporarily read active connections for Win9x, NT4 and W2K do
  not read it, used by GetConnection. }
  RasConnect: array[1..MaxConnections] of TRasConnW absolute RasConnectAll ; // Angus 3.2
  RasConnectNT4: array[1..MaxConnections] of TRasConnNT4W absolute RasConnectAll ; // Angus 3.2
  RasConnectW2K: array[1..MaxConnections] of TRasConnW2KW absolute RasConnectAll ; // Angus 4.62
  RasConnectXP: array[1..MaxConnections] of TRasConnXPW absolute RasConnectAll ; // Angus 4.92
...
{ Class containing details of active connection, set by GetConnections.   }
  TConnectionList = class(TList)
    function AddConnection(Connection: TRasConnXPW): Word;
    ...
    function LogonSessId(Index: Integer): TLargeInteger ;    // 4.62
    function GetConn(Index: Integer): TRasConnXPW;
    procedure ClearFree;
  ...
  private
    ...
    fEntryNameW,
    fPhoneBookPathW: WideString ;  // 6.0
    ...
    RasDialParamsNT4: TRasDialParamsNT4W;
...
// Angus - more useful things
    ...
    fCallbackId: pointer ;      // Angus 6.30 was integer
    fCallbackIdDummy: integer ; // Angus 6.30 copy of fCallbackId for backward compatibility
    fTotSubEntries: integer ;   // Angus 3.2
    fDialMode: integer ;        // Angus 3.2
    fCurDevName: string ;       // Angus 3.2  reported by RasEnumConnections
    fCurDevType: string ;       // Angus 3.2  reported by RasEnumConnections
    fCurPhonebook: string ;     // Angus 3.2  reported by RasEnumConnections
    ...
    fConnectPhoneNr: string ;   // Angus 3.2 reported by GetConnectStatus, etc (NT4)
    fPPPError: integer ;        // Angus 4.0 reported by GetIPAddress
    fConnProtocol: string ;     // Angus 4.0 reported by GetIPAddress
    fPPPReplyMessage: string ;  // Angus 4.0 reported by GetIPAddress
    fStateEventSource: TStateEventSource ; // Angus 4.0, set before StateChanged
    fCountryCode: integer ;      // Angus 4.0
    fAreaCode: string ;         // Angus 4.0
    ...
    fLocalPhoneNumber: string ;     // Angus 4.30 local phone number from connection
//    OsInfo: TOSVERSIONINFO ;    // Angus 4.31 removed 5.20 using MagSubs1 extended version
    fPBPtr: Pointer ;           // Angus 4.60 Pointer to fPhoneBookPathW or Nil for Win9x
...
// string lists
    ...
    fSubLocalPhoneNumber: array [1..MaxSubEntries] of string ; // Angus 4.0
    fSubPhoneCanonical: array [1..MaxSubEntries] of string ;   // Angus 4.0
    fSubDeviceType: array [1..MaxSubEntries] of string ;       // Angus 4.0
    fSubDeviceName: array [1..MaxSubEntries] of string ;       // Angus 4.0
    fSubDevicePort: array [1..MaxSubEntries] of string ;       // Angus 4.0
...
// queue used to buffer state event changes that happen too fast for application to process, 4.50
    ...
    procedure SetEntryName (Value: String) ;  // 6.0
...
{ Check if RASAPI32.DLL is available without calling any functions. Not
  ...
  current OS as: OSW9x, OSNT4, OSW2K or OSWXP, OSVista, OS7, OS8.
  some RAS properties are only  available on newer OSs.  }
...
{ Creates a canonical phone number from CountryCode, AreaCode and
  ...
  the number in a dialable number with optional calling card, etc }
    function GetCanonical (UseCountryAndAreaCodes: boolean; CountryCode: DWORD;
                                AreaCode, LocalPhoneNumber: string): string ;
...
{ Copies an entry previously read with GetEntryProperties to create a new
  ...
  Calls ValidateName and PutEntryProperties. }
    function CopyPhonebook (Newname: string): LongInt ; // 3.1 ANGUS
...
{ Splits a canonical phone number into CountryCode, AreaCode,
  ...
  '+country code (area code) local number'. }
    procedure UnpackCanonical (PhoneCanonical: string;
...
{ Encapsulates the RasGetCountryInfo function to retrieve country specific
  ...
  Result is 0 if successful, or an error code.}
    function GetOneCountryInfo (CountryId: integer;
            var CountryCode: integer; var CountryName: string) : integer; // Angus 4.0
...
{ Encapsulates the lineTranslateDialog function to display the Dialing
  ...
  on a phone number about to be dialed.   This dialog is called 'Phone and
  Modem Options' in XP, with 'Dialling Rules' for My Location. 
...
{ Encapsulates the lineTranslateAddress function which translates the
  ...
  Calls InitTAPI. }
    function TransAddr (Devid: integer; InputNum: string;           // 4.3
              Card, Flags: integer ; var DisNum, DialNum: string): Boolean ;
...
{ Ensure at least one dialling location exists otherwise create a default
  ...
  Returns true if a location exists or was created OK.}
    function SetDefDialLocation: boolean ;  // 6.30

{ Windows 10 version 1703 (March 2017) broke RAS modem dialling so this
  function checks the registry for the new RequiredPrivileges, and updates
  it if neccesary. Needs Admin rights to update the registry, and a reboot
  afterwards before dialling will work again.  }

    function FixWin10Ras(var DialupBroken, RebootNeeded: Boolean): string;  // 6.4
...
{ Helper to information from an array. }
    function GetSubLocalPhoneNumber (Index: Integer): string ;

{ Helper to information from an array. }
    function GetSubPhoneCanonical (Index: Integer): string ;

{ Helper to information from an array. }
    function GetSubDeviceType (Index: Integer): string ;

{ Helper to information from an array. }
    function GetSubDeviceName (Index: Integer): string ;

{ Helper to information from an array. }
    function GetSubDevicePort (Index: Integer): string ;
...
{ Contains the information on protocols negotiated, varies between windows
  ...
  Set by GetProtocolEx. }
    PROPERTY ConnProtocol: string     read fConnProtocol ;          // Angus 3.2

//    PROPERTY PPPMultilink: boolean    read fPPPMultilink ;          // Angus 3.2

{ Reply message from a LCP/multi-link negotiation.
Set by GetProtocolEx. }
    PROPERTY PPPReplyMessage: string  read fPPPReplyMessage ;       // Angus 3.2
...
{ Specifies a telephone number for a sub entry in a phonebook entry.
  ...
  Used by GetEntryProperties. }
    PROPERTY SubLocalPhoneNumber [index: integer]: string read GetSubLocalPhoneNumber ;  // Angus 4.10
...
{ Specifies the canonical phone number for a sub entry in a phonebook entry.
  ...
  Used by GetEntryProperties. }
    PROPERTY SubPhoneCanonical [index: integer]: string read GetSubPhoneCanonical ; // Angus 4.10
...
{ Specifies the RAS device type for DeviceName for a sub entry in a phonebook.
  ...
  Returned by GetEntryProperties. }
    PROPERTY SubDeviceType [index: integer]: string read GetSubDeviceType ; // Angus 4.10

{ Specifies the name of a TAPI device use with a sub entry in a phonebook entry.

  Returned by GetEntryProperties. }
    PROPERTY SubDeviceName [index: integer]: string read GetSubDeviceName ; // Angus 4.10

{ Specifies the com port of a TAPI device use for a sub entry in a phonebook
  entry, NT4 only.

  Returned by GetEntryProperties. }
    PROPERTY SubDevicePort [index: integer]: string read GetSubDevicePort ; // Angus 4.10
...
{ Specifies the phonebook entry to use for various methods. Note that entries
 ...
 SetDialParams, EditPhonebook. DeletePhonebook, RenamePhonebook. }
    PROPERTY EntryName: String        read fEntryName write SetEntryName; // 6.0 keep widestring version
    PROPERTY EntryNameW: WideString   read fEntryNameW;                   // 6.0 get widestring version
...
{ Specifies the full path and filename of a phonebook (.PBK)
 ...
 SetDialParams, EditPhonebook. DeletePhonebook, RenamePhonebook. }
    ...
    PROPERTY PhoneBookPathW:  WideString  read fPhoneBookPathW;  // 6.0
...
{ Specifies the canonical phone number for a phonebook entry.  This is
  ...
  Used by GetEntryProperties and UpdatePhonebook. }
    PROPERTY PhoneCanonical: string   read fPhoneCanonical write fPhoneCanonical ;  // Angus
...
{ Note this is a dummy property and always blank. }
    PROPERTY CallbackId: integer      read fCallbackIdDummy write fCallbackIdDummy ; // Angus 6.30 publish dummy for backward compatibility
...
{ Specifies the area code for a phonebook entry.  If the dialing location
  ...
  Used by GetEntryProperties and PutPhonebook. }
    PROPERTY AreaCode: string         read fAreaCode write fAreaCode ;    // Angus 4.0
...
{ Specifies a telephone number for a phonebook entry. The way RAS uses this
  ...
  Used by GetEntryProperties and PutPhonebook. }
    PROPERTY LocalPhoneNumber: string read fLocalPhoneNumber write fLocalPhoneNumber; // Angus 4.30
...
function TConnectionList.AddConnection (Connection: TRasConnXPW): Word;
var
    Conn: PRasConnXPW;
begin
    Conn := New (PRasConnXPW);
    Conn^ := Connection;
    Add (Conn);
    result := 0 ;
end;
...
begin
    ...
    result := PRasConnXPW (Items[Index])^.RASConn; // handle
end;
...
begin
    result := '' ;
    if NOT CheckIndex (Index) then exit ;
    if PRasConnXPW (Items [Index])^.szEntryName [0] <> #0 then
                result := PRasConnXPW (Items [Index])^.szEntryName ;
end;
...
begin
    result := '' ;
    if NOT CheckIndex (Index) then exit ;
    if PRasConnXPW (Items [Index])^.szDeviceType [0] <> #0 then
        result := Lowercase (PRasConnXPW (Items [Index])^.szDeviceType) ;
end;
...
begin
    result := '' ;
    if NOT CheckIndex (Index) then exit ;
    if PRasConnXPW (Items [Index])^.szDeviceName [0] <> #0 then
                result := PRasConnXPW (Items [Index])^.szDeviceName ;
end;
...
begin
    result := '' ;
    if NOT CheckIndex (Index) then exit ;
    if PRasConnXPW (Items [Index])^.szPhonebook [0] <> #0 then
                result := PRasConnXPW (Items [Index])^.szPhonebook ;
end;
...
begin
    result := 0 ;
    if NOT CheckIndex (Index) then exit ;
    result := PRasConnXPW (Items [Index])^.dwSubEntry ;
end;
...
begin
    ...
    if NOT CheckIndex (Index) then exit ;
    result := PRasConnXPW (Items [Index])^.guidEntry ;
end;
...
begin
    result := 0 ;
    if NOT CheckIndex (Index) then exit ;
    result := PRasConnXPW (Items [Index])^.dwFlags ;
end;

function TConnectionList.LogonSessId(Index: Integer): TLargeInteger ;    // 4.62
begin
    result := 0 ;
    if NOT CheckIndex (Index) then exit ;
    result := PRasConnXPW (Items [Index])^.luid ;
end;

function TConnectionList.GetConn(Index: Integer): TRasConnXPW;
begin
    FillChar (result, SizeOf (result), #0) ;
    if NOT CheckIndex (Index) then exit ;
    result := PRasConnXPW (Items [Index])^ ;
end;
...
begin
    if NOT CheckIndex (Index) then exit ;
    Dispose (PRasConnXPW (Items [Index]));
    Items [Index] := Nil;
...
begin
  ...
  // should bDefaultCreds be set true for AllUsers?????
    ...
    // changed LOCALE_IDEFAULTCOUNTRY (obsoleted) to LOCALE_ICOUNTRY - Angus 6.10
...
//          if fDUNVersion [3] > '1' then fDUNInfo := 'DUN ' + OsInfo.szCSDVersion ;
        end
        ...
        begin
            ...
            else if fDUNVersion [3] = '1' then  // 6.10
            begin
                if OsInfo.wProductType <= VER_NT_WORKSTATION then
            ...
            else if fDUNVersion [3] = '2' then  // 6.33
            begin
                if OsInfo.wProductType <= VER_NT_WORKSTATION then
                    fDUNInfo := 'DUN Windows 8 ' + OsInfo.szCSDVersion
                else
                    fDUNInfo := 'DUN Win2012 ' + OsInfo.szCSDVersion ;
            end
            else if fDUNVersion [3] = '3' then  // 6.33
            begin
                if OsInfo.wProductType <= VER_NT_WORKSTATION then
        ...
        else if Pos ('10.', fDUNVersion) = 1 then // 6.40
        begin
            if OsInfo.wProductType <= VER_NT_WORKSTATION then
                fDUNInfo := 'DUN Windows 10 ' + OsInfo.szCSDVersion
            else
                fDUNInfo := 'DUN Win2016 ' + OsInfo.szCSDVersion ;
        end
        else fDUNInfo := 'DUN ' + OsInfo.szCSDVersion ;
    end ;
...
function TMagRasCon.GetCanonical (UseCountryAndAreaCodes: boolean;
            CountryCode: DWORD; AreaCode, LocalPhoneNumber: string): string ;
begin
...
procedure TMagRasCon.SetEntryName (Value: String) ;  // 6.0
begin
    fEntryName := Value ;
    fEntryNameW := Value ;
end ;
...
//    tempkey: HKEY ;
//    keyname: string ;
    wDevicePort: UnicodeString ;   // 6.10
    tempsize {, dwType, dwSize, dwInfo}: DWORD ;
//    reginfo: array [1..400] of char ;
begin
...
// NT returns more info, but first part of record is identical to Win9x
    ...
    if MagRasOSVersion >= OSWXP then                    // 14 Sept 2012 avoid specific OSs
        RasEntryAll.dwSize := Sizeof (TRasEntryXPW)
     else
        RasEntryAll.dwSize := Sizeof (TRasEntryW2KW);

// get buffer sizes
    Result := RasGetEntryPropertiesW (fPBPtr, PWideChar (fEntryNameW), nil,
                                                EntrySize, nil, tempsize) ;  // 6.0 use vars
...
// get entry details from RAS
    ...
    begin
        ...
        Result := RasGetEntryPropertiesW (fPBPtr, PWideChar (fEntryNameW),
                                        @EntryInfo, EntrySize, Nil, tempsize) ;  // 6.0 use vars
    end ;
    fLastError := Result ;
    ...
    begin
        ...
        begin
            ...
            fAreaCode := trim (szAreaCode) ;
            fLocalPhoneNumber := trim (szLocalPhoneNumber) ;    // Angus, 4.3
        ...
        // warning, NT devices have two nulls strings, device and port
            fDeviceName := GetDevNamePortW (szDeviceName,
                                            Length (szDeviceName), wDevicePort);
            fDevicePort := wDevicePort ;
            fDeviceType := lowercase (szDeviceType) ;
    ...
    // may be extra phone numbers after structure, null separated list
            ...
            begin
                strptr := @EntryInfo [dwAlternateOffset] ;  // WideChar
                offset := dwAlternateOffset ;
                ch := strptr^ ;
                while (ch <> #00) do
                begin
                    stringlen := StrLenWide (strptr) ;
                    fAltPhoneNrList.Add (Trim (FixedToPasStrW (strptr, stringlen))) ;
...
// WARNING - RASSubEntryNT4 is in same absolute memory as SubEntryEdtInfo
    ...
    RasSubEntryNT4.dwSize := Sizeof (TRasSubEntryNT4W);

// get buffer sizes
    Result := RasGetSubEntryPropertiesW (fPBPtr, PWideChar (fEntryNameW),
    ...
    begin
       ...
       // Device name, port and type
        // warning, NT4 (not W2K) devices have two nulls strings, device and port
            fSubDeviceType [SubEntry] := lowercase (szDeviceType) ;
            fSubDeviceName [SubEntry] := GetDevNamePortW (szDeviceName,
                                    Length (szDeviceName), wDevicePort);
            fSubDevicePort [SubEntry] := wDevicePort ;
            ...
            begin
                ...
                begin
                    ...
                    fSubAltPhoneNrList [SubEntry].Add
                                            (FixedToPasStrW (strptr, stringlen)) ;
                    strptr := strptr + stringlen + 1 ;
                    offset := offset + stringlen + 1 ;
...
// update phonebook currently ignoring the device specific information
    Result := RasSetEntryPropertiesW (fPBPtr, PWideChar (fEntryNameW),
                                                    @EntryInfo, EntrySize, Nil, 0) ;
    fLastError := Result ;
...
procedure TMagRasCon.UnpackCanonical (PhoneCanonical: string;
    var UseCountryAndAreaCodes: boolean; var CountryCode: integer;
                                var AreaCode, LocalPhoneNumber: string) ;
begin
...
begin
    ...
    begin
        ...
        StringToWideChar (fAreaCode, szAreaCode, Length (szAreaCode)) ;
        StringToWideChar (fLocalPhoneNumber, szLocalPhoneNumber,
                                             Length (szLocalPhoneNumber)) ;
        StringToWideChar (fAutoDialDLL, szAutoDialDLL, Length (szAutoDialDLL)) ;
        StringToWideChar (fAutoDialFunc, szAutodialFunc, Length (szAutodialFunc)) ;
...
function TMagRasCon.CopyPhonebook (newname: string): LongInt ;
var
    newnamew: WideString ;
begin
    if (NOT MagLoadRasApi) then
...
// check new name
    newnamew := newname ;
    Result := RasValidateEntryNameW (nil, PWideChar(newnamew));
    fLastError := Result ;
    if fLastError = 0 then
    begin
        fEntryName := newname ;
...
var
    RasCredentials: TRasCredentialsW ;
begin
    if (NOT MagLoadRasApi) then
    begin
        fLastError := ERROR_DLL_NOT_FOUND ;
  ...
  // 5.10  use NT4 and later API
    ...
    fLastError := RasGetCredentialsW (fPBPtr, PWideChar (fEntryNameW),
                                                         RasCredentials) ;
    result := fLastError ;
    if fLastError = 0 then
    begin
...
begin
    ...
    Begin
        dwSize := Sizeof (RasDialParamsNT4);
        StringToWideChar (fEntryName, szEntryName, RAS_MaxEntryName);
        StringToWideChar (fPhoneNumber, szPhoneNumber, RAS_MaxPhoneNumber);  // Angus 4.30
        StringToWideChar (fCallBackNumber, szCallbackNumber, RAS_MaxCallbackNumber);
        StringToWideChar (fUserName, szUserName, UNLEN);
        StringToWideChar (fPassWord, szPassword, PWLEN);
        StringToWideChar (fDomain, szDomain, DNLEN);
...
begin
  ...
  // Angus 5.10, use NT4 and later API that might work properly on Visa
    ...
    if (fPBLocation = REN_AllUsers) and fDefaultCreds then           // Angus 5.30
        RasCredentials.dwMask := RasCredentials.dwMask OR RASCM_DefaultCreds ;
    StringToWideChar (fUserName, RasCredentials.szUserName, UNLEN);
    StringToWideChar (fPassWord, RasCredentials.szPassword, PWLEN);
    StringToWideChar (fDomain, RasCredentials.szDomain, DNLEN);
    fLastError := RasSetCredentialsW (fPBPtr, PWideChar (fEntryNameW),
...
begin
    ...
    if (MagRasDlgLib = 0) then
        Result := RasEditPhonebookEntryW (winHandle, fPBPtr, PWideChar(fEntryNameW))    // 4.60
    else
    begin
 // Angus 5.20 - NT4 and later
        ...
        if NOT RasEntryDlgW (fPBPtr, PWideChar(fEntryNameW), EntryDlg) then
...
begin
    ...
    Result := RasDeleteEntryW (fPBPtr, PWideChar(fEntryNameW));   // 5.20 added phonebook
    fLastError := Result ;
    if Result = 0 then
        fEntryName := ''
    else
...
function TMagRasCon.RenamePhonebook (newname: string): LongInt ;
var
    newnamew: WideString ;
begin
    if (NOT MagLoadRasApi) then
    ...
    newnamew := newname ;
    Result := RasValidateEntryNameW (fPBPtr, PWideChar(newnamew));    // 5.20 added phonebook
    fLastError := Result ;
    if fLastError = 0 then
    begin
        Result := RasRenameEntryW (fPBPtr, PWideChar(fEntryNameW),
                                        PWideChar(newnamew));    // 5.20 added phonebook
        if Result = 0 then EntryName := newname ; // 6.0
        fLastError := Result ;
    end  ;
    if fLastError <> 0 then
            fStatusStr := GetErrorString (fLastError);
...
function TMagRasCon.ValidateName (newname: string): LongInt ;
var
    newnamew: WideString ;
begin
    if (NOT MagLoadRasApi) then
...
begin
    ...
    if (MagRasDlgLib = 0) then
        Result := RasCreatePhonebookEntryW (winHandle, Nil)
    else
    begin
 // Angus 5.20 - NT4 and later
        result := 0 ;
        ...
        if NOT RasEntryDlgW (fPBPtr, Nil, EntryDlg) then
                                        result := EntryDlg.dwError ;
        fLastError := Result ;
    end ;
    if fLastError <> 0 then
...
//    if fSubEntry = 0 then fSubEntry := 1 ; // Angus 4.51, allow multilink on W2K
    MoveDialParms ;
    fLastError := RasDialW (Nil, fPBPtr, @RasDialParamsNT4,
...
// set-up dial extensions
    ...
    fLastError := RasDialW (@RasDialExtensions, fPBPtr,
...
begin
    ...
    FillChar (szErrorString, Length (szErrorString), #0);
...
//    fPPPMultilink := false ;
    ...
    Result := RasGetProjectionInfoW (ConnHandle, RASP_PppIp, @RasPppIp, varsize) ;
    fLastError := Result;
    if Result = 0 then
    begin
        fConnProtocol := 'PPP ';
...
// now get LCP stuff
    ...
    RasPppLcpW2K.dwSize := SizeOf (RasPppLcpW2K);
    varsize := RasPppLcpW2K.dwSize;
    retval:= RasGetProjectionInfoW (ConnHandle, RASP_PppLcp, @RasPppLcpW2K, varsize) ;
    if retval = 0 then
    begin
        result := 0 ;
...
begin
    ...
    result := RASGetSubEntryHandleW (ConnHandle, SubEnt, SubRasConn) ;
    fLastError := result ;
end ;
...
// need to cope with three arrays of different lengths, with trailing nulls
    ...
    begin
        ...
        FillChar (newconn, SizeOf (TRasConnXPW), 0);
        if MagRasOSVersion >= OSWXP then    // 14 Sept 2012 avoid specific OSs
            Move (RasConnectXP [index], newconn, Sizeof (TRasConnXPW))
         else
            Move (RasConnectW2K [index], newconn, Sizeof (TRasConnW2KW)) ;
    end ;
    ...
    // see if connections have changed, avoids rebuilding list and changing display
        ...
        begin
            ...
            begin
                ...
                if NOT CompareMem (@oldconn, @newconn, Sizeof (TRasConnXPW))
...
function TMagRasCon.GetConnection: String;
var
    BufSize: DWord ;
begin
    fCurConnName := '' ;
    ...
    if MagRasOSVersion >= OSWXP then    // 14 Sept 2012 avoid specific OSs
        RasConnectAll [1].dwSize := Sizeof (TRasConnXPW)
     else
        RasConnectAll [1].dwSize := Sizeof (TRasConnW2KW);
    BufSize := SizeOf (RASConnectAll);
    fLastError := RasEnumConnectionsW (@RasConnectAll, BufSize, fNumConns);
    ...
    begin
        ...
        fCurDevType := Lowercase (RASConnectAll [1].szDeviceType) ;
...
var
    RasDevNames, RasDevNames2: PRasDevInfoW ;  // 4.91 dynamic instead of fixed memory
...
begin
    ...
    BufSize := 0 ;
    Entries := 0 ;
    result := RasEnumDevicesW (Nil, BufSize, Entries) ;  // 4.91 find memory needed
    ...
    try
        ...
        RasDevNames.dwSize := SizeOf (TRasDevInfoW) ;
        Entries := 0 ;
        result := RasEnumDevicesW (RasDevNames, BufSize, Entries) ;
        fLastError := result;
        ...
        begin
            ...
            begin
                fDeviceNameList.Add (RasDevNames2.szDeviceName) ;  // !! no port name
                fDeviceTypeList.Add (Lowercase (RasDevNames2.szDeviceType)) ;
            end ;
...
function TMagRasCon.GetOneCountryInfo (CountryId: integer;
            var CountryCode: integer; var CountryName: string)  : integer;
var
    RasExtCtryInfo: TRasExtCtryInfoW ;
    BuffSize: DWORD ;
    strptr: PWideChar ;
    stringlen: integer ;
begin
    ...
    CountryCode := 0 ;
    CountryName := '' ;
    Buffsize := SizeOf(RasExtCtryInfo) ;
    ...
    Result := RasGetCountryInfoW (@RasExtCtryInfo, Buffsize);
    fLastError := Result;
    If (Result <> 0) THEN exit ;
    ...
    strptr := @RasExtCtryInfo.buffer
                [RasExtCtryInfo.RasCtryInfo.dwCountryNameOffset -
                                                     SizeOf(TRasCtryInfo)] ;
    stringlen := StrLenWide (strptr) ;
    CountryName := Trim (FixedToPasStrW (strptr, stringlen)) ;
end ;
...
begin
    ...
    for I := 0 to Pred (MagRasNumEntryRec) do
                fPhoneBookEntries.Add (MagRasEntryRecs [I].EntryName) ;
end ;
...
// implement queue for event changes, since application may not process them fast enough, 4.50
    ...
    begin
        ...
        begin
            ...
            CurDevName := fCurDevName ;
            CurDevType := fCurDevType ;
            ConnectPhoneNr := fConnectPhoneNr ;
...
var
    RasConnStatusNT4: TRasConnStatusNT4W ;
begin
    ...
    RasConnStatusNT4.dwSize := Sizeof (RasConnStatusNT4);
    fLastError := RasGetConnectStatusW (ConnHandle, @RasConnStatusNT4);
    if fLastError = 0 then
    begin
...
PROCEDURE TMagRasCon.SetPhoneBookPath (Value: String);
BEGIN
    exit ; // 6.0 no custom paths
END;
...
begin
    ...
    begin
        ...
        fPhoneBookPathW := fPhoneBookPath ;
        fPBPtr := PWideChar (fPhoneBookPathW) ;
    end ;
end;
...
begin
    ...
    begin
        ...
        fLastError := RasGetConnectStatusW (fRasConn, @RASConnStatus);
...
// display the Translate dialog box, aka 'Phone and Modem Options', 'Dialling Rules' for My Location
// should refresh GetTransCaps afterwards
...
begin
    ...
    addrw := addr ;
    Result := (lineTranslateDialogW (fTAPILineApp, devid,
                            TAPI_CURRENT_VERSION, parh, PWideChar(Addrw)) = 0);
end;
...
// 6.30 ensure at least one dialling location exists,
...
// 6.01 ensure at least one location exists, to avoid 'Location Information' dialog being displayed
    SetDefDialLocation ;  // 6.30 moved code to separate function
...
// dialling properties first, with current location and calling card
    ...
    result := lineGetTranslateCapsW (fTAPILineApp, TAPI_CURRENT_VERSION,
                                                      @LineTranslateCaps) ;
    if result = 0 then
    begin
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        LocationName := Trim (FixedToPasStrW (@Data
                                            [dwLocationNameOffset],
                                                        dwLocationNameSize div 2));
                        CountryCode := dwCountryCode ;
                        CityCode := Trim (FixedToPasStrW (@Data [dwCityCodeOffset],
                                                            dwCityCodeSize div 2));
                        PreferredCardID := dwPreferredCardID ;
                        LocalAccessCode := Trim (FixedToPasStrW
                                            (@Data [dwLocalAccessCodeOffset],
                                                        dwLocalAccessCodeSize div 2));
                        LongDistanceCode := Trim (FixedToPasStrW (@Data
                                            [dwLongDistanceAccessCodeOffset],
                                                dwLongDistanceAccessCodeSize div 2));
                        TollPrefixList := Trim (FixedToPasStrW
                                        (@Data [dwTollPrefixListOffset],
                                                        dwTollPrefixListSize div 2));
                        CountryID := dwCountryID ;
                        Options := dwOptions ;
                        CancelCallWaiting := Trim (FixedToPasStrW (@Data
                                    [dwCancelCallWaitingOffset],
                                                    dwCancelCallWaitingSize div 2));
            ...
            begin
                ...
                for I := 0 to Pred (dwNumCards) do
                    ...
                    begin
                        ...
                        CardName := Trim (FixedToPasStrW (@Data [dwCardNameOffset],
                                                            dwCardNameSize div 2));
                        CardNumberDigits := dwCardNumberDigits ;
                        SameAreaRule := Trim (FixedToPasStrW (@Data [dwSameAreaRuleOffset],
                                                            dwSameAreaRuleSize div 2));
                        LongDistanceRule := Trim (FixedToPasStrW (@Data
                                            [dwLongDistanceRuleOffset],
                                                    dwLongDistanceRuleSize div 2));
                        InternationalRule := Trim (FixedToPasStrW (@Data
                                            [dwInternationalRuleOffset],
                                                    dwInternationalRuleSize div 2));
                        Options := dwOptions ;
...
// now country
    ...
    begin
        ...
        result := lineGetCountryW (DialLocation.CountryId,
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        CountryCode := dwCountryCode ;
                        CountryName := Trim (FixedToPasStrW (@Data [dwCountryNameOffset],
                                                            dwCountryNameSize div 2));
                        SameAreaRule := Trim (FixedToPasStrW (@Data [dwSameAreaRuleOffset],
                                                            dwSameAreaRuleSize div 2));
                        LongDistanceRule := Trim (FixedToPasStrW (@Data
                                            [dwLongDistanceRuleOffset],
...
function TMagRasCon.TransAddr (devid: integer; inputnum: string;
              card, flags: integer ; var disnum, dialnum: string): Boolean ;
var
...
begin
 ...
 // 6.40 check for period in IP address or host name and give up
    if Pos ('.', inputnum) > 0 then
    begin
        disnum := inputnum ;
        dialnum := inputnum ;
        exit ;
    ...
    try
        inputnumw := inputnum ;
        if lineTranslateAddressW (fTAPILineApp, devid, TAPI_CURRENT_VERSION,
                           PWideChar(inputnumw), card, flags, TransOutput) = 0 then
        begin
        with TransOutput do
        begin
...
function TMagRasCon.TranslateAddr (devid: integer; inputnum: string;
                                var disnum, dialnum: string): Boolean ;
begin
    result := TransAddr (devid, inputnum, 0, 0, disnum, dialnum) ;
end ;

function TMagRasCon.GetSubLocalPhoneNumber (Index: Integer): string ;
begin
    result := fSubLocalPhoneNumber [Index] ;
end ;

function TMagRasCon.GetSubPhoneCanonical (Index: Integer): string ;
begin
    result := fSubPhoneCanonical [Index] ;
end ;

function TMagRasCon.GetSubDeviceType (Index: Integer): string ;
begin
    result := fSubDeviceType [Index] ;
end ;

function TMagRasCon.GetSubDeviceName (Index: Integer): string ;
begin
    result := fSubDeviceName [Index] ;
end ;

function TMagRasCon.GetSubDevicePort (Index: Integer): string ;
begin
...
function TMagRasCon.FixWin10Ras(var DialupBroken, RebootNeeded: Boolean): string;
const
    RegRasMan = 'SYSTEM\CurrentControlSet\Services\RasMan' ;
    RegRequiredPrivileges = 'RequiredPrivileges' ;
    RegSeLoadDriverPrivilege = 'SeLoadDriverPrivilege' ;
var
    total, I: Integer;
    List: TStringList;
begin
    ...
    if OsInfo.dwMajorVersion < 10 then Exit ;
    if OsInfo.dwBuildNumber < 15063 then Exit;

  // check correct privileges are in registry
    List := TStringList.Create;
    try
        total := MagRegReadStrings (HKEY_LOCAL_MACHINE, RegRasMan,
                                                RegRequiredPrivileges, List) ;
        if total = 0 then
        begin
            result := 'Failed to read RasMan registry privileges' ;
            exit ;
        end ;
        for I := 0 to total - 1 do
        begin
          // this privilege needed for Windows 10 1703
            if List [I] = RegSeLoadDriverPrivilege then
            begin
             // found it OK, nothing more to do
             // result := 'Found correct RasMan registry privileges' ;  // temp
                exit;
            end;
        end;
        DialupBroken := True;
        ...
        begin
            result := 'Program requires administrator rights to update RasMan registry privileges' ;
            exit ;
        end ;
        List.Add (RegSeLoadDriverPrivilege) ; // add new privilege and write all back to registry
        if MagRegWriteStrings (HKEY_LOCAL_MACHINE, RegRasMan,
                                                RegRequiredPrivileges, List) then
        begin
            RebootNeeded := True ;
            Result := 'Updated RasMan registry privileges, PC reboot needed to be effective' ;
        end
        else
            Result := 'Failed to update RasMan registry privileges' ;
    finally
        List.Free ;
    end ;
