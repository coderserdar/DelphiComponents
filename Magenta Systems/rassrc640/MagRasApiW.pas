Unit magrasapiw;  // Unicode/Widestring version
{$IFNDEF VER140}
...
// 10 Aug 2011, don't want anything packed
...
6th June 2017 - Release 6.40, Copyright 2017, Magenta Systems Ltd' ;
...
Removed support for Windows 9x (and NT4) since they are likely missing W APIs

Changes in 6.10
Added more IpV6 stuff for RasEntry that seems to have been added to a later Vista
release, but which are not used by TMagRas
...
Windows 7 - note that the RasEntry structure has been lengthened with more IPv6
elements, but these are not yet added to TMagRas mainly because we don't support IPv6 yet
...
Type
// 10 Aug 2011 not available in all versions of Delphi
  PLUID = ^TLUID;
  _LUID = record
    LowPart: DWORD;
    HighPart: INTEGER; // INT32;
  end;
  TLUID = _LUID;
  LUID = _LUID;
...
// Identifies an active RAS Connection, RasConnectEnum
  PRasConnW = ^TRasConnW;             // Windows 9x only
  TRasConnW = record
    dwSize: DWORD;
    rasConn: HRasConn;
  ...
  PRasConnNT4W = ^TRasConnNT4W;       // Windows NT4
  TRasConnNT4W = record
    dwSize: DWORD;
    rasConn: HRasConn;
    szEntryName: Array[0..RAS_MaxEntryName] Of WideChar;
  ...
  PRasConnW2KW = ^TRasConnW2KW;        // Windows 2000
  TRasConnW2KW = record
    dwSize: DWORD;
    rasConn: HRasConn;
    szEntryName: Array[0..RAS_MaxEntryName] Of WideChar;
  ...
  PRasConnXPW = ^TRasConnXPW;        // Windows XP
  TRasConnXPW = record
    dwSize: DWORD;
    rasConn: HRasConn;
    szEntryName: Array[0..RAS_MaxEntryName] Of WideChar;
  ...
  PRasConnW7W = ^TRasConnW7W;        // Windows 7
  TRasConnW7W = record
    dwSize: DWORD;
    rasConn: HRasConn;
    szEntryName: Array[0..RAS_MaxEntryName] Of WideChar;
    ...
    guidEntry: TGUID ;      // guid that represents the phone-book entry
    dwFlags: DWORD ;        // RASCF literals
    luid: TLUID ; // 20 July 2011 was TLargeInteger   // (64-bits) locally unique identifier (LUID) for the logon session in which the connection was established
...
// status of a specific RAS connection, RasGetConnectStatus
  PRasConnStatusW = ^TRasConnStatusW;      // Windows 9x
  ...
  PRasConnStatusNT4W = ^TRasConnStatusNT4W;      // Windows NT4 and 2000
  TRasConnStatusNT4W = Record
    dwSize: DWORD;
    rasConnstate: Word;
    dwError: DWORD;
    szDeviceType: Array[0..RAS_MaxDeviceType] Of WideChar;
...
// dial params, used for both RasDial and RasGetEntryDialParams
  PRasDialParamsW = ^TRasDialParamsW;       // Windows 9x
  ...
  PRasDialParamsNT4W = ^TRasDialParamsNT4W;       // Windows NT4 and 2000
  TRasDialParamsNT4W = Record
    dwSize: DWORD;
    szEntryName: Array[0..RAS_MaxEntryName] Of WideChar;
    szPhoneNumber: Array[0..RAS_MaxPhoneNumber] Of WideChar;
    szCallbackNumber: Array[0..RAS_MaxCallbackNumber] Of WideChar;
...
// These calls supercede Ras{Get,Set}EntryDialParams in NT4
  PRasCredentialsW = ^TRasCredentialsW ;      // Windows NT4 and 2000
...
type
  PRasEntryNameW = ^TRasEntryNameW;   // Windows 9x and NT
  TRasEntryNameW = Record
    dwSize: DWORD;
    szEntryName: Array[0..RAS_MaxEntryName] Of WideChar;
  End;

  PRasEntryNameW2KW = ^TRasEntryNameW2KW;   // Windows W2K and later
  TRasEntryNameW2KW = Record
    dwSize: DWORD;
    szEntryName: Array[0..RAS_MaxEntryName] Of WideChar;
...
// Note this structure is undocumented, info from ras.h file
  ...
  PRasPppLcpW2KW = ^TRasPppLcpW2KW; // Windows 2000, probably
  TRasPppLcpW2KW = record
    dwSize: DWORD;
    fBundled: LongBool;
    dwError,
...
// RAS Country Information (currently retreieved from TAPI).
  ...
  TRasExtCtryInfoW = record             // version for RasGetCountryInfo
    RasCtryInfo: TRasCtryInfo ;
    buffer: array [0..255] of WideChar ;   // returns the country name
  end ;
...
// PPP/Ip
  ...
  PRasEntryNT4W = ^TRasEntryNT4W ;    // this version fails under Win9x (might be OK with DUN 1.3)
  TRasEntryNT4W = record
    dwSize,
    dwfOptions,
    //
    // Location/phone number.
...
// PPP/Ip
  ...
  PRasEntryW2KW = ^TRasEntryW2KW ;    // this version fails under Win9x and NT4
  TRasEntryW2KW = record
    dwSize,
    dwfOptions,
    //
    // Location/phone number.
...
// PPP/Ip
    ...
    szCustomDialDll: Array [0..MAX_PATH - 1] of WideChar;  // DLL for custom dialing
    dwVpnStrategy: DWORD ;      // specifies type of VPN protocol
  end;  // followed by alternate numbers

  PRasEntryXPW = ^TRasEntryXPW ;    // also Windows 2003, this version fails under Win9x, NT4 and W2K
  TRasEntryXPW = record
    dwSize,
    dwfOptions,
    //
...
// PPP/Ip
    ...
    dzDnsSuffix: Array [0..RAS_MaxDnsSuffix - 1] of WideChar; // Domain Name Service (DNS) suffix for the connection
    ...
    szPrerequisitePbk: Array [0..MAX_PATH - 1] of WideChar;  // VPN only, see next element
    szPrerequisiteEntry: Array [0..RAS_MaxEntryName] of WideChar;  // VPN only, an entry RAS dials before establishing this connecton
    dwRedialCount: DWORD ;    // number of times RAS attempts to dial
    dwRedialPause: DWORD ;    // seconds to wait between retry attempts 
  end;  // followed by alternate numbers

  PRasEntryVistaW = ^TRasEntryVistaW ;   // also Windows 2008, this version fails under Win9x, NT4, W2K and XP
  TRasEntryVistaW = record
    dwSize,
    dwfOptions,
    //
    // Location/phone number.
...
// PPP/Ip
    ...
    dwIPv4InterfaceMetric: DWORD ;  // 6.10 added during lifetime of Vista
    dwIPv6InterfaceMetric: DWORD ;  // 6.10 added during lifetime of Vista
  end;  // followed by alternate numbers
...
type
  ...
  LpRasSubEntryNT4W = ^TRasSubEntryNT4W;
  TRasSubEntryNT4W = record
    dwSize,
    dwfFlags: DWORD;
    //
    // Device
...
// TRasEntry dwfOptions2 bit flags   Windows Vista and 2008
    ...
    RASEO2_IPv4ExplicitMetric          = $00010000;    // 6.10 added during lifetime of Vista
    RASEO2_IPv6ExplicitMetric          = $00020000;    // 6.10 added during lifetime of Vista
    RASEO2_DisableIKENameEkuCheck      = $00040000;    // 6.10 added during lifetime of Vista
...
type
  TRasPbdDlgFuncW = procedure (dwCallbackId: Pointer; dwEvent: DWORD;  // 22 July 2011 callback was DWORD
...
//  PRasNoUser = PRasNoUserW;
  tagRASNOUSERW = record
    dwSize: DWORD;
    dwFlags: DWORD;
    dwTimeoutMs: DWORD;
    szUserName: array[0..UNLEN] of WideChar;   // 18 Aug 2011
    szPassword: array[0..PWLEN] of WideChar;   // 18 Aug 2011
    szDomain: array[0..DNLEN] of WideChar;     // 18 Aug 2011
  end;
  TRasNoUserW = tagRASNOUSERW;
//  TRasNoUser = TRasNoUserW;
...
//  PRasEntryDlg = PRasEntryDlgW;
  tagRASENTRYDLGW = record
    ...
    xDlg: LongInt;
    yDlg: LongInt;
    szEntry: array[0..RAS_MaxEntryName] of WideChar;  // 18 Aug 2011
    dwError: DWORD;
    reserved: Pointer ;  // 22 July 2011 was DWORD
    reserved2: Pointer ;  // 22 July 2011 was DWORD
  end;
...
RasDialW: Function (
    ...
    lpszPhonebook: PWideChar;   // pointer to full path and filename of phonebook file
    lpRasDialParams : PRasDialParamsNT4W;    // pointer to calling parameters data
...
RasEnumConnectionsW: Function (
    RASConn: PRasConnW;      { buffer to receive Connections data }
    var BufSize: DWord;     { size in bytes of buffer }
    var Connections: DWord  { number of Connections written to buffer }
    ): DWORD; stdcall;

RasEnumEntriesW: Function (
    reserved: PWideChar;    // reserved, must be NULL
    lpszPhonebook: PWideChar  ; // pointer to full path and filename of phonebook file
    lprasentryname: PRasEntryNameW2KW ;  // buffer to receive phonebook entries
...
RasGetConnectStatusW: Function (
    ...
    RASConnStatus: PRASConnStatusNT4W    { buffer to receive status data }
    ): DWORD; stdcall;

RasGetErrorStringW: Function (
    ErrorCode: DWord;   { error code to get string for }
    szErrorString: PWideChar;   { buffer to hold error string }
...
RasGetEntryDialParamsW: Function (         // ANGUS
    lpszPhonebook:PWideChar;    // pointer to the full path and filename of the phonebook file
    var lprasdialparams: TRasDialParamsNT4W; // pointer to a structure that receives the connection parameters
    var lpfPassword : BOOL  // indicates whether the user's password was retrieved
    ): DWORD; stdcall;

RasSetEntryDialParamsW: Function (         // ANGUS
    lpszPhonebook:PWideChar;    // pointer to the full path and filename of the phonebook file
    var lprasdialparams: TRasDialParamsNT4W; // pointer to a structure that contains the connection parameters
    fRemovePassword: BOOL   // // indicates whether to remove password from entry's parameters
    ): DWORD; stdcall;

RasMonitorDlgW: Function (
    lpszDeviceName:PWideChar;   // pointer to the name of the device to display initially
    var lprasmonitordlg:TRasRasMonitorDlg   // pointer to a structure with I/O parms
    ): DWORD; stdcall;

RasEditPhonebookEntryW: Function (    // ANGUS
    hWndParent : HWND;   // handle to the parent window of the dialog box
    lpszPhonebook : PWideChar; // pointer to the full path and filename of the phonebook file
    lpszEntryName : PWideChar  // pointer to the phonebook entry name
    ) : DWORD; stdcall;

RasCreatePhonebookEntryW: Function ( // ANGUS
    hWndParent : HWND;  // handle to the parent window of the dialog box
    lpszPhonebook : PWideChar // pointer to the full path and filename of the phonebook file
    ) : DWORD; stdcall;

RasGetProjectionInfoW: Function (    // ANGUS
    hConn: HRasConn;
...
RasGetCountryInfoW: Function (   // ANGUS  -  from RNAPH.DLL
    lpCtryInfo: PWideChar;
    var lpdwSize: DWORD
    ): DWORD; stdcall;

RasGetEntryPropertiesW: Function (   // ANGUS     -  from RNAPH.DLL
    lpszPhonebook,
    szEntry: PWideChar;
    lpbEntry: PWideChar;              // Angus 4.11
    var lpdwEntrySize: DWORD;         // Angus 5.40 made var
    lpbDeviceInfo: PWideChar;         // Angus 4.11
    var lpdwDeviceInfoSize: DWORD     // Angus 5.40 made var
    ): DWORD; stdcall;

RasSetEntryPropertiesW: Function (   // ANGUS    -  from RNAPH.DLL
    lpszPhonebook,
    szEntry: PWideChar;
    lpbEntry: PWideChar;               // Angus 4.11
    dwEntrySize: DWORD;
...
RasRenameEntryW: Function (  // ANGUS         -  from RNAPH.DLL
    lpszPhonebook,
    szEntryOld,
    szEntryNew: PWideChar
    ): DWORD; stdcall;

RasDeleteEntryW: Function (  // ANGUS          -  from RNAPH.DLL
    lpszPhonebook,
    szEntry: PWideChar
    ): DWORD; stdcall;

RasValidateEntryNameW: Function (    // ANGUS     -  from RNAPH.DLL
    lpszPhonebook,
    szEntry: PWideChar
    ): DWORD; stdcall;

RasEnumDevicesW: Function (  // ANGUS      -  from RNAPH.DLL
    lpBuff: PRasDevInfoW;
    var lpcbSize: DWORD;
...
RasGetSubEntryHandleW: Function (    // ANGUS     - NT4 only
    rasconn: HRasConn;  // 20 July 2011, was hrasconn which was duplicate
    dwSubEntry: DWORD;
    var lphrasconn: HRasConn
    ): DWORD; stdcall;

RasGetCredentialsW: Function (   // ANGUS     - NT4 only
    lpszPhoneBook,
    lpszEntry: PWideChar;
    var lpCredentials: TRasCredentialsW
    ): DWORD; stdcall;

RasSetCredentialsW: Function (   // ANGUS     - NT4 only
    lpszPhoneBook,
    lpszEntry: PWideChar;
    var lpCredentials: TRasCredentialsW;
...
RasConnectionNotificationW: Function (   // ANGUS     - NT4 only
  rasconn: HRasConn;  // 20 July 2011, was hrasconn which was duplicate
  hEvent: THandle;
  dwFlags: DWORD
  ): DWORD; stdcall;

RasGetSubEntryPropertiesW: Function (    // ANGUS     - NT4 only
    lpszPhoneBook,
    lpszEntry: PWideChar;
    dwSubEntry: DWORD;
    lpRasSubEntry: PWideChar;            // Angus 4.11
    var lpdwcb: DWORD;                   // Angus 5.40 made var
    lpbDeviceInfo: PWideChar;            // Angus 4.11  unused
    var lpdwDeviceInfoSize: DWORD        // Angus 5.40 made var
    ): DWORD; stdcall;

RasSetSubEntryPropertiesW: Function (    // ANGUS     - NT4 only
    lpszPhoneBook,
    lpszEntry: PWideChar;
    dwSubEntry: DWORD;
    lpRasSubEntry: PWideChar;             // Angus 4.11
...
RasGetAutodialAddressW: Function (   // ANGUS     - NT4 only
...
RasSetAutodialAddressW: Function (   // ANGUS     - NT4 only
    lpszAddress: PWideChar;
    dwReserved: DWORD;
    lpAutoDialEntries: LPRasAutoDialEntryW;
...
RasEnumAutodialAddressesW: Function (    // ANGUS     - NT4 only
    lppAddresses: Pointer;
    var lpdwcbAddresses: DWORD;
    var lpdwAddresses: DWORD
    ): DWORD; stdcall;

RasGetAutodialEnableW: Function (    // ANGUS     - NT4 only
    dwDialingLocation: DWORD;
    var lpfEnabled: LongBool
    ): DWORD; stdcall;

RasSetAutodialEnableW: Function (    // ANGUS     - NT4 only
    dwDialingLocation: DWORD;
    fEnabled: LongBool
    ): DWORD; stdcall;

RasGetAutodialParamW: Function (     // ANGUS     - NT4 only
    dwKey: DWORD;
    lpvValue: Pointer;
    var lpdwcbValue: DWORD
    ): DWORD; stdcall;

RasSetAutodialParamW: Function (     // ANGUS     - NT4 only
    dwKey: DWORD;
    lpvValue: Pointer;
    dwcbValue: DWORD
    ): DWORD; stdcall;

RasPhonebookDlgW: function (        // Angus 5.20 - NT4 and later
    lpszPhonebook: PWideChar;
    lpszEntry: PWideChar;
    var lpInfo: TRasPbdDlgW): Bool stdcall;

RasEntryDlgW: function (             // Angus 5.20 - NT4 and later
    lpszPhonebook: PWideChar;
    lpszEntry: PWideChar;
    var lpInfo: TRasEntryDlgW): Bool stdcall;

RasDialDlgW: function (               // Angus 5.20 - NT4 and later
    lpszPhonebook: PWideChar;
    lpszEntry: PWideChar;
    lpszPhoneNumber: PWideChar;
...
RasDeleteSubEntryW: function (               // Angus  - W2K SP2 and XP only
    lpszPhoneBook,
    lpszEntry: PWideChar;
    dwSubEntryId: DWORD
    ): DWORD; stdcall;
...
// open libraries - only come here once
    ...
    if (MagRasOSVersion <= OSNT4) then exit ;  // 6.0 no longer supporting Win9x or NT4
...
// set function addresses to ASCII versions in DLL
    RasDialW := GetProcAddress(MagRasLib, 'RasDialW') ;
    RasEnumConnectionsW := GetProcAddress(MagRasLib, 'RasEnumConnectionsW');
    RasEnumEntriesW := GetProcAddress(MagRasLib, 'RasEnumEntriesW');
    RasGetConnectStatusW := GetProcAddress(MagRasLib, 'RasGetConnectStatusW');
    RasGetErrorStringW := GetProcAddress(MagRasLib, 'RasGetErrorStringW');
    RasHangUpW := GetProcAddress(MagRasLib, 'RasHangUpW');
    RasGetEntryDialParamsW := GetProcAddress(MagRasLib, 'RasGetEntryDialParamsW');
    RasSetEntryDialParamsW := GetProcAddress(MagRasLib, 'RasSetEntryDialParamsW');
    RasMonitorDlgW := GetProcAddress(MagRasLib, 'RasMonitorDlgW');
    RasEditPhonebookEntryW := GetProcAddress(MagRasLib, 'RasEditPhonebookEntryW');
    RasCreatePhonebookEntryW := GetProcAddress(MagRasLib, 'RasCreatePhonebookEntryW');
    RasGetProjectionInfoW := GetProcAddress(MagRasLib, 'RasGetProjectionInfoW');
    RasConnectionNotificationW := GetProcAddress(MagRasLib, 'RasConnectionNotificationW');

// Windows NT4 stuff, and later
    RasGetSubEntryHandleW := GetProcAddress(MagRasLib, 'RasGetSubEntryHandleW');
    RasGetSubEntryPropertiesW := GetProcAddress(MagRasLib, 'RasGetSubEntryPropertiesW');
    RasSetSubEntryPropertiesW := GetProcAddress(MagRasLib, 'RasSetSubEntryPropertiesW');
    RasGetAutodialAddressW := GetProcAddress(MagRasLib, 'RasGetAutodialAddressW');
    RasSetAutodialAddressW := GetProcAddress(MagRasLib, 'RasSetAutodialAddressW');
    RasEnumAutodialAddressesW := GetProcAddress(MagRasLib, 'RasEnumAutodialAddressesW');
    RasGetAutodialEnableW := GetProcAddress(MagRasLib, 'RasGetAutodialEnableW');
    RasSetAutodialEnableW := GetProcAddress(MagRasLib, 'RasSetAutodialEnableW');
    RasGetAutodialParamW := GetProcAddress(MagRasLib, 'RasGetAutodialParamW');
    RasSetAutodialParamW := GetProcAddress(MagRasLib, 'RasSetAutodialParamW');
    RasGetCredentialsW := GetProcAddress(MagRasLib, 'RasGetCredentialsW');   // 5.10
    RasSetCredentialsW := GetProcAddress(MagRasLib, 'RasSetCredentialsW');   // 5.10
...
// Windows XP (NT5.1) stuff, and later
    ...
    begin
        RasDeleteSubEntryW := GetProcAddress(MagRasLib, 'RasDeleteSubEntryW');
    end ;

// now get API extensions that may be in rasapi32.dll or rnaph.dll
    RasGetCountryInfoW := GetProcAddress(MagRasLib, 'RasGetCountryInfoW');
    RasGetEntryPropertiesW := GetProcAddress(MagRasLib, 'RasGetEntryPropertiesW');
    RasSetEntryPropertiesW := GetProcAddress(MagRasLib, 'RasSetEntryPropertiesW');
    RasRenameEntryW := GetProcAddress(MagRasLib, 'RasRenameEntryW');
    RasDeleteEntryW := GetProcAddress(MagRasLib, 'RasDeleteEntryW');
    RasValidateEntryNameW := GetProcAddress(MagRasLib, 'RasValidateEntryNameW');
    RasEnumDevicesw := GetProcAddress(MagRasLib, 'RasEnumDevicesW');
    result := Assigned (RasDialW) ;
...
// rasdlg.dll dialogs
    ...
    begin
        RasEntryDlgW := GetProcAddress(MagRasDlgLib, 'RasEntryDlgW');               // 5.20
        RasPhonebookDlgW := GetProcAddress(MagRasDlgLib, 'RasPhonebookDlgW');       // 5.20
        RasDialDlgW := GetProcAddress(MagRasDlgLib, 'RasDialDlgW');                 // 5.20
    end ;
