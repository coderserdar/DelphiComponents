...
12th August 2013 - Release 5.72, Copyright 2013, Magenta Systems Ltd
...
// Identifies an active RAS Connection, RasConnectEnum
  ...
  PRasConnWXP = ^TRasConnWXP;        // Windows XP
  TRasConnWXP = record
    dwSize: DWORD;
    rasConn: HRasConn;
    szEntryName: Array[0..RAS_MaxEntryName] Of AnsiChar;
  ...
  PRasConnW7 = ^TRasConnW7;        // Windows 7
  TRasConnW7 = record
    dwSize: DWORD;
    rasConn: HRasConn;
    szEntryName: Array[0..RAS_MaxEntryName] Of AnsiChar;
...
// dial params, used for both RasDial and RasGetEntryDialParams
  ...
  PRasDialParamsNT4 = ^TRasDialParamsNT4;       // Windows NT4 and 2000 and later
...
// for TRasConnWXP dwFlag values - Windows XP
...
type
  ...
  PRasEntryNameW2K = ^TRasEntryNameW2K;   // Windows W2K and later 
...
// PPP/Ip
  ...
  PRasEntryNT4 = ^TRasEntryNT4 ;    // this version fails under Win9x (might be OK with DUN 1.3)
  TRasEntryNT4 = record
    dwSize,
    dwfOptions,
    //
...
// PPP/Ip
  ...
  PRasEntryWXP = ^TRasEntryWXP ;    // this version fails under Win9x, NT4 and W2K
  TRasEntryWXP = record
    dwSize,
    dwfOptions,
    //
...
// PPP/Ip
  ...
  PRasEntryWVista = ^TRasEntryWVista ;   // also Windows 2008/7, this version fails under Win9x, NT4, W2K and XP
  TRasEntryWVista = record
    dwSize,
    dwfOptions,
    //
    // Location/phone number.
...
RasEnumConnections: Function (
    lpRASConn: PRasConnWXP; { buffer to receive Connections data }
...
RasPhonebookDlg: function (        // Angus 5.20 - NT4 and later
    lpszPhonebook: PAnsiChar;
    lpszEntry: PAnsiChar;
    var lpInfo: TRasPbdDlg): Bool stdcall;

RasEntryDlg: function (             // Angus 5.20 - NT4 and later
    lpszPhonebook: PAnsiChar;
    lpszEntry: PAnsiChar;
    var lpInfo: TRasEntryDlg): Bool stdcall;

RasDialDlg: function (               // Angus 5.20 - NT4 and later
    lpszPhonebook: PAnsiChar;
    lpszEntry: PAnsiChar;
...
RasDeleteSubEntry: function (               // Angus  - W2K SP2 and XP only
    lpszPhoneBook,
    lpszEntry: PAnsiChar;
    dwSubEntryId: DWORD
    ): DWORD; stdcall;
...
{ List of the rasphone.pbk files in various directories, NT4/W2K only }
  MagRasPhoneFiles: array [0..MaxPhoneBkFiles-1] of AnsiString ;     // Angus 4.60
...
// Windows NT4 stuff, and later
    ...
    begin
        ...
        RasGetAutodialAddress := GetProcAddress(MagRasLib, 'RasGetAutodialAddressA');
        RasSetAutodialAddress := GetProcAddress(MagRasLib, 'RasSetAutodialAddressA');
        RasEnumAutodialAddresses := GetProcAddress(MagRasLib, 'RasEnumAutodialAddressesA');
        RasGetAutodialEnable := GetProcAddress(MagRasLib, 'RasGetAutodialEnableA');
        RasSetAutodialEnable := GetProcAddress(MagRasLib, 'RasSetAutodialEnableA');
        RasGetAutodialParam := GetProcAddress(MagRasLib, 'RasGetAutodialParamA');
        RasSetAutodialParam := GetProcAddress(MagRasLib, 'RasSetAutodialParamA');
        RasGetCredentials := GetProcAddress(MagRasLib, 'RasGetCredentialsA');   // 5.10
        RasSetCredentials := GetProcAddress(MagRasLib, 'RasSetCredentialsA');   // 5.10
...
// Windows XP (NT5.1) stuff, and later
        ...
        begin
            RasDeleteSubEntry := GetProcAddress(MagRasLib, 'RasDeleteSubEntryA');
        end ;
    end ;
...
// rasdlg.dll dialogs
    ...
    begin
        ...
        begin
            RasEntryDlg := GetProcAddress(MagRasDlgLib, 'RasEntryDlgA');               // 5.20
            RasPhonebookDlg := GetProcAddress(MagRasDlgLib, 'RasPhonebookDlgA');       // 5.20
            RasDialDlg := GetProcAddress(MagRasDlgLib, 'RasDialDlgA');                 // 5.20
        end ;
    end ;
...
begin
    ...
    if MagRasOSVersion = OSNT4 then
        MagRasPhoneFiles [0] := AnsiString (GetWinDir + NT4Rasphone)  // 9 Aug 2010
    else if MagRasOSVersion >= OSW2K then  // 5.10, allow for future windows versions
    begin
        MagRasPhoneFiles [REN_User] := AnsiString (GetShellPath (CSIDL_APPDATA) + W2KRasphone) ; // 9 Aug 2010
        MagRasPhoneFiles [REN_AllUsers] := AnsiString (GetShellPath (CSIDL_COMMON_APPDATA) + W2KRasphone) ;  // 9 Aug 2010
    end ;
end ;
