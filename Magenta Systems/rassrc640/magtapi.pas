...
uses
  ...
  MagTapiApi, MagTapiStr, MagSubs1 ;

const
  MagVersion = 'TMagTapi, 3rd September 2012 - Release 5.71, Copyright 2012, Magenta Systems Ltd' ;
...
type
  ...
  public
    ...
    function  GetModemDevice (NewName: AnsiString): integer ;
...
var
    callstr, tinfo1, tinfo2 : String;  // 11 Aug 2010
    ...
    begin
        ...
        begin
          ...
          // get application that started connects, RASAPI32, RASCOM, AMEOL2, etc
            ...
            begin
                tinfo1 := Trim (String (FixedToPasStr (@callinfo.Data [dwAppNameOffset],
                                                             dwAppNameSize))) ;   // 11 Aug 2010
            end ;
...
// change in line state
    ...
    begin
        callstate := dwParam1 ;
     // following call status stuff is probably unnecessary, nothing of interest
  //      callstatus.dwTotalSize := sizeof (callstatus.data) ;
  //     if lineGetCallStatus (hCall, callstatus) = 0 then
  //      with callstatus do
  //      begin
  //            if dwDevSpecificSize > 1 then
  //          begin
  //               info2 := FixedToPasStr (@callstatus.Data [dwDevSpecificOffset],
  //                                         dwDevSpecificSize);
  //          end ;
  //        end;
        ProcCallState ;
...
function  strLastCh(const S: AnsiString): AnsiChar;
begin
    result := #0 ;
    if length (S) <> 0 then Result := S [Length (S)];
end;

function regReadString(aKey: HKEY; const Path: AnsiString): AnsiString;
var
    aRegistry : TRegistry;
    aPath     : AnsiString;
    aValue    : AnsiString;
begin
    ...
    try
        ...
        begin
            ...
            OpenKey(String (aPath),True); // 11 Aug 2010
            Result:=AnsiString (ReadString(String (aValue))); // 11 Aug 2010
        end;
    finally
        aRegistry.Free;
    end;
...
var
    ...
    DeviceID: TVarString ;
    DevName, DevDispName, DevProvider, DevSpecific: AnsiString ;
    DevComPort, DeviceType, DevCommName, DeviceClass: AnsiString ;
    DevRegKey: String ;
    ...
    begin
      ...
      {Negotiate the API version to use for this device}
        ...
        begin
            ...
            retval := lineGetDevCaps (fLineApp, I, DevApiVersion, 0, LineCaps) ;
            if retval <> 0 then
            begin
            ...
            // Extract the device name
                DevName := TrimAnsi (FixedToPasStr (@LineCaps.Data
                                    [dwLineNameOffset], dwLineNameSize)) ; // 11 Aug 2010
                DevDispName := DevName ;

            // Extract the provider information
                DevProvider := TrimAnsi (FixedToPasStr (@LineCaps.Data
                                [dwProviderInfoOffset], dwProviderInfoSize)); // 11 Aug 2010
                J := pos (#0, String (DevProvider)) ; // 11 Aug 2010
                if J > 1 then DevProvider [J] := ' ' ;

              // Extract the registry key - undocumented
                DevSpecific := FixedToPasStr (@LineCaps.Data
                                    [dwDevSpecificOffset], dwDevSpecificSize);
                DevRegKey := trim (copy (String (DevSpecific), 9, 99)) ; // 11 Aug 2010
            ...
            // see if handling modem name specially, ISDN cards on NT4 only
                ...
                // NT seems to reverse ordered
                    ...
                    DevComport := 'ISDN' +  IntToStrAnsi (isdnbase) ;  // 11 Aug 2010
                    inc (isdnbase) ;
                end ;
            ...
            // see if handling name specially on NT4 only
                ...
                if DevName = '' then Devname := AnsiString (STapiGenUnnamed) ;  // (Unnamed Device)  // 11 Aug 2010
...
//                if DeviceType = RASDT_Isdn then DeviceClass := ClassNDIS ;  // temp
                retcode := lineGetID (DevLine, 0, 0, LINECALLSELECT_LINE,
                                              DeviceID, PAnsiChar(DeviceClass)) ;
                if retcode = 0 then
                begin
                    with DeviceId do
                    begin
                        if dwStringSize >  4 then
                        begin
                             DevCommName := TrimAnsi (FixedToPasStr
                                    (@DeviceId.Data [dwStringOffset + 4],
                                                          dwStringSize - 4)) ; // 11 Aug 2010
                        end ;
                    end ;
                end ;
              ...
              // get com port
                ...
                begin
                    ...
                    DeviceID.dwTotalSize := SizeOf (TVarString);
                    retcode := lineGetID (DevLine, 0, 0, LINECALLSELECT_LINE,
                                          DeviceID, PAnsiChar(ClassCommModemPort)) ;
                    if retcode = 0 then
                    begin
                        with DeviceId do
                        begin
                            if dwStringSize > 0 then
                             DevComPort := UpperCaseAnsi (TrimAnsi (FixedToPasStr
                                    (@DeviceId.Data [dwStringOffset],
                                                            dwStringSize))) ; // 11 Aug 2010
                        end ;
                    end ;
                end ;
            end ;
        ...
        // try and get com port from registry - if not got it yet
            if ((DevMedia AND LINEMEDIAMODE_DATAMODEM) <> 0) and
                                                        (DevComPort = '') then
            begin
                if RegOpenKey (HKEY_LOCAL_MACHINE, PChar(DevRegKey),
                                                bKey) = ERROR_SUCCESS then
                begin
                    DevComPort := UppercaseAnsi (regReadString (bKey, 'AttachedTo')) ; // 11 Aug 2010
                    RegCloseKey (bKey);
                end;
            end ;
            if (copy (DevComport, 1, 3) = 'COM') and (length (DevComport) <= 6) then // 12 June 2008 COM1 to COM999
                                      DevPortnr := StrToInt (copy (String (DevComport), 4, 3)) ; // 11 Aug 2010
        ...
        // check for special devices on Windows 2000, RAS uses very strange names
            ...
            begin
                if Pos ('VPN', String (DevProvider)) > 0 then  // 11 Aug 2010
                begin
                ...
                if Pos ('L2TP', String (DevDispName)) > 0 then            // Angus 4.60 // 11 Aug 2010
                begin
                    DeviceType := RASDT_Vpn ;
                    DevComport := 'VPN1' ;
                end ;
                if Pos ('LPT1', String (DevDispName)) > 0 then            // Angus 4.60 // 11 Aug 2010
                begin
        ...
        // still no port, set to (None)
            if (DevComport = '') then DevComport := AnsiString (STapiGenNone) ; // (None)
        ...
        // now add modem object to list
            ...
            begin
                Line := 0 ;
                Name := TrimAnsi (DevDispName) ; // 11 Aug 2010
                TAPIName := DevName ;
                Provider := DevProvider ;
                ApiVersion := DevApiVersion ;
                Media := DevMedia ;
                RegKey := AnsiString (DevRegKey) ; // 11 Aug 2010
                portnr := DevPortnr ;
                PortStr := DevComPort ;
                Device := I ;
                DevType := DeviceType ;
    ...
    if Assigned (fTapiDiag) then
    begin
        DevName := AnsiString (Format (STapiGenModemListed, [fCount])) ;  // TAPI Modems Listed  11 Aug 2010
        fTapiDiag (String (DevName)) ; // 11 Aug 2010
    end ;
end ;
...
function TMagTAPI.GetModemDevice (NewName: AnsiString): integer ;
var
    num: integer ;
begin
    result := -1 ;
        ...
        begin
            ...
            begin
                if Assigned (fTapiDiag) then fTapiDiag (Format (STapiGenMonFailed, // Device x Failed Check For New Calls
                            [Modems [num].Device]) + ': ' + String (Modems [num].TAPIName) +
                                                   ' - Err $' + IntToHex (ret, 8)) ;     // 11 Aug 2010
            end ;
...
function TMagTAPI.TranslateDialog (parh: HWND; devid:
                                    integer; addr : AnsiString) : Boolean;
begin
    Result := (lineTranslateDialog (fLineApp, devid, TAPI_CURRENT_VERSION,
                                                     parh, PAnsiChar(Addr)) = 0);
end;
...
function TMagTAPI.TranslateAddr (devid: integer; inputnum: AnsiString;
                                var disnum, dialnum: AnsiString) : Boolean;
var
...
begin
    ...
    try
    if lineTranslateAddress (fLineApp, devid, TAPI_CURRENT_VERSION,
                                  PAnsiChar(inputnum), 0, 0, TransOutput) = 0 then
        begin
        with TransOutput do
        begin
            disnum := TrimAnsi (FixedToPasStr (@TransOutput.Data
                    [dwDisplayableStringOffset], dwDisplayableStringSize)); // 11 Aug 2010
            ...
            dialnum := TrimAnsi (FixedToPasStr (@TransOutput.Data
                            [dwDialableStringOffset], dwDialableStringSize)); // 11 Aug 2010
            result := true ;
        end;
    end ;
    except
...
begin
    ...
    LineCaps.dwTotalSize := SizeOf (LineCaps) ;
    result := (lineGetDevCaps (fLineApp, devnr, DevApiVersion, 0, LineCaps) = 0) ;
end ;
