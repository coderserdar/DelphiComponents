...
Changed DevicePortW from WideString to UnicodeString
...
uses
  ...
  MagRasApiW, MagTapiApiW, MagRasStr, MagRasEntW, MagSubs1 ;

const
  MagVersion = 'TMagRasEdt, 6th June 2017 - Release 6.40, Copyright 2017, Magenta Systems Ltd' ;
...
{ A common buffer used for RasEntryAll }
 EntryEdtInfo: array [0..8190] of byte ;          // 4.70 Angus, lots of space
...
{ Used for reading and writing phonebook sub entries, followed by a variable number
 ...
 normally necessary to access this information, it is public primarily for
 debugging purposes.  Used by GetAllEntryProps and PutAllEntryProps.  }
  RasSubEntryNT4: TRasSubEntryNT4W absolute SubEntryEdtInfo ;
...
//   Private declarations
    ...
    fCountryID                  : LongInt;
    fAreaCode                   : string;
    fLocalPhoneNumber           : string;
    fPhoneCanonical             : string ;

    // PPP/IP
    fIPAddress                  : string;
    fDNSAddress                 : String;
    fDNSAddressAlt              : string;
    fWINSAddress                : string;
    fWINSAddressAlt             : string;
    ...
    // auto dial
    fAutoDialDll                : string;
    fAutoDialFunc               : string;

    // device
    fDeviceName                 : string;
    fDevicePort                 : string ;     // read only, NT4 only
    fDeviceType                 : string;

    // X25
    fX25Address                 : string;
    fX25Facilities              : string;
    fX25PadType                 : string;
    fX25UserData                : string;
   ...
   // who knows
    ...
    fPrerequisitePbk            : string ;
    fPrerequisiteEntry          : string ;
    fRedialCount                : DWORD ;
    fRedialPause                : DWORD ;
...
// real procs to manipulate properties
    ...
    procedure SetSubLocalPhoneNumber (Index: Integer; Value: string) ;
    function GetSubPhoneCanonical (Index: Integer): string ;
    procedure SetSubPhoneCanonical (Index: Integer; Value: string) ;
    function GetSubDeviceType (Index: Integer): string ;
    procedure SetSubDeviceType (Index: Integer; Value: string) ;
    function GetSubDeviceName (Index: Integer): string ;
    procedure SetSubDeviceName (Index: Integer; Value: string) ;
    function GetSubDevicePort (Index: Integer): string ;
    procedure SetSubDevicePort (Index: Integer; Value: string) ;
    ...
    function GetEntry (EntryName: string): LongInt ;
    function PutEntry (EntryName: string): LongInt ;
    procedure SetPhoneBookPath (Value: string) ;    // 4.60
...
{ Convert a RASIP address into a ASCII representation (ie 127.0.0.1).
  ...
  Result is string with an ASCII version of the IP address.}
    function IPToStr (IPAddr: TRASIPAddr): string;
...
{ Convert an ASCII representation of an IP address (ie 127.0.0.1) into
  ...
  Calls Str2IP. }
    function StrToIP (strIP: string): TRASIPAddr;
...
{ Convert an ASCII representation of an IP address (ie 127.0.0.1) into
  ...
  IPAddr is the binary IP address. }
    function Str2IP (strIP: string; var IPAddr: TRASIPAddr): boolean ;
...
{ Encapsulates the RasGetEntryProperties and RasGetSubEntryProperties functions
  ...
  Calls GetSubEntryProps.  }
    function GetAllEntryProps (EntryName: string): LongInt ;
...
{ Splits a canonical phone number into the properties  CountryCode, AreaCode,
  ...
  '+country code (area code) local number'. }
    procedure SetCanonical (PhoneCanonical: string) ;
...
{ Encapsulates the RasSetEntryProperties and RasSetSubEntryProperties
  ...
  Calls PutSubEntryProps.  }
    function PutAllEntryProps (EntryName: string): LongInt ;
...
{ Encapsulates the RasGetSubEntryProperties function to retrieve properties
  ...
  only by GetAllEntryProperties. }
    function GetSubEntryProps (EntryName: string;
...
{ Encapsulates the RasSetSubEntryProperties function to set properties
  ...
  only by PutAllEntryProperties. }
    function PutSubEntryProps (EntryName: string;
                                            SubEntry: integer): LongInt ;
...
{ Encapsulates the RasGetEntryDialParams function to retrieve connection
  ...
  and Domain. }
    function GetDialProps (EntryName: string): LongInt ;
...
{ Encapsulates the RasSetEntryDialParams function to save connection
  ...
  Result is 0 if successful, or an error code.  }
    function PutDialProps (EntryName: string): LongInt ;
...
{ Specifies a telephone number for a sub entry in a phonebook entry.
  ...
  all sub entries.}
    PROPERTY SubLocalPhoneNumber [index: integer]: string read GetSubLocalPhoneNumber write SetSubLocalPhoneNumber ;
...
{ Specifies the canonical phone number for a sub entry in a phonebook entry.
  ...
  CountryCode, AreaCode and sub entry LocalPhoneNumber.}
    PROPERTY SubPhoneCanonical [index: integer]: string read GetSubPhoneCanonical write SetSubPhoneCanonical ;
...
{ Specifies the RAS device type for DeviceName for a sub entry in a phonebook.
  ...
  RASDT_Vpn  - A virtual private network connection.}
    PROPERTY SubDeviceType [index: integer]: string read GetSubDeviceType write SetSubDeviceType ;

{ Specifies the name of a TAPI device use with a sub entry in a phonebook
  entry.}
    PROPERTY SubDeviceName [index: integer]: string read GetSubDeviceName write SetSubDeviceName ;

{ Specifies the com port of a TAPI device use for a sub entry in a phonebook
  entry, NT4 only. }
    PROPERTY SubDevicePort [index: integer]: string read GetSubDevicePort write SetSubDevicePort ;
...
{ Specifies a telephone number for a phonebook entry. The way RAS uses this
  ...
  as the entire phone number.  }
    property LocalPhoneNumber : String      read fLocalPhoneNumber write fLocalPhoneNumber;
...
{ Specifies the canonical phone number for a phonebook entry.  This is
  ...
  CountryCode, AreaCode and LocalPhoneNumber.  }
    property PhoneCanonical : string        read fPhoneCanonical write fPhoneCanonical ;
...
{ Specifies the IP address of a secondary or backup DNS server to be used
  while this connection is active. Ignored unless bSpecificNameServers is set. }
    property DNSAddressAlt : string         read fDNSAddressAlt write fDNSAddressAlt;

{ Specifies the IP address of the WINS server to be used while this
  connection is active. Ignored unless bSpecificNameServers is set. }
    property WINSAddress : string           read fWINSAddress write fWINSAddress;
...
{ Specifies the IP address of a secondary WINS server to be used while
  this connection is active. Ignored unless bSpecificNameServers is set. }
    property WINSAddressAlt : string        read fWINSAddressAlt write fWINSAddressAlt;
...
{ Specifies the full path and filename of the dynamic link library (DLL) for
  ...
  the AutoDialFunc member is ignored.}
    property AutoDialDLL : String           read fAutoDialDLL write fAutoDialDll;
...
{ Specifies the the exported name of the RASADFunc function for the
  ...
  "A" or "W" suffix in the name specified by AutoDialFunc. }
    property AutoDialFunc : String          read fAutoDialFunc write fAutoDialFunc;
...
{ Specifies the com port of a TAPI device use with a phonebook entry, NT4
  only, can not be updated.}
    property DevicePort : string            read fDevicePort ;
...
{ Specifies the X.25 address to connect to. }
    property X25Address : string            read fX25Address write fX25Address ;

{ Specifies the XX.25 facilities. }
    property X25Facilities : string         read fX25Facilities write fX25Facilities ;

{ Specifies the X.25 PAD type. }
    property X25PadType : string            read fX25PadType write fX25PadType ;

{ Specifies additional connection information supplied to the X.25 host
  at connection. }
    property X25UserData : string           read fX25UserData write fX25UserData ;
...
{ Specifies the the full path and filename for the DLL that implements
  the custom dialing functions, W2K and later only. }
    property CustomDialDll: string          read fCustomDialDll write fCustomDialDll  ;
...
{ Specifies the full path and file name of a phone-book (PBK) file. This
  ...
  property, used only for VPN connections, XP only. }
   property  PrerequisitePbk: string            read fPrerequisitePbk          write fPrerequisitePbk ;
...
{ Specifies a phone-book entry. This entry should exist in the phone-book
  ...
  properties, used only for VPN connections, XP only. }
   property  PrerequisiteEntry: string          read fPrerequisiteEntry        write fPrerequisiteEntry ;
...
// 5.20 warning - on Vista elevated admin rights are needed to create AllUsers phonebook
    ...
    begin
        if Pos (':', MagRasPhoneFiles [I]) <> 2 then continue ;
        if NOT FileExists (MagRasPhoneFiles [I]) then
        begin
            MagRasPhoneBookInf.Add (' ') ;  // one blank line
            try
                if ForceDirs (ExtractFileDir (MagRasPhoneFiles [I])) then // 4.71
                        MagRasPhoneBookInf.SaveToFile (MagRasPhoneFiles [I]) ;
            except
            end ;
...
function TMagRasEdt.GetSubLocalPhoneNumber (Index: Integer): string ;
begin
    result := fSubLocalPhoneNumber [Index] ;
end ;

function TMagRasEdt.GetSubPhoneCanonical (Index: Integer): string ;
begin
    result := fSubPhoneCanonical [Index] ;
end ;

function TMagRasEdt.GetSubDeviceType (Index: Integer): string ;
begin
    result := fSubDeviceType [Index] ;
end ;

function TMagRasEdt.GetSubDeviceName (Index: Integer): string ;
begin
    result := fSubDeviceName [Index] ;
end ;

function TMagRasEdt.GetSubDevicePort (Index: Integer): string ;
begin
...
procedure TMagRasEdt.SetSubLocalPhoneNumber (Index: Integer; Value: string) ;
begin
    if value <> fSubLocalPhoneNumber [Index] then
                       fSubLocalPhoneNumber [Index] := value ;
end ;

procedure TMagRasEdt.SetSubPhoneCanonical (Index: Integer; Value: string) ;
begin
    if value <> fSubPhoneCanonical [Index] then
                       fSubPhoneCanonical [Index] := value ;
end ;

procedure TMagRasEdt.SetSubDeviceType (Index: Integer; Value: string) ;
begin
    if value <> fSubDeviceType [Index] then
                       fSubDeviceType [Index] := value ;
end ;

procedure TMagRasEdt.SetSubDeviceName (Index: Integer; Value: string) ;
begin
    if value <> fSubDeviceName [Index] then
                       fSubDeviceName [Index] := value ;
end ;

procedure TMagRasEdt.SetSubDevicePort (Index: Integer; Value: string) ;
begin
...
procedure TMagRasEdt.SetPhoneBookPath (Value: string) ;    // 4.60
begin
    exit ; // 6.0 no custom paths for W2K and later
end;
...
function TMagRasEdt.IPToStr(IPAddr : TRASIPAddr) : string;
var
  strTemp : String;
begin
  strTemp := IntToStr(IPAddr.a);
  strTemp := strTemp + '.' + IntToStr(IPAddr.b);
  strTemp := strTemp + '.' + IntToStr(IPAddr.c);
  strTemp := strTemp + '.' + IntToStr(IPAddr.d);
  Result := strTemp;
end;

function TMagRasEdt.StrToIP (strIP: string): TRASIPAddr ;
begin
    Str2IP (strIP, result) ;
end ;

function TMagRasEdt.IsIPStr (strIP: string): boolean ;
var
    IPAddr: TRASIPAddr ;
begin
...
function TMagRasEdt.IsFmtIPStr (var strIP: string): boolean ;
var
    IPAddr: TRASIPAddr ;
begin
    result := Str2IP (strIP, IPAddr) ;
...
function TMagRasEdt.Str2IP (strIP: string; var IPAddr: TRASIPAddr): boolean ;
var
...
function TMagRasEdt.GetEntry (EntryName: string): LongInt ;
var
    EntryNameW: WideString ;
    tempsize: DWORD ;
begin
    EntryNameW := EntryName ;
...
// get buffer sizes
    Result := RasGetEntryPropertiesW (fPBPtr, PWideChar (EntryNameW),
                                            nil, EntryEdtSize, nil, tempsize) ;
...
// get entry details
    ...
    begin
        ...
        Result := RasGetEntryPropertiesW (fPBPtr, PWideChar (EntryNameW),
                                    @EntryEdtInfo, EntryEdtSize, Nil, tempsize) ;
    end ;
    LastError := Result ;
...
function TMagRasEdt.GetAllEntryProps (EntryName: string): LongInt ;
var
    EntryNameW, DevicePortW: UnicodeString ;   // 6.10
    strptr: PWideChar ;
    ch: WideChar ;
...
// get entry and device details, unpack for properties
    ...
    begin
        ...
        // warning, NT4 (not W2K and later) devices have two nulls strings, device and port
            ...
            fDeviceName := GetDevNamePortW (szDeviceName, Length (szDeviceName), DevicePortW);
            fDevicePort := DevicePortW ; // 6.0
...
function TMagRasEdt.GetSubEntryProps (EntryName: string;
                                            SubEntry: integer): LongInt ;
var
    EntryNameW, DevicePortW: UnicodeString ;   // 6.10
    strptr: PWideChar ;
...
// WARNING - RASSubEntryNT4 is in same absolute memory as SubEntryEdtInfo
    ...
    RasSubEntryNT4.dwSize := Sizeof (RasSubEntryNT4);

// get buffer sizes
    Result := RasGetSubEntryPropertiesW (fPBPtr, PWideChar (EntryNameW),
       ...
       // Device name, port and type
        // warning, NT4 (not W2K and later) devices have two nulls strings, device and port
            ...
            fSubDeviceName [SubEntry] := GetDevNamePortW (szDeviceName,
                                            Length (szDeviceName), DevicePortW);
            fSubDevicePort [SubEntry] := DevicePortW ;
            ...
            begin
                ...
                begin
                    ...
                    fSubAltPhoneNrList [SubEntry].Add
                                            (Trim (FixedToPasStrW (strptr, stringlen))) ;
                    strptr := strptr + stringlen + 1 ;
                    offset := offset + stringlen + 1 ;
...
procedure TMagRasEdt.SetCanonical (PhoneCanonical: string) ;
begin
...
function TMagRasEdt.PutEntry (EntryName: string): LongInt ;
var
    EntryNameW: WideString ;
//    devptr: pointer ;
begin
...
//    if DeviceEdtSize <> 0 then devptr := @DeviceEdtInfo ;
    EntryNameW := EntryName ;
    Result := RasSetEntryPropertiesW (fPBPtr, PWideChar (EntryNameW),
                                       @EntryEdtInfo, EntryEdtSize, Nil, 0) ;
    LastError := Result ;
    if LastError <> 0 then  StatusStr := GetErrorString (LastError) ;
end ;
...
function TMagRasEdt.PutAllEntryProps (EntryName: string): LongInt ;
var
    EntryNameW: WideString ;
    strptr: PWideChar ;
    I: integer ;
...
// WARNING - RasEntryAll is in same absolute memory as EntryEdtInfo
 ...
 // set properties in same order as RASENTRY
    ...
    // Location/phone numbers
        ...
        StringToWideChar (fLocalPhoneNumber, szLocalPhoneNumber, RAS_MaxEntryName);
        StringToWideChar (fAreaCode, szAreaCode, RAS_MaxAreaCode);
   ...
   // PPP/IP
    ...
    // Scripting
        StringToWideChar (fScript, szScript, SizeOf(szScript));

    // Autodial
        StringToWideChar (fAutoDialDll, szAutodialDll, SizeOf(szAutodialDll));
        StringToWideChar (fAutoDialFunc, szAutodialFunc, SizeOf(szAutoDialFunc));

    // Device
        StringToWideChar (fDeviceType, szDeviceType, RAS_MaxDeviceType );
        StringToWideChar (fDeviceName, szDeviceName, RAS_MaxDeviceName );

    // X25
        StringToWideChar (fX25PadType, szX25PadType, RAS_MaxPadType);
        StringToWideChar (fX25Address, szX25Address, RAS_MaxX25Address );
        StringToWideChar (fX25Facilities, szX25Facilities, RAS_MaxFacilities);
        StringToWideChar (fX25UserData, szX25UserData, RAS_MaxUserData);
  ...
  //        guidId := fguidId ;  // not settable  ??  // Angus 4.3
        StringToWideChar (fCustomDialDll, szCustomDialDll, MAX_PATH - 1);
    ...
    // Windows XP extensions - 4.62
      ...
      //    dwfOptions3;  // spare
            StringToWideChar (fDnsSuffix, dzDnsSuffix, RAS_MaxDnsSuffix - 1) ;
            dwTcpWindowSize := fTcpWindowSize ;
            StringToWideChar (fPrerequisitePbk, szPrerequisitePbk, MAX_PATH - 1) ;
            StringToWideChar (fPrerequisiteEntry, szPrerequisiteEntry, RAS_MaxEntryName) ;
...
//                                              fAltPhoneNrList.Delete (0) ;
            ...
            begin
                ...
                strptr := @EntryEdtInfo [EntryEdtSize] ;  // WideChar
                for I := 0 to fAltPhoneNrList.Count - 1 do
                begin
                    stringlen := length (fAltPhoneNrList [I]) ;
                    if stringlen > 0 then
                    begin
                        temp := fAltPhoneNrList [I] ;
                        move (temp[1], strptr^, stringlen * 2) ;  // WideChars
                        strptr := strptr + stringlen + 1 ;
...
// see if deleting un-needed subentries (before saving main entry), XP only - 4.62
    ...
    begin
        ...
        begin
            RasDeleteSubEntryW (fPBPtr, PWideChar (EntryNameW), fSubInitTotal) ;
            dec (fSubInitTotal) ;
        end ;
    end ;
...
function TMagRasEdt.PutSubEntryProps (EntryName: string;
                                            SubEntry: integer): LongInt ;
var
    EntryNameW: WideString ;
    strptr: PAnsiChar ;
...
// WARNING - RASSubEntryNT4 is in same absolute memory as SubEntryEdtInfo
    SubEntryEdtSize := Sizeof (TRasSubEntryNT4W);
    FillChar (SubEntryEdtInfo, SizeOf (SubEntryEdtInfo), #0) ;
    ...
    begin
        ...
        // Device
        StringToWideChar (fSubDeviceType [SubEntry], szDeviceType, RAS_MaxDeviceType );
        StringToWideChar (fSubDeviceName [SubEntry], szDeviceName, RAS_MaxDeviceName );

        // phone number
        StringToWideChar (fSubLocalPhoneNumber [SubEntry],szLocalPhoneNumber,
                                                               RAS_MaxEntryName);
...
//                                  fSubAltPhoneNrList [SubEntry].Delete (0) ;
            ...
            begin
                ...
                begin
                    ...
                    if stringlen > 0 then
                    begin
                        temp := fSubAltPhoneNrList [SubEntry] [I] ;
...
// update phonebookcurrently ignoring the device specific information
    Result := RasSetSubEntryPropertiesW (fPBPtr, PWideChar (EntryNameW),
...
FUNCTION TMagRasEdt.GetDialProps (EntryName: string): LongInt ;
var
    EntryNameW: WideString ;
...
begin
    ...
    LastError := RasGetCredentialsW (fPBPtr, PWideChar (EntryNameW),
                                                     RasCredentials) ;
    result := LastError ;
    if LastError = 0 then
    begin
...
function TMagRasEdt.PutDialProps (EntryName: string): LongInt ;
var
    EntryNameW: WideString ;
//    removePW: boolean ;
    RasCredentials: TRasCredentialsW ;
begin
    ...
    LastError := RasSetCredentialsW (fPBPtr, PWideChar (EntryNameW),
...
begin
    ...
    while nextid <> 0 do
    begin
        Buffsize := SizeOf(TRasExtCtryInfoW) ;
        ...
        stringlen := StrLenWide (strptr) ;
        fCountryList.Add (Trim (FixedToPasStrW (strptr, stringlen)) + ' (' +
                IntToStr (RasExtCtryInfo.RasCtryInfo.dwCountryCode) + ')') ;
