TMAGRAS - DELPHI RAS COMPONENT - RELEASE 6.40
=============================================

Last Updated: 22nd November 2018, Release 6.40
...
7, 8, 2012, 10, 2016 and 2019. The last version of TMagRas compatible
with windows 9x and NT4 was release 5. With XE2 and later, Win64 is
supported as a target, as well as Win32. Windows XP and earlier are
no long supported but should still work.
...
getting performance information for connections.

TMagRas 6.40 is supplied as five separate installable Delphi components:
...
edit connections/phonebooks, and TMagTapi lists TAPI devices and monitors
them (but does not make calls).
...
TMagLsaPw provides access to RAS credentials bypassing the RAS APIs,
...
TMagRas 6.40 is copyrighted software, for which the component source
code may be licensed for a cost of £100 (UKP) (£120 with UK tax).
Applications using TMagRas may be distributed without further cost.
Existing licensees of the earlier TMagRas versions will need to pay
a £50 (UKP) (£60 with UK tax) upgrade fee for access to TMagRas 6.00.
...
Delphi 2009 and later support Unicode, with types String=UnicodeString
...
the UnicodeString type. TMagRas 6.00 will work with earlier versions of
Delphi, but only with ANSI strings.  All the TMagRas 6 widestring RAS
units have W suffixes to keep them separate from the old units, but the
component names are the same so only one version can be installed on
toolbar at the time. To simplify the source code, support has been
removed from TMagRas 6 and later for Windows 9.x and NT4, which are now
15 years old and rarely used, so TMagRas 5.40 should be retained if support
for these is still needed. The main demo application is now RASDEMO6.
TMagRas 6 is only available to source code licensees, no free compiled
objects version is available.


Changes in TMagRas Release 6.40
-------------------------------

1 - Windows 10 version 1703 (March 2017) broke RAS modem dialling, it
...
2- To fix using regedit, key HLM\SYSTEM\CurrentControlSet\Services\RasMan,
...
3 - We have provide a simple Windows Powershell script win10ras.ps1 which
will add the new privilege to the registry manually, and added a FixWin10Ras
method to TMagRasCon which may be called when an application starts to do
it automatically.  The method has flags to indicate if a reboot is required,
and whether dialup is broken, note administrator rights are needed for any
of these registry changes.

4 - The rasdemo6 sample has been updated to use the new FixWin10Ras method.
Also increased the window size since we now all use massive monitors.

5 - Beware Microsoft could fix the Windows 10 bug in a different way so this
change may need reviewing in a few months.

6 - Fixed a problem with VPN addresses that may have been corrupted by
TransAddr and TranslateAddress, which now check for an IP address or host
name and don't attempt to generate a dialling string.

7 - Corrected Windows 10/2016 version display.

8 - Added packages for Delphi 10.3 Rio which is version 26.  Display
Windows 2019 versison correctly.


Changes in TMagRas Release 6.33
-------------------------------
...
which is version 24 and Delphi 10.2 Tokyo which is version 25.  Note only
Windows platforms are supported.


Changes in TMagRas Release 6.32
-------------------------------
...
3 - No longer supporting Windows 2000 or earlier, although no code has
been removed yet.


Changes in TMagRas Release 6.31
-------------------------------
...
4 - Removed incoming call monitoring with TMagRasAdm, not supported since
NT4.  Clean up code removing old Win9x and NT4 code and winperf.pas.


Changes in TMagRas Release 6.30
-------------------------------

1 - Fixes for 64-bit compiler, mainly in magrasapiw where DWORD was often
used instead of pointers (which are 64-bits in Win64).  Note the various
RAS dialogs do not seem to display in Win64 applications, and MagLsaPw
does not yet run on Win64, still under investigation.
...
3 - RegisterComponent is now in magrasregw.pas instead of each of the
...
Changes in TMagRas Release 6.20
-------------------------------

1 - Fixed cast warnings for Delphi 2009 and later

2 - Updated GetOSVersion to use GetProductInfo for edition with Vista and
later so the correct edition names are always displayed.

3 - Updated all the example applications to support TMagRas version 6, they
all have the suffix 6, see example6.txt.

4 - Added packages for Delphi XE, which is version 15.


Changes in TMagRas Release 6.10
-------------------------------
...
Changes in TMagRas Release 6.00
-------------------------------

1 - Support RAS WideChar APIs and Unicode for Delphi 2009 with new
MagRasxxxW units. Removed support for Win9x and NT4, both 10 years old now
...
2 - There are two new demo applications, RASDEMO6 demonstrates all the
unicode RAS functions, while TAPIDEMO6 is for the Unicode Tapi functions.
Note the Example applications have not been updated for 6.00, they are
all still 5.40.

3 - The TEntryRec has a new element DialParamsUID read from the
rasphone.pbk file.

4 - Added a new unit MagLsaPw that may be used to find the RAS
credentials (username and password) from the Local Security Authority
(LSA) protected subsystem of Windows that maintains information about
all aspects of local security on a system, saved in the registry under
the protected key: HKEY_LOCAL_MACHINE\SECURITY\Policy\Secrets\.  This
overcomes the limitation in Windows 2000 and later where the RAS APIs
are unable to read clear text passwords.  A new DEMOPW application
lists all RAS entries with the credentials.  Currently, the MagLsaPw
unit has functions to write to LSA, but not to specifically update RAS
credentials.

5 - There is another new demo program PSTEST that demonstrates reading
from the Protected Storage service, another protected part of the
registry used to store confidential information, specifically Outlook
Express credentials, but which may be used to save application passwords.
...
application as needed, as shown in the RASDEMO6 program.  However the
example programs do require TMagRas to be properly installed.

TMagRas 6.x is supplied as source code only. To install the component,
the files should be copied into a library directory, typically a
...
Note that due to common units and component names, only TMagRas v5 or
v6 packages may be installed, not both together.  However, both versions
can be created in code (as is done in the demo program) and used in
different projects.
...
TMagRas works with all current versions of Windows by internally
checking the version and using different APIs, DLLs and structures as
needed.  Some functionality is missing from earlier versions, as
detailed below. The DUNInfo property in TMagRasCon shows the actual
DUN version, based on the rasapi32.dll version number.

Windows 95 Retail - came with DUN 1.0 which did not support phonebook
editing, an extra DLL rnaph.dll is supplied with TMagRas that adds this
support.  It is not needed for any subsequent Windows release.  No
support for performance statistics or ISDN.  Can be upgraded by DUN 1.2
or 1.3 from http://www.microsoft.com/windows95/downloads/default.asp.
No longer supported.

Windows 95 OSR2 - came with DUN 1.1 which supports phonebook editing
and ISDN, but still no performance statistics.  Can be upgraded by DUN
1.2 and 1.3. No longer supported.

Windows 95 with DUN 1.2, 1.3 or 1.4 - supports phonebook editing and
performance statistics using TMagRasPer.  Although ISDN multilink is
supported by the Windows property pages, the RAS API was never updated
for Win9x so multilink entries can not be created by the component.
rnaph.dll is no longer needed.  Performance statistics are read from
simple registry keys. No longer supported.

Windows 98 - came with DUN 1.3, see comments above.

Windows ME - came with DUN 1.3, see comments above.

DUN 1.4 became available in 2001 for Windows 95, 98 and 98SE.  The main
RAS DLL has not changed since 1.3, so it can not be easily identified,
but it provides better security.

Windows NT4 SP0 - had the same RAS functionality as Windows 95, but
not supported by TMagRas.

Windows NT4 SP1 and later - RAS changes included extended dialling
(ConnectNT method), multilink support (sub entries), phonebook files,
and the actual telephone number dialled by RAS.  Performance statistics
usually originate from a special registry key, but sometimes these
'disappear' so an alternative is to use pdh.dll that is supplied with
TMagRas but which provides slightly less detailed information.
Performance statistics do not include the modem connection speed, which
must be obtained from TAPI using TMagTapi. Can not delete a sub entry,
only a whole connection entry.  All entries are usually in the same
system wide phonebook file.  Note the RAS APIs can not access all the
parameters set-up in the Windows property pages.

Windows 2000 - finally added new RAS statistics functions which include
the modem connection speed.  Also added a lot of new security stuff for
phonebook entries.  There are multiple phonebook files, one for 'all
users' and one for each defined user.  Enumerating phonebook entries
returns the location of each entry, and this should be used when editing
the entry so it's saved in the correct file. Note the RAS APIs can still
...
Windows Server 2008 - same as Vista, except programs again have
...
Windows Server 2012 - same as Windows 8, except programs have
administrator rights as standard.

Windows 8.1 - internally this is version 6.3 and similar to 7.

Windows Server 2012 R2 - same as Windows 8.1, except programs have
administrator rights as standard.
