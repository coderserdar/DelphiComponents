...
Apr 15, 2022 V1.28 Added IPv6 support.
                   Added dummy telnet option negociation.
                   Added echo selection.
...
type
  ...
  TClientForm = class(TForm)
    ...
    procedure EchoCheckBoxClick(Sender: TObject);
  private
    FCommand     : String;
    FRcvdCR      : Boolean;
    FDataFlag    : Boolean;
    FTelnetChar1 : AnsiChar;
    FTelnetChar2 : AnsiChar;
    FEcho        : Boolean;
  ...
  public
    ...
    Reference  : Pointer;
    PortNum    : Integer;
    property Echo        : Boolean    read  FEcho
                                      write SetEcho;
  end;
...
begin
    if FTelnetChar1 = #0 then begin
        ...
        else if Ch = TNCH_IAC then begin
            // Telnet IAC character
            FTelnetChar1 := Ch;
            Exit;
        end;
    end
    else if FTelnetChar1 = TNCH_IAC then begin
        FTelnetChar1 := Ch;
        Exit;
    end
    else begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TClientForm.SetEcho(const Value: Boolean);
begin
    FEcho                := Value;
    EchoCheckBox.Checked := FEcho;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TClientForm.EchoCheckBoxClick(Sender: TObject);
begin
    FEcho := EchoCheckBox.Checked;
end;
