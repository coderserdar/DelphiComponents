...
Support:      Use the forum https://en.delphipraxis.net/forum/37-ics-internet-component-suite/
...
Sep 27, 1997 V1.21 Adapted for TWSocket converted to support C++Builder
...
Apr 15, 2022 V1.27 Added IPv6 option, echo option and dummy telnet option
                   negociation.
...
type
    ...
    TServerForm = class(TForm)
        ...
        IPv4RadioButton: TRadioButton;
        IPv6RadioButton: TRadioButton;
        ...
        procedure IPv4RadioButtonClick(Sender: TObject);
        procedure IPv6RadioButtonClick(Sender: TObject);
    protected
...
begin
    SrvSocket.Close;
    // Use any interface for listening
    if IPv4RadioButton.Checked then
        SrvSocket.Addr  := ICS_ANY_HOST_V4
    else
        SrvSocket.Addr  := ICS_ANY_HOST_V6;
    SrvSocket.Proto := 'tcp';
...
var
    NewHSocket   : TSocket;
    PeerName6    : TSockAddrIn6;
    PeerName4    : TSockAddrIn;
    Client       : TClient;
...
begin
    ...
    Client.Form.Echo          := EchoCheckBox.Checked;
    ClientSocket              := Client.Form.Socket;
    ClientSocket.SocketFamily := SrvSocket.SocketFamily;
    ClientSocket.Dup(NewHSocket);

    if ClientSocket.SocketFamily = sfIPv6 then begin
        ClientSocket.GetPeerName(PSockAddrIn(@PeerName6)^, Sizeof(PeerName6));
        Client.Peer := WSocketIPv6ToStr(@PeerName6)
    end;
    if SrvSocket.SocketFamily = sfIPv4 then begin
        ClientSocket.GetPeerName(PeerName4, Sizeof(PeerName4));
        Client.Peer := WSocketIPv4ToStr(PeerName4.sin_addr.S_addr);
    end;
...
{$ENDIF}
           ...
           'Free Software, Copyright François Piette 1997-2022' + #10 + #10 +
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TServerForm.IPv4RadioButtonClick(Sender: TObject);
begin
    RestartServer;
end;
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TServerForm.IPv6RadioButtonClick(Sender: TObject);
begin
    RestartServer;
end;
