 ...
 * March 2, 2022 - V8.69 Added comments about samples and units using SuperObject.
 * June 6, 2022  - V8.70 Fixed bug in V8.65 where FloattoStr could create Json with
 *                    comma decimal point instead of a period with some languages.
 *
 *
 *
 ...
 * into a grid at multiple levels.  The OverbyteIcsHttpRestTst.dpr sample does the same thing
 * for Json APIs over the internet.  The OverbyteIcsSslX509Certs.pas unit makes extensive use
 * of SuperObject to create and parse Json for SSL certificate orders.
 *
 *
 *
 *  v1.2
...
//  fbuffer: array[0..31] of AnsiChar;
  JsonFormatSettings: TFormatSettings;       { V8.70 }
const
  ENDSTR_A: PSOChar = '": ';
  ENDSTR_B: PSOChar = '":';
begin
  ...
  { V8.70 must not use localised format symbols for floating point conversion }
 ...
 //           Result := Append(PSOChar(SOString(FloatToStrF(FO.c_double, ffFixed, 15, 10))));  { V8.65 was gcvt }
         Result := Append(PSOChar(SOString(FloatToStrF(FO.c_double, ffFixed, 15, 10, JsonFormatSettings))));  { V8.70 need period for decimal sep }
      stCurrency:
        begin
