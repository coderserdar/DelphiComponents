{**************************************************************************************}
{                                                                                      }
{ AutoCorrect components for Delphi 2005 or later                                      }
...
unit CCR.AutoCorrect;
{
  ...
  - For D7-2007, the code is not MBCS aware, and for D2009+, UCS2 is assumed. That said,
    something like AutoCorrecting TWo INitial CApitals possibly only makes sense for
    languages that are unaffected by this limitation anyway.
  - While the code internally makes heavy use of EM_XXX messages, for the TWo INitial
    CApitals correction, what is the current word is determined 'manually' rather than
    via the EM_FINDNEXTWORDBREAK message. The algorithm used is very simple: firstly, 48
    characters (or thereabouts) from the current selection going backwards is copied
    from the control; secondly, the buffer is traversed, again going backwards, until
    the current character is neither a letter nor a digit, the latter determined by the
    TCharacter RTL in D2009+ and the CharLower/CharUpper API in earlier versions.
  - Up to D2009, TRichEdit just wraps v1 of the rich edit DLL, and even in D2009,
    CodeGear did the absolute minimum necessary to use v2. Nonetheless, the v1 DLL in
    Windows 2000 and above is not the 'real' one, but a wrapper round the v3 DLL, which
    means various v3 messages can be sent to it and v3 interfaces queried from it, a
    fact this unit takes advantage of (look for TCharFormat2 and ITextDocument for where
    and how exactly).
  - This PAS file should be UTF-8, since there's some non-ASCII character constants.
}
interface
...
type
  ...
  public
    ...
    procedure ApplyFormatting(Dest: TCustomRichEdit); overload; inline;
  ...
  public type
    ...
    strict private
      FIndex: Integer;
      FCollection: TAutoCorrectEntries;
      function GetCurrent: TAutoCorrectEntry; inline;
    public
      constructor Create(ACollection: TAutoCorrectEntries);
      function MoveNext: Boolean; inline;
      property Current: TAutoCorrectEntry read GetCurrent;
    end;
  strict private
  ...
  protected
    ...
    function GetDummyEntry(Key: Char): TAutoCorrectEntry; inline;
...
procedure OffsetFirstIndent(RichEdit: TCustomRichEdit; Amount: Integer); inline; overload;
procedure OffsetLeftIndent(RichEditWnd: HWND; Amount: Integer); overload;
procedure OffsetLeftIndent(RichEdit: TCustomRichEdit; Amount: Integer); inline; overload;

const
  EnDash = Char('–');
  EmDash = Char('—');
  TabChar = #9;
  LeftSingleQuote = Char('‘');
  LeftDoubleQuote = Char('“');
  RightSingleQuote = Char('’');
  RightDoubleQuote = Char('”');

implementation

uses
  {$IFDEF UNICODE}Character,{$ENDIF}Contnrs, Math, RichEdit, Themes, CCR.AutoCorrect.Consts;
...
function IsLetterOrDigit(Ch: Char): Boolean; inline;
begin
...
procedure Edit_GetSel(EditWnd: HWND; var SelStart, SelEnd: Integer); inline;
begin
  SendMessage(EditWnd, EM_GETSEL, WPARAM(@SelStart), LPARAM(@SelEnd));
end;

procedure Edit_ReplaceSel(EditWnd: HWND; const NewText: string); inline;
begin
  SendMessage(EditWnd, EM_REPLACESEL, -1, LPARAM(PChar(NewText)));
end;

procedure Edit_SetSel(EditWnd: HWND; const SelStart, SelEnd: Integer); inline;
begin
  SendMessage(EditWnd, EM_SETSEL, WPARAM(SelStart), LPARAM(SelEnd));
end;

function IsKeyDown(Key: Word): Boolean; inline;
begin
...
{ TAutoCorrectEntries - for/in loop support }

constructor TAutoCorrectEntries.TEnumerator.Create(ACollection: TAutoCorrectEntries);
begin
  FCollection := ACollection;
  FIndex := -1;
end;

function TAutoCorrectEntries.TEnumerator.GetCurrent: TAutoCorrectEntry;
begin
  Result := FCollection[FIndex];
end;

function TAutoCorrectEntries.TEnumerator.MoveNext: Boolean;
begin
...
function TAutoCorrectEntries.GetEnumerator: TEnumerator;
begin
...
var
  IniFile: TMemIniFile;
begin
  if (Behaviour = lbKeepExisting) and not FileExists(FileName) then Exit;
...
var
  IniFile: TMemIniFile;
begin
  IniFile := TMemIniFile.Create('', TEncoding.UTF8);
  try
    SaveToIniFile(IniFile);
    IniFile.Rename(FileName, False);
    IniFile.UpdateFile;
  finally
    IniFile.Free;
  end;
