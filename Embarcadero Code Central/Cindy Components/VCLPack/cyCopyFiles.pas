...
function cyCopyFileToDir(const SourceFile: string; ToDir: String; const FileExists: TCopyFileExists = feCopy; FileNotExists: TCopyFileNotExists = fnCopyForceDir; const ResetAttr: Boolean = false): TCopyFileResult;

function cyCopyFile(const FromFile, ToFile: String; const FileExists: TCopyFileExists = feCopy; const FileNotExists: TCopyFileNotExists = fnCopyForceDir; const ResetAttr: boolean = false): TCopyFileResult;

function cyCopyFileEx(const FromFile, ToFile: String; aProgressBar: TProgressBar;
                      const FileExists: TCopyFileExists = feCopy; const FileNotExists: TCopyFileNotExists = fnCopyForceDir; const ResetAttr: boolean = false): TCopyFileResult;

function cyCopyFilesEx(const SourcePath, DestinationPath, IncludeFilters, ExcludeFilters: String; const ArchiveFiles, ReadOnlyFiles, HiddenFiles, SystemFiles: TcyFileAttributeMode;
                       const FileExists: TCopyFileExists = feCopy; const FileNotExists: TCopyFileNotExists = fnCopyForceDir;
                       const SubFolders: Boolean = true; const ResetAttributes: Boolean = false): Integer;
...
function DetermineCopyFile(const FromFile, ToFile: String; const FileExists: TCopyFileExists = feCopy; const FileNotExists: TCopyFileNotExists = fnCopyForceDir): TCopyFileResult;
begin
  Result := cfNoNeed;
...
function PrepareCopyFile(const FromFile, ToFile: String; const FileExists: TCopyFileExists = feCopy; FileNotExists: TCopyFileNotExists = fnCopyForceDir): TCopyFileResult;
var
...
function cyCopyFileToDir(const SourceFile: string; ToDir: String; const FileExists: TCopyFileExists = feCopy; FileNotExists: TCopyFileNotExists = fnCopyForceDir; const ResetAttr: Boolean = false): TCopyFileResult;
begin
  if RightStr(ToDir, 1) <> '\' then
    ToDir := ToDir + '\';

  ToDir := ToDir + '\' + ExtractFilename(SourceFile);

  Result := cyCopyFile(SourceFile, ToDir, FileExists, FileNotExists, ResetAttr);
end;
...
begin
  ...
  then begin
    ...
    then
      FCurrentFileProcess := PrepareCopyFile(FFileSource, FFileDestination, FDestinationOptions.FIfFileExists, fnTmp);
