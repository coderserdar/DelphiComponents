...
uses Classes, Windows, Controls, Graphics, Grids, DBGrids, Messages, Db, jpeg, ExtCtrls, StdCtrls, ComCtrls, RichEdit, Forms, StrUtils,
      Dialogs, Themes, VCL.cyTypes, VCL.cyGraphics, cyModalContainer,
       {$IFDEF UNICODE}
...
type
  ...
  TFieldContentRendering = (fcDefault, fcWordwrap, fcPathEllipsis, fcEndEllipsis, fcWordEllipsis, fcMemo, fcBitmap, fcJpeg, fcIcon, fcPng, fcCheckBox, fcCheckBoxWithText);
  ...
  private
    ...
    FColor: TColor;
    FCanParse: Boolean;
    FAutoEnlargeColumn: Boolean;
  public
    constructor Create(AOwner: TComponent); virtual;
    property CanParse: Boolean read FCanParse write FCanParse;
  published
    property AutoEnlargeColumn: Boolean read FAutoEnlargeColumn write FAutoEnlargeColumn default false;
  ...
  TEnterKeyBehavior = (ekDefault, ekGotoRightCell, ekGotoDownCell);

  TcyNavigationOptions = class(TPersistent)
  private
    FBaseGrid: TcyBaseDBGrid;
    FAutoEnlargeColumn: Boolean;
    FEnterKeyBehavior: TEnterKeyBehavior;
  public
    constructor Create(AOwner: TComponent); virtual;
  published
    property AutoEnlargeColumn: Boolean read FAutoEnlargeColumn write FAutoEnlargeColumn default true;
    property EnterKeyBehavior: TEnterKeyBehavior read FEnterKeyBehavior write FEnterKeyBehavior default ekDefault;
  end;
  ...
  TcyContentCheckBoxMouseChangeMode = (ccChangeLeftClick, ccChangeDblClick);
  ...
  private
    ...
    FMargin: Word;
    FAlignLeft: Boolean;
    FMouseChangeMode: TcyContentCheckBoxMouseChangeMode;
    procedure SetSize(const Value: Word);
    procedure SetMargin(const Value: Word);
    procedure SetAlignLeft(const Value: Boolean);
  protected
  ...
  published
    ...
    property Margin: Word read FMargin write SetMargin default 1;
    property Size: Word read FSize write SetSize default 16;
    property MouseChangeMode: TcyContentCheckBoxMouseChangeMode read FMouseChangeMode write FMouseChangeMode default ccChangeLeftClick;
  end;
  ...
  private
    ...
    FNavigationOptions: TcyNavigationOptions;
  ...
  protected
    ...
    function GetFieldContentRendering(Column: TColumn): TFieldContentRendering; virtual;
    ...
    property NavigationOptions: TcyNavigationOptions read FNavigationOptions write FNavigationOptions;
    ...
    property MouseWheelAppend: Boolean read FMouseWheelAppend write FMouseWheelAppend default false;
  ...
  public
    ...
    procedure ClientColumnByFieldName(const ColumnFieldName: String; const MinWidth: Integer);
    procedure ClientColumnByIndex(ColumnIndex: Integer; const MinWidth: Integer);
    procedure AutoEnlargeColumn(const ACol: Integer; Column: TColumn; const ToContent: String);
    ...
    // Fields rendering :
    procedure DrawGraphicField(const Rect: TRect; Field: TBlobField; const FieldContent: TGraphicClass; const Transparent: Boolean; const bgStyle: TBgStyle; const Position: TBgPosition);
    procedure DrawMemoField(Rect: TRect; Field: TField; const TextWordwrap, BgTransparent: Boolean; const MaxCars, MaxLines: Integer);
    procedure DrawTextField(Rect: TRect; Field: TField; const FieldContentRendering: TFieldContentRendering);
    procedure DrawBooleanField(const Rect: TRect; Field: TField; const DrawTextField: Boolean);
    //
    function GetRecordPosition: TRecordPosition;
    function GotoRecordPosition(const RecordPosition: TRecordPosition): Boolean;
    procedure FreeRecordPosition(const RecordPosition: TRecordPosition);
    procedure TestSomething(Value: Integer);
    procedure DoKeyDown(var Key: Word; Shift: TShiftState);
    procedure DataSetNext(const AllowAppend: Boolean);
...
const
  ...
  function GetGraphicClass(const GraphicClassField: TGraphicClassField): TGraphicClass;
...
begin
  ...
  FAutoEnlargeColumn := false;
  FColor := clWindow;
  FParse := true;
end;

{TcyNavigationOptions}
constructor TcyNavigationOptions.Create(AOwner: TComponent);
begin
  FBaseGrid := TcyBaseDBGrid(AOwner);
  FAutoEnlargeColumn := true;
  FEnterKeyBehavior := ekDefault;
end;
...
function TcyClientColumn.GetColumn: TColumn;
var
  c: Integer;
begin
  Result := Nil;
  ...
  case Mode of
    cmByIndex:
      if FIndex in [0..FBaseGrid.Columns.Count-1] then
        Result := FBaseGrid.Columns[FIndex];

    cmByFieldName:
    begin
      for c := 0 to FBaseGrid.Columns.Count-1 do
        if AnsiUpperCase(FFieldName) = AnsiUpperCase(FBaseGrid.Columns[c].FieldName) then
        begin
          Result := FBaseGrid.Columns[c];
          Break;
        end;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FGrid.ClientColumn.Enabled then
...
//    ScrollBars := ssHorizontal;
  ...
  FNavigationOptions := TcyNavigationOptions.Create(self);
  FMoveRows := false;
...
procedure TcyBaseDBGrid.Loaded; // !!! Not called for Dynamic controls !!!
begin
  Inherited;
...
//      EndLayout;
  ...
  // 2016-10-24 :
  if not (ACol in [0..ColCount-1]) then
  begin
    ShowMessage('cyBaseDBGrid error in line: 1318');
    Exit;
  end;

  if (not (csDesigning in Self.ComponentState)) and (not (csLoading in Self.ComponentState)) then
    if FNavigationOptions.FAutoEnlargeColumn then
      if Focused then  // 2017-06-05 ... Avoid error on procedure ColWidthsChanged : 'inherited' will call .WMSize that will raise the error 'GRID INDEX OUT OF RANGE' !
    begin
      ColumnIndex := RawToDataColumn(ACol);

      if ColumnIndex in [0..Columns.Count-1] then
      begin
        ...
        begin
          ...
          if Assigned(FOnSetContentFieldRendering) then
            FOnSetContentFieldRendering(Self, ACol, ARow, Rect(0, 0, 0, 0), Column, [gdSelected], True, FieldContentRendering);

          if FieldContentRendering = fcDefault then
            AutoEnlargeColumn(ACol, Column, Column.Field.Text);
        end;
      end;
    end;
...
procedure TcyBaseDBGrid.SelectAllRows;
begin
...
procedure TcyBaseDBGrid.CheckAllRecords;
var
  i: Integer;
  DS: TDataset;
begin
  ...
  begin
    {$IFDEF UNICODE}
    FCheckedList.InsertBookmark(DS.GetBookmark);
    {$ELSE}
    FCheckedList.InsertBookmark(String(DS.GetBookmark));
    {$ENDIF}
...
begin
  ...
  begin
    ...
    // 2016-10-27 Enter key navigation :
    if Key = #13 then
      case FNavigationOptions.FEnterKeyBehavior of
        ekGotoRightCell:
        begin
          Key := #0;
          EditorMode := false;

          if (Col >= 0) and (ColCount <> 0) then
            if Col = ColCount-1 then
            begin
              DataSetNext(FAllowAppendRecord);

              if FirstDataColIndex <> 0 then
                Col := FirstDataColIndex;
            end
            else
              Col := Col + 1;

          // Also works :
          // keybd_event(VK_RIGHT, 0, 1, 0); // Right Key down ...
          // keybd_event(VK_RIGHT, 0, 2, 0); // Right Key up ...
        end;
        ...
        mwRowSelect:
          begin
            ...
            then begin
              ...
              DataSetNext(FAllowAppendRecord and FMouseWheelAppend);
          end;
...
procedure TcyBaseDBGrid.DataSetNext(const AllowAppend: Boolean);
begin
  with Datalink.Dataset do
  begin
    if (State = dsInsert) and (not Modified) and (not Datalink.Editing) then
      if DataLink.EOF
      then Exit
      else Cancel
    else
    ...
    if DataLink.EOF and CanModify and (not ReadOnly) and AllowAppend and (dgEditing in Options) then
      Append;
  end;
end;
...
begin
  ...
  if (Cell.X >= IndicatorOffset) and (Cell.Y >= 0) then
    ...
    begin
      ...
      // Set Boolean fields :
      if (ColumnIndex > -1) and (FContentFieldsRender.BooleanField in [bfCheckBox, bfCheckBoxWithText]) and (FContentCheckBoxOptions.FMouseChangeMode = ccChangeDblClick) then
        if Columns[ColumnIndex].Field is TBooleanField then
        begin
          EditorMode := false;

          // Don' t have X, Y ... if PointInContentCheckBox(CellRect(FMouseDownCell.X, FMouseDownCell.Y), X, Y) then
              CheckBoxFieldSwitchValue(Columns[ColumnIndex]);
        end;
...
(* 2016-05-05 OLD ...  !!! Columns[c].Width may not have real value !!!
  ...
  for c := 0 to ColCount-1 do
    if ColWidths[c] <> -1 then   // Hidden col ...
      Inc(Result, ColWidths[c] + ColInc);
end;

procedure TcyBaseDBGrid.ClientColumnByIndex(ColumnIndex: Integer; const MinWidth: Integer);
var
...
begin
  ...
  begin
    ...
    NewWidth := _ClientWidth - (ColsWidth - ColWidths[ColumnIndex + IndicatorOffset]);

    if NewWidth < MinWidth then
      NewWidth := MinWidth;
    // 2016-04-01 ... if Columns[ColumnIndex].Width <> NewWidth then
    if ColWidths[ColumnIndex + IndicatorOffset] <> NewWidth then
    begin
      // 2016-04-01 New code called 4 times instead of 7 !
      // Columns[ColumnIndex].Width := NewWidth; // !!! Not always working after loading columns attributes because Columns[c].Width may not have real value !!!
      ColWidths[ColumnIndex + IndicatorOffset] := NewWidth;
    end;
  end;
end;

procedure TcyBaseDBGrid.ClientColumnByFieldName(const ColumnFieldName: String; const MinWidth: Integer);
var i: Integer;
begin
  for i := 0 to Columns.Count-1 do
...
begin
  inherited;
  // 2016-05-05 STACK OVERFLOW - Changing colWidth in AdjustClientColumn will call again ColWidthsChanged !
...
procedure TcyBaseDBGrid.DrawGraphicField(const Rect: TRect; Field: TBlobField; const FieldContent: TGraphicClass; const Transparent: Boolean; const bgStyle: TBgStyle; const Position: TBgPosition);
var
  aGraphic: TGraphic;
...
procedure TcyBaseDBGrid.DrawMemoField(Rect: TRect; Field: TField; const TextWordwrap, BgTransparent: Boolean; const MaxCars, MaxLines: Integer);
var
...
begin
  ...
  InflateRect(Rect, -1, 0);      // 2016-07-14 because too close of left cell's border !
...
procedure TcyBaseDBGrid.DrawTextField(Rect: TRect; Field: TField; const FieldContentRendering: TFieldContentRendering);
const
  // 2016-10-06
  FieldContentRenderings : array [TFieldContentRendering] of Integer = (0, 0, DT_PATH_ELLIPSIS, DT_END_ELLIPSIS, DT_WORD_ELLIPSIS, 0, 0, 0, 0, 0, 0, 0);
begin
  Canvas.Brush.Style := bsClear;
  DrawText(Canvas.Handle, PChar(Field.AsString), Length(Field.AsString), Rect,
           WordWraps[FieldContentRendering = fcWordwrap] or dt_NoPrefix or FieldContentRenderings[FieldContentRendering]);
  Canvas.Brush.Style := bsSolid;
end;
...
begin
  ...
  if Result.ActiveRecord <> 0 then
    Result.Bookmark := DataLink.DataSet.GetBookmark;
end;

function TcyBaseDBGrid.GotoRecordPosition(const RecordPosition: TRecordPosition): Boolean;
var m, LastRow: Integer;
begin
  Result := false;
  if not DataLink.Active then Exit;

  // No record bookmarked !
  if RecordPosition.ActiveRecord = 0 then Exit;   // Avoid error
...
procedure TcyBaseDBGrid.FreeRecordPosition(const RecordPosition: TRecordPosition);
begin
  if not DataLink.Active then Exit;

  if RecordPosition.ActiveRecord <> 0 then
...
procedure TcyBaseDBGrid.DrawBooleanField(const Rect: TRect; Field: TField; const DrawTextField: Boolean);
var
  ...
  if (not Handled) and (ColumnIndex > -1) and (FContentFieldsRender.BooleanField in [bfCheckBox, bfCheckBoxWithText]) then
    ...
    begin
      ...
      begin
        ...
        if FContentCheckBoxOptions.FMouseChangeMode = ccChangeLeftClick then
          CheckBoxFieldSwitchValue(Columns[ColumnIndex]);
      end;
    end;
...
//    TCustomComboBox(FSavInplaceEdit).Style := csDropDownList;
  ...
  FEditorOptions.IsIntegerField := SelectedField.DataType in [ftSmallint, ftInteger, ftWord, ftLargeint{$IFDEF UNICODE}, ftShortint, ftByte{$ENDIF}];
  FEditorOptions.IsFloatField := SelectedField.DataType in [ftFloat, ftCurrency, ftBCD, ftFMTBcd];
...
function TcyBaseDBGrid.GetFieldContentRendering(Column: TColumn): TFieldContentRendering;
begin
  Result := fcDefault;
end;
...
var
  ...
  Column: TColumn;
  DoAutoEnlargeColumn: Boolean; // 2017-04-26
  c, ClientColumnIndex: Integer;
begin
  ModifiedValue := Value;
  ...
  // 2016-10-24
  if FEditorOptions.FAutoEnlargeColumn and (not EditorMode) then      // EditorMode is false after validate value ...
    try
      DoAutoEnlargeColumn := true;
      Column := Columns[ RawToDataColumn(ACol) ];

      // 2017-04-26
      if FClientColumn.Enabled then
      begin
        if FClientColumn.Mode = cmByFieldName then
        begin
          ClientColumnIndex := -1;

          for c := 0 to Columns.Count-1 do
            if AnsiUpperCase(FClientColumn.FFieldName) = AnsiUpperCase(Columns[c].FieldName) then
            begin
              ClientColumnIndex := c;
              Break;
            end;
          ...
          ClientColumnIndex := FClientColumn.FIndex;

        if ClientColumnIndex = Column.Index then
          DoAutoEnlargeColumn := false;
      end;

      if DoAutoEnlargeColumn then
        AutoEnlargeColumn(ACol, Column, ModifiedValue);
    finally
    end;
...
procedure TcyBaseDBGrid.AutoEnlargeColumn(const ACol: Integer; Column: TColumn; const ToContent: String);
var
  FieldValueWidth: Integer;
const
  OffsetX = 3; // Text does not start at pixel 0 ...
begin
  if not (ACol in [0..ColCount-1]) then
  begin
    ShowMessage('AutoEnlargeColumn error: check cyBaseDBGrid.pas line: 2855');
    Exit;
  end;
  ...
  if ColWidths[ACol] = -1 then Exit;  // Hidden col ...

  if (not (Column.Field is TGraphicField)) and (not (Column.Field is TMemoField)) and (not (Column.Field is TBlobField)) then
    if (FClientColumn.Column <> Column) or (not FClientColumn.FEnabled) then
    begin
      Canvas.Font.Assign(Column.Font);
      FieldValueWidth := Canvas.TextWidth(ToContent) + OffsetX;

      if FieldValueWidth < ClientWidth then
        if FieldValueWidth > ColWidths[ACol] then           // ColWidths[ACol] = -1 ...
        begin
          Columns.BeginUpdate;

          try
            ColWidths[ACol] := FieldValueWidth;   // 2017-06-05 ... Error on procedure ColWidthsChanged : 'inherited' will call .WMSize that will raise the error 'GRID INDEX OUT OF RANGE' !
          except
          end;
...
begin
  ...
  // Custom Indicator colwidth:
  if (dgIndicator in Options) and (ColCount > 0) then
  begin
    if FCustomLayoutOptions.FIndicatorColumnWidth <> 0 then
      ColWidths[0] := FCustomLayoutOptions.FIndicatorColumnWidth
    else
      if (FCheckBoxes.ColumnMode = cmIndicatorsCol) and (FCheckBoxes.Visible) then
        ColWidths[0] := FCheckBoxes.Size + FCheckBoxes.Margin * 2;
  end;

  // Custom Data RowHeight :
  if (FCustomLayoutOptions.FDataRowHeight <> 0) and (RowCount > 0) then
  begin
    if FCustomLayoutOptions.FTitleRowHeight = 0 then   // We need FCalculatedTitleRowHeight in order to avoid title height to be set with DefaultRowHeight
      FCalculatedTitleRowHeight := RowHeights[0];
  ...
  if (dgTitles in Options) and (RowCount > 0) then
    if FCustomLayoutOptions.FTitleRowHeight = 0 then
    begin
  ...
  // Custom Selected Data Row Height:
...
procedure TcyBaseDBGrid.DoKeyDown(var Key: Word; Shift: TShiftState);
begin
  if not DataLink.Active then Exit;
  ...
  begin
    Key := 0;
    DataLink.DataSet.MoveBy((-1) * Self.RowCount-1);
  end
  else
    if Key = VK_NEXT then // PAGE DOWN ...
    begin
      Key := 0;
      DataLink.DataSet.MoveBy(Self.RowCount-1);
    end
    else
      if Key = VK_DOWN then
      begin
...
begin
  ...
  // UpdateActive;
  // MoveColRow(Col, Row + 1, false, true);
  // LayoutChanged;
  // InvalidateEditor;
