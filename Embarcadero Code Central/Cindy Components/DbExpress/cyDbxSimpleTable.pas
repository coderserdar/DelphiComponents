...
uses Classes, sysUtils, Dialogs, DB, DBClient, Provider, SqlConst, {$IFDEF DELPHI2007_OR_ABOVE} DBXCommon, {$ENDIF} SimpleDS, SQLExpr,
      cyDBX, cyDbxBaseTable, cyDbxReconcileError, cyDB;
...
type
  ...
  private
    ...
    FImportPropertiesFrom: TDataset;
    function GetDbxConnectionDrv: TDbxConnectionDriver;
    procedure SetImportPropertiesFrom(const Value: TDataset);
  protected
  ...
  public
    ...
    property DbxConnectionDriver: TDbxConnectionDriver read GetDbxConnectionDrv;
  protected
    ...
    property ImportPropertiesFrom: TDataset read FImportPropertiesFrom write SetImportPropertiesFrom;
...
function cyDbxSimpleTableCreate(aOwner: TComponent; Connection: TSQLConnection; const CommandText, IndexFieldNames: string; const DeactivateAutoRefresh: Boolean = true; const ParamCheck: Boolean = false): TcyDbxSimpleTable;

implementation

function cyDbxSimpleTableCreate(aOwner: TComponent; Connection: TSQLConnection; const CommandText, IndexFieldNames: string; const DeactivateAutoRefresh: Boolean = true; const ParamCheck: Boolean = false): TcyDbxSimpleTable;
begin
  Result := TcyDbxSimpleTable.Create(aOwner);

  Result.FDataSet.ParamCheck := ParamCheck;
  Result.Connection := Connection;

  if DeactivateAutoRefresh then
  begin
    Result.AutoRefreshOptions.AfterDelete := false;
    Result.AutoRefreshOptions.AfterPostModifiedRecord := rfNone;
    Result.AutoRefreshOptions.AfterPostNewRecord := rfNone;
    Result.AutoRefreshOptions.MovingToFirst := false;
    Result.AutoRefreshOptions.MovingToLast := false;
  end;

  Result.DataSet.CommandText := CommandText;
...
begin
  ...
  FImportPropertiesFrom := Nil;
  AllocProvider;
  AllocDataSet;
end;

function TcySimpleDataset.GetDbxConnectionDrv: TDbxConnectionDriver;
begin
  Result := cyDBX.GetDbxConnectionDriver(FConnection);
end;
...
begin
  ...
  if FImportPropertiesFrom <> nil then
    if (Operation = opRemove) and (AComponent = FImportPropertiesFrom) then
      FImportPropertiesFrom := nil;
...
begin
  if Assigned(FConnection) then     // 2014-05-29  Let use LoadFromFile() ...
  begin
...
procedure TcySimpleDataset.SetImportPropertiesFrom(const Value: TDataset);

    function AddIdentifiersToFields(const Fields: string): string;
    var
      i: Integer;
    begin
      Result := cyDBX.SQLIdentifier(Fields, DbxConnectionDriver);
      Result := StringReplace(Result, ', ', cyDBX.SQLIdentifier(', ', DbxConnectionDriver), [rfReplaceAll]);
    end;

var
  RsltFields: string;
begin
  if Assigned(FImportPropertiesFrom) then
    FImportPropertiesFrom.RemoveFreeNotification(Self);

  FImportPropertiesFrom := Value;
  ...
  begin
    ...
    cyDB.CopyDatasetProperties(Value, Self, RsltFields);

    if csDesigning in ComponentState then
      if not Self.Active then
        if Self.DataSet.CommandText = '' then
          Self.DataSet.CommandText := 'SELECT ' + AddIdentifiersToFields(RsltFields);
  end;
end;
...
procedure TcyDbxSimpleTable.Loaded;   // !!! Not called for Dynamic controls !!!
begin
  inherited;
end;
...
var
  ...
  RsltAutoIncField: Boolean;
begin
  // Adjust providerFlags on Dataset:
  if not (poReadOnly in FProvider.Options) then
    if not (csDesigning in ComponentState) then
      ...
      begin
        ...
        if TableName <> '' then
        begin
          // mySQL Get table PRIMARY Key :
          if GetDbxConnectionDrv = cdMySQL then
          begin
            aQuery := TSQLQuery.Create(Nil);
            aQuery.SQLConnection := Connection;
            aQuery.SQL.Clear;
            aQuery.SQL.Text := 'DESCRIBE ' + TableName;      // Cannot not use WHERE Clause in result field `Key` ...
            aQuery.Active := true;
          ...
          if GetDbxConnectionDrv in [cdMSSQL, cdMSSQL9] then
          begin
            // PKName := SQLGetPrimaryKey(FConnection, TableName);
...
begin
  FDbxReconcileError.Execute(Self, DataSet, E, UpdateKind, Action);
...
begin
  ...
  if not CachedUpdates then
    if Assigned(FConnection) then     // 2014-05-29
        ...
        PacketRecords := -1;
          ...
          if FAutoRefreshOptions.MovingToFirst then
            if State = dsBrowse then     // Can be inserting new record
              if ChangeCount = 0 then    // 2014-10-10
              begin
                cyRefreshQuery;
                InternalRefresh;
              end;
...
begin
  ...
  // Apply updates :
  if (not CachedUpdates) and (Assigned(FConnection)) then    // 2014-05-29
    if ChangeCount <> 0 then
    begin
      FLastErrorCount := ApplyUpdates(FUpdatesOptions.MaxErrors);
...
var
  aDatabaseName, aTableName, aPkFieldName, aPrimaryKeyValue: String;
begin
  Result := '';
  aPrimaryKeyValue := '';
  ...
  if aTableName <> '' then
  begin
    aPkFieldName := SQLGetPrimaryKey(FConnection, aTableName);

    if FindField(aPkFieldName) <> Nil then
    begin
      aPrimaryKeyValue := FieldByName(aPkFieldName).AsString;
...
function TcyDbxSimpleTable.LockRecord: Boolean;
begin
  Result := false;
  ...
  begin
    ...
    FQLock.SQL.Text := 'SELECT GET_LOCK(' + cyDBX.SQLGetStringExpr(FReleaseLockId, false, 0, GetDbxConnectionDrv) + ',' + intToStr(FRecordLockOptions.WaitSeconds) + ')';
    FQLock.Active := true;
...
procedure TcyDbxSimpleTable.UnlockRecord;

    function InternalLock: Boolean;
    begin
      Result := false;

      if Assigned(FQLock) then
        if FQLock.Active then
          Result := true;
    end;

begin
  try
    if InternalLock then       // locked by this cursor ...
    begin
      FQLock.Active := false;   // No need to execute RELEASE_LOCK() because closing cursor will release the lock ...
      FQLock := Nil;
    end
    else begin
      // RHR Don' t work because lock was made by another connection ...
      FConnection.ExecuteDirect('SELECT RELEASE_LOCK(' + cyDBX.SQLGetStringExpr(GetInternalRecordLockId, false, 0, GetDbxConnectionDrv) + ')');
    end;
  finally
