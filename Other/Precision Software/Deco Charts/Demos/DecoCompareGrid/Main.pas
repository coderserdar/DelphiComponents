...
uses
  ...
  Dialogs, ExtCtrls, ImgList, StdCtrls, DecoChart, DecoCompareGrid, ComCtrls;
...
begin
  ...
  begin
    if Value<>nil then
    begin
      {if (Value=compTable.SelectedCell) or (Value=compTable.HoverCell) then
      begin
        FR:=CellRect;
        TargetCanvas.Brush.Style:=bsSolid;
        if Value=compTable.SelectedCell then
          TargetCanvas.Brush.Color:=clSkyBlue
        else
          TargetCanvas.Brush.Color:=clInfoBk;
        Dec(FR.Bottom); Dec(FR.Right);
        TargetCanvas.FillRect(FR);
        TargetCanvas.Brush.Style:=bsClear;
      end;}
