...
uses
  ...
  Dialogs, ExtCtrls, TagCloud, StdCtrls, ComCtrls, jpeg, pngimage;
...
type
  TfrmMain = class(TForm)
    ...
    procedure cbVistaGlassClick(Sender: TObject);
    ...
    procedure cbAutoShrinkRowsClick(Sender: TObject);
...
begin
  ...
  try
    for i:=statusBar.Panels.Count-2 downto 2 do
      statusBar.Panels.Delete(i);
    for i:=tagCloud.PageCount downto 1 do
    begin
      P:=statusBar.Panels.Insert(2);
      P.Text:=IntToStr(i);
      P.Alignment:=taCenter;
      P.Width:=32;
      if tagCloud.PageIndex=i-1 then
        P.Bevel:=pbLowered
      else
        P.Bevel:=pbNone;
    end;
...
begin
  ...
  for i:=0 To statusBar.Panels.Count-2 do
    if (p.x>=x) and (p.x<=x+statusBar.Panels[i].Width) then
    begin
      if i>=2 then
      begin
        statusBar.Panels[tagCloud.PageIndex+2].Bevel:=pbNone;
        statusBar.Panels[i].Bevel:=pbLowered;
        tagCloud.PageIndex:=i-2;
      end;
      break;
    end
    else
...
procedure TfrmMain.cbAutoShrinkRowsClick(Sender: TObject);
begin
  if not FUpdatingInfo then
...
begin
  ...
  begin
    ...
    try
      ...
      if cbClr1.Checked then tagCloud.Colors.Add.Color:=clrbx1.Selected;
      if cbClr2.Checked then tagCloud.Colors.Add.Color:=clrbx2.Selected;
      if cbClr3.Checked then tagCloud.Colors.Add.Color:=clrbx3.Selected;
      if cbClr4.Checked then tagCloud.Colors.Add.Color:=clrbx4.Selected;
    finally
...
procedure TfrmMain.cbTranspClick(Sender: TObject);
begin
  if not FUpdatingInfo then
  begin
    if cbTransp.Checked then
    begin
      tagCloud.Transparent:=True;
      imgBack.Visible:=True;
      imgLogo.Visible:=True;
    end
    else
    begin
      tagCloud.Transparent:=False;
      imgBack.Visible:=False;
      imgLogo.Visible:=False;
      if cbVistaGlass.Checked then
      begin
        cbVistaGlass.Checked:=False;
        cbVistaGlassClick(cbVistaGlass);
      end;
    end;
  end;
end;

procedure TfrmMain.cbVistaGlassClick(Sender: TObject);
begin
  FUpdatingInfo:=True;
  imgBack.Visible:=False;
  imgLogo.Visible:=False;
...
begin
  tagCloud.Filter:=edSearch.Text;
end;

procedure TfrmMain.edValueChange(Sender: TObject);
begin
  if (not FUpdatingInfo) and Assigned(tagCloud.SelectedItem) then
  begin
    if Sender=edCaption then
      tagCloud.SelectedItem.Caption:=edCaption.text
    else
    if Sender=edHint then
      tagCloud.SelectedItem.Hint:=edHint.text
    else
    if Sender=edTag then
      tagCloud.SelectedItem.Tag:=StrToIntDef(edHint.Text,0)
    else
    if Sender=edValue then
      tagCloud.SelectedItem.Value:=StrToTagCloudValue(edValue.Text);
  end;
end;
...
procedure TfrmMain.sbClearFilterClick(Sender: TObject);
begin
  edSearch.Text:='';
end;
...
begin
  if Assigned(tagCloud.SelectedItem) then
  begin
    tagCloud.Items.Delete(tagCloud.SelectedItem.Index);
    tagCloudTagClick(nil,nil);
  end;
end;
