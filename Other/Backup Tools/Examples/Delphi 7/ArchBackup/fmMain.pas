...
type
  TMain = class(TForm)
    ...
    procedure btnViewContentClick(Sender: TObject);
...
implementation
uses FileCtrl, fmInspect, fmOptions, fmViewContent;
{$R *.DFM}
...
procedure TMain.btnViewContentClick(Sender: TObject);
begin
  if not FileExists(edFileName.Text) then
    begin
      MessageDlg( Format( 'Archive %s does not exist !', [edFileName.Text]), mtError, [mbOk], 0 );
  ...
  with Archiver1 do
    begin
      ...
      try
        ...
        with ViewContent do
        begin
          Caption := Format('View content of %s', [FileName]);
          ShowModal;
        end;
      finally
        Close;
...
procedure TMain.Archiver1Enumeration(Sender: TObject;
  const FileEntry: TFileEntry);
begin
  with ViewContent.lbFiles.Items do
  begin
    Add(Format('%s (%d)', [FileEntry.Name, FileEntry.ArchiveInfo.Size]));
  end;
end;

procedure TMain.Archiver1ClearFileList(Sender: TObject);
begin
  ViewContent.lbFiles.Items.Clear;
end;
