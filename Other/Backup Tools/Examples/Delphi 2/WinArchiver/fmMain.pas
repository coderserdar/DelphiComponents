...
type
  ...
  TMain = class(TForm)
    ...
    tbOpen: TSpeedButton;
    tbAdd: TSpeedButton;
    tbExtract: TSpeedButton;
    tbDelete: TSpeedButton;
    tbView: TSpeedButton;
    tbInstall: TSpeedButton;
  ...
  private
    ...
    function  IsFileSelected( const FileEntry : TFileEntry ) : Boolean;
...
begin
  ...
  if (GetTickCount - gCounter) >= 333 then
    begin
...
function TMain.IsFileSelected( const FileEntry : TFileEntry ) : Boolean;

  function IsEqual( const entry1, entry2 : TFileEntry ) : Boolean;
  begin
    Result := (entry1.Name = entry2.Name) and
              (entry1.Date = entry2.Date) and
              (entry1.ArchiveInfo.Size = entry2.ArchiveInfo.Size);
  end;

  function IsSelected( const FileEntry : TFileEntry; idx : Integer ) : Boolean;
  begin
    Result := IsEqual( Archiver1.Files[ Integer(lbFiles.Items.Objects[idx]) ].FileEntry, FileEntry );
  end;
...
begin
  ...
  0: // Selected files
    begin
      Accept := IsFileSelected(FileEntry);
    end;
  2: // pattern
    begin
      ...
      for i := 0 to FileCount - 1 do
        begin
          if FileCount > 1 then
            ...
            iPercent := 0;
          if (PanelProgress.Visible = False) and (GetTickCount-start > 1000) and
             (percent < 50) then
            begin
              SetProgressBar( True );
...
begin
  ...
  if lbFiles.Items.Count > 1 then
    ...
    percent := 0;
  if (PanelProgress.Visible = False) and (GetTickCount-FStart > 1000) and
     (percent < 50) then
    begin
      SetProgressBar( True );
      btnAbort.Enabled := False;
