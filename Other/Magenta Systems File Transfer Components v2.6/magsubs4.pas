unit magsubs4; // Delphi 6 and later version
{$IFNDEF VER140}
...
Updated by Angus Robertson, Magenta Systems Ltd, England, 6th October 2008
...
uses
  ...
  ExtCtrls, Graphics, Math ;

const
 { important registry keys / items }
  REG_CURRENT_VERSION95 = 'Software\Microsoft\Windows\CurrentVersion';
  REG_CURRENT_VERSIONNT = 'Software\Microsoft\Windows NT\CurrentVersion';
...
function ConvertIPAddr (Addr: u_long): string ;
...
function IpToStr (Addr: u_long): string ;
function StrToIP (strIP: string): u_long ;
function IsIPStr (strIP: string): boolean ;
function IsFmtIPStr (var strIP: string): boolean ;
function Str2IP (strIP: string; var IPAddr: u_long): boolean ;
...
begin
  ...
  path := path + '\' + Value ;
  Result := regReadString (HKEY_LOCAL_MACHINE, path) ;  // 8 Aug 2002, was regGetString, but only admin on NT
end;
...
begin
    ...
    temp :=  Int2StrZ (length (temp), 2) + temp ;  // add length to start
...
begin
   result := '' ;
    try
        ...
        keylen := AscToInt (copy (temp, 1, 2)) ;  // get length from start
...
begin
    ...
    temp := strREncrypt (S, Key) ;
    temp :=  Int2StrZ (length (temp), 3) + temp ;  // add length to start
    result := base64encode (temp) ;
end ;
...
begin
   result := '' ;
    try
        ...
        keylen :=  AscToInt (copy (temp, 1, 3)) ;  // get length from start
        if keylen = 0 then exit ;
...
// found a valid IP address, keep it
    Move (MyIPAddr, IPAddr, SizeOf(u_long)) ;
    result := true ;
end ;
...
//                loghandle := FileCreate (FFullName, fmOpenReadWrite OR fmShareDenyWrite) ;
                ...
                begin
                    if (Pos ('"', FHeader) > 0) and (Pos ('","', FHeader) = 0) then
                        logline := FormatDateTime (FHeader, Now)
                    else
                        logline := FHeader ;
...
begin
    Result := False;
    ErrInfo := '' ;
    if (Length (FilePath) <= 2) or ((CharPos ('\', FilePath) <= 0) and
                                     (CharPos (':', FilePath) <= 0)) then
    begin
...
var
    KeyHandle: HKEY;
    ErrCode, Index, I, J: Integer;
    ValueName, Data, SortStr: string;
        ...
        repeat
            ...
            begin
                ...
                begin
                    ...
                    SortStr := 'xxx' ;
                SortStr := SortStr + Data + '|' + ValueName ;
