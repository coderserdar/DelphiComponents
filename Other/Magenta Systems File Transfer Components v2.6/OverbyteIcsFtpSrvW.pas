...
const
    ...
    CopyRight : String       = ' TFtpServerW (c) 1998-2009 F. Piette V7.08 ';
...
{$IFDEF USE_SSL}
    if Self is TSslFtpServerW then begin     {  V1.48 }
        if ftpImplicitSsl in TSslFtpServerW(Self).FFtpSslTypes then   { V1.47 }
...
begin
    ...
    {$IFDEF USE_SSL}
        if Self is TSslFtpServerW then begin     {  V1.48 }
        if TSslFtpServerW(Self).FFtpSslTypes <> [] then begin             { V1.47 }
                if not (ftpImplicitSsl in TSslFtpServerW(Self).FFtpSslTypes) then begin
                Answer := Answer + ' AUTH ';
                if ftpAuthTls in TSslFtpServerW(Self).FFtpSslTypes then
                    Answer := Answer + 'TLS;';
                if ftpAuthSsl in TSslFtpServerW(Self).FFtpSslTypes then
                    Answer := Answer + 'SSL;';
                if ftpAuthTlsP in TSslFtpServerW(Self).FFtpSslTypes then
                    Answer := Answer + 'TLS-P;';
                if ftpAuthTlsC in TSslFtpServerW(Self).FFtpSslTypes then
                    Answer := Answer + 'TLS-C;';
                Answer := Answer +  #13#10 +
                          ' CCC'+ #13#10;
            {if TSslFtpServerW(Self).FFtpSslType = sslTypeAuthSsl then
       ...
       (AThread.Client.ID = AThread.ClientID) then begin
        ...
        if (AThread.Keyword = 'MD5') or (AThread.Keyword = 'XMD5') then begin { angus V1.54 }
           ...
           { angus V1.54 }
            ...
            Answer := WideFormat('500 Executing command %s failed', [AThread.Keyword]);
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpCtrlSocketW.SetCodePage(const Value: Cardinal);{ AG 7.02 }
begin
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TFtpCtrlSocketW.SetCurrentCodePage(const Value: Cardinal);{ AG 7.02 }
begin
    if Value = FtpServer.FSystemCodePage then
...
begin
    ...
    if Client.State <> wsConnected then
        Exit;
    Client.SslEnable  := ftpImplicitSsl in TSslFtpServerW(Self).FFtpSslTypes;
