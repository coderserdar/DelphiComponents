...
uses
  ...
  MagentaHtmlPars, MagentaCopy, MagSubs1, MagClasses,
  OverbyteIcsHttpProt, OverbyteIcsLibrary, OverbyteIcsWSocket,
  OverbyteIcsUrl, OverbyteIcsFtpSrvT
...
// build sorted list of source records, then remove duplicates
    ...
    begin
        if PTFileRec (SrcFileList [I]).FrFullName =
                    PTFileRec (SrcFileList [Pred (I)]).FrFullName then
...
// show user the list we found
    if fLogRDir then doCopyEvent (LogLevelInfo, CRLF +
           'Source HTTP Files:' + CRLF + FmtFileDirList (SrcFileList, false)) ;
    Application.ProcessMessages ;
    if fCancelFlag then exit ;
...
// find size of stuff to copy
    info := CRLF + 'HTTP URLs Selected for Downloading are: ' + CRLF ;
    newsize := 0 ;
        ...
        begin
            ...
            begin
                ...
                if CheckFiles then info := info + FrFullName +
                            ', Size ' + IntToCStr (FrFileBytes) + CRLF ;
            end ;
        end ;
    end ;
...
// start real HTTP downloading
    ...
    begin
        ...
        begin
            ...
            fProgMessBase := fProgMessBase + CRLF + 'URL: ' + curURL ;
