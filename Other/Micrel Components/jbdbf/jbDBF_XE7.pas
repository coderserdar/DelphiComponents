...
{                                                       }
{       Downloaded from:                                }
{       https://torry.net/db/direct/db_dbf/jbdbf.zip    }
{                                                       }
{       Modified for XE7 by Peter Miglinski             }
{          2019                                         }
{                                                       }
{*******************************************************}
...
Older history:
  21.06.2019 XE7 compatibility, Fixed System locale bug.
...
begin
  ...
  {$IFDEF UNICODE}
  SetCodePage(tmprec, FLangCodePage, True);
  {$ENDIF}
  ...
  if IsNotNull then
    ...
    begin
      ...
      if FDBFieldRec.places > 0 then
        Insert({$IFDEF VER15UP}TDBFields(FOwner).FormatSettings.{$ENDIF}DecimalSeparator, S, Length(S) - FDBFieldRec.places);
      Result := StrToFloat(S);
    end;
end;
...
begin
  if FDBFieldRec.what in ['F', 'N', 'C'] then
    TjbDBF(FOwner.Owner).InternalSave(FDBFieldRec.name,
         Format('%'+IntToStr(FDBFieldRec.len)+'.' + IntToStr(FDBFieldRec.places) + 'f',
                [Value]{$IFDEF VER15UP}, TDBFields(FOwner).FormatSettings{$ENDIF}));
end;
...
begin
  case FDBFieldRec.what of
    'F': TjbDBF(FOwner.Owner).InternalSave(FDBFieldRec.name,
              Format('%'+IntToStr(FDBFieldRec.len)+'.' + IntToStr(FDBFieldRec.places) + 'f',
                     [Value]{$IFDEF VER15UP}, TDBFields(FOwner).FormatSettings{$ENDIF}));
...
begin
  ...
  FillChar(FDBFieldRec.name, SizeOf(FDBFieldRec.name), SpacerZ);
...
var
  ...
  I: Integer;
begin
  FillChar(FDBFieldRec.idx, SizeOf(FDBFieldRec.idx), SpacerZ);
