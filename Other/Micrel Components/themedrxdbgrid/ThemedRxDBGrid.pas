...
{
    ThemedRxDBGrid
    When this unit is added to a project, all TRxDBGrid components are painted
    ...
    Inherited from ThemedDBGrid unit code with some additions by Andreas Hausladen.
    ...
    History:
      Version 2010-11-04
        - Adapted to RxLibrary - RxDBGrid by Jaro B.
      Version 2009-01-08:
...
uses
  ...
  RxDBCtrl, DBGrids, DB, ImgList, Themes;
...
type
  ...
  TRxDBGrid = class(RxDBCtrl.TRxDBGrid) // keep TDBGrid.ClassName = 'TDBGrid'
  private
...
function TRxDBGrid.BeginColumnDrag(var Origin, Destination: Integer; const MousePt: TPoint): Boolean;
begin
  Result := inherited BeginColumnDrag(Origin, Destination, MousePt);
  FPaintInfo.ColMoving := Result;
end;

function TRxDBGrid.CentreV(ARect: TRect; ATextHeight: Integer): Integer;
begin
  Result := ((ARect.Bottom - ARect.Top) - ATextHeight) div 2;
end;

function TRxDBGrid.CentreH(ARect: TRect; ATextWidth: Integer): Integer;
begin
  Result := ((ARect.Right - ARect.Left) - ATextWidth) div 2;
end;

procedure TRxDBGrid.CMMouseEnter(var Message: TMessage);
var
  Cell: TGridCoord;
  lPt: TPoint;
begin
...
procedure TRxDBGrid.CMMouseLeave(var Message: TMessage);
begin
...
procedure TRxDBGrid.ColumnMoved(FromIndex, ToIndex: Integer);
begin
...
function TRxDBGrid.ColumnOffset(AOptions: TDBGridOptions): Integer;
begin
...
function TRxDBGrid.RowIsMultiSelected: Boolean;
var
  Index: Integer;
begin
...
function TRxDBGrid.DoMouseWheel(Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint): Boolean;
begin
...
procedure TRxDBGrid.TitleClick(Column: TColumn);
begin
...
function TRxDBGrid.TitleOffset(AOptions: TDBGridOptions): Integer;
begin
...
function TRxDBGrid.ValidCell(ACell: TGridCoord): Boolean;
begin
  Result := (ACell.X <> -1) and (ACell.Y <> -1);
end;

{$IF CompilerVersion >= 18.0}
procedure TRxDBGrid.WMPaint(var Message: TWMPaint);
var
  R: TRect;
begin
...
procedure TRxDBGrid.WMEraseBkgnd(var Message: TWMEraseBkgnd);
var
  R: TRect;
  Size: TSize;
begin
...
initialization
  OrgTDBGrid_NewInstance := GetVirtualMethod(TRxDBGrid, vmtNewInstance);
  SetVirtualMethod(RxDBCtrl.TRxDBGrid, vmtNewInstance, @TDBGrid_NewInstance);

finalization
  SetVirtualMethod(RxDBCtrl.TRxDBGrid, vmtNewInstance, OrgTDBGrid_NewInstance);
