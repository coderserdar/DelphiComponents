...
function FindWord(what, S: string; WordDelims: CharSet {$IFNDEF CLR}= [',']{$ENDIF}): Boolean; {$IFNDEF CLR}{$IFDEF SUPP_INL}inline;{$ENDIF}{$ENDIF}
...
function RemLastChar(const DirName: string; const WhenIs: Char {$IFNDEF CLR}= #0{$ENDIF}): string; {$IFDEF SUPP_INL}inline;{$ENDIF}
...
function StripChars(const S: string; ch: CharSet): string; {$IFDEF SUPP_INL}inline;{$ENDIF}
function StripNoChars(const S: string; ch: CharSet): string; {$IFDEF SUPP_INL}inline;{$ENDIF}
...
var
  I, J: Integer;
  C: {$IFDEF CLR}Char{$ELSE}AnsiChar{$ENDIF};
begin
  Result := False;
...
function RemLastChar(const DirName: string; const WhenIs: Char {$IFNDEF CLR}= #0{$ENDIF}): string;
  {-ubere \ ke jmenu adresare}
begin
  Result := DirName;
  if Length(DirName) > 1 then
  begin
    if WhenIs <> #0 then
      if DirName[Length(DirName)] <> WhenIs then Exit;
...
function StripChars(const S: string; ch: CharSet): string;
 {-vytahne z retezce jen pozadovane znaky}
var I: Integer;
begin
  Result := '';
  for I := 1 to Length(S) do
...
function StripNoChars(const S: string; ch: CharSet): string;
 {-vytahne z retezce vse bez vyznacenych znaku}
var I: Integer;
begin
  Result := '';
  for I := 1 to Length(S) do
