...
{}
{PBMaskEdit is a standard Delphi Edit component with Alignment, DisabledColor
...
constructor TPBMaskEdit.Create(AOwner: TComponent);
begin
...
procedure TPBMaskEdit.CreateParams(var Params: TCreateParams);
const
...
procedure TPBMaskEdit.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
...
procedure TPBMaskEdit.KeyDown(var Key: Word; Shift: TShiftState);
begin
	inherited KeyDown(Key, Shift);
	FEnter := False;
end;

procedure TPBMaskEdit.KeyPress(var Key: Char);
begin
...
procedure TPBMaskEdit.SetAlignment(Value: TAlignment);
var
	SelSt, SelLe : integer;
begin
...
procedure TPBMaskEdit.SetColor(Value : TColor);
begin
...
procedure TPBMaskEdit.SetDisabledColor(Value : TColor);
begin
...
procedure TPBMaskEdit.SetEnabled(Value : Boolean);
begin
	inherited;
	if Enabled then inherited Color := FEnabledColor
	else inherited Color := FDisabledColor;
...
procedure TPBMaskEdit.SetParentColor(Value : Boolean);
begin
...
procedure TPBMaskEdit.SetVersion(Value: String);
begin
	{ Read only! }
end;

procedure TPBMaskEdit.WMGetDlgCode(var Msg: TWMGetDlgCode);
begin
	Msg.Result := DLGC_WANTCHARS or DLGC_WANTARROWS;
end;

procedure TPBMaskEdit.CMPARENTCOLORCHANGED(var M:TMessage);
begin
	if FParentColor and (Parent <> nil) then FEnabledColor := Parent.Brush.Color;
...
procedure Register;
begin
	RegisterComponents('PBEdit', [TPBMaskEdit]);
end;
