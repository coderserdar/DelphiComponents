...
uses
	SysUtils, Classes, ukcyConsts;

{
	type declarations for use with block cyphers
	--------------------------------------------
}
...
type
	TKBCRetCode = Integer;  // return codes for ukCrypto calls

{ returns the size needed to encipher or decipher a given buffer,
	for a given pair of block and key sizes; to be used with
	TKBCEncipherBufferProc and TKBCDecipherBufferProc types of
	functions in order to guarantee enough room for the output buffers }
	TKBCOutputSizeProc = function( szBuffer, szBlock, szKey: LongInt ): LongInt;

{ enciphers the plaintext message in sIn putting the corresponding
	ciphertext in sOut, using the key and block information passed; all
	padding and size calculations will be carried out automatically; this
	function will return a Block Cipher Return Code with the given
	success level for the operation }
	TKBCEncipherStreamProc = function( sIn, sOut: TStream; Key: Pointer; szBlock, szKey: LongInt ): TKBCRetCode;

{ deciphers the ciphertext message in sIn putting the corresponding
	plaintext in sOut, using the key and block information passed; all
	padding and size calculations will be carried out automatically; this
	function will return a Block Cipher Return Code with the given
	success level for the operation }
	TKBCDecipherStreamProc = function( sIn, sOut: TStream; Key: Pointer; szBlock, szKey: LongInt ): TKBCRetCode;

{ enciphers exactly like TKBCEncipherStreamProc, except that the
	stream sInOut will be used both for reading the plaintext and
	generating the ciphertext message; this function will return a Block
	Cipher Return Code with the given success level for the operation }
	TKBCEncipherStreamExProc = function( sInOut: TStream; Key: Pointer; szBlock, szKey: LongInt ): TKBCRetCode;

{ deciphers exactly like TKBCDecipherStreamProc, except that the
	stream sInOut will be used both for reading the ciphertext and
	generating the plaintext message; this function will return a Block
	Cipher Return Code with the given success level for the operation }
	TKBCDecipherStreamExProc = function( sInOut: TStream; Key: Pointer; szBlock, szKey: LongInt ): TKBCRetCode;

{ enciphers the plaintext message in bIn putting the corresponding
	ciphertext in bOut, using the key and block information passed; all
	padding and size calculations will be carried out; the function
	will use szInBuffer as the size of the input buffer, and will
	return in szOutBuffer the number of bytes actually written to bOut;
	this function expects bOut to have enough room for the ciphertext
	message; the size of the expected bOut buffer can be obtained by
	calling the appropriate TKBCOutputSizeProc function; this function
	will return a Block Cipher Return Code with the given success level
	for the	operation }
	TKBCEncipherBufferProc = function( const bIn; var bOut; Key: Pointer; szInBuffer, szBlock, szKey: LongInt; var szOutBuffer: LongInt ): TKBCRetCode;

{ deciphers the ciphertext message in bIn putting the corresponding
	plaintext in bOut, using the key and block information passed; all
	padding and size calculations will be carried out; the function
	will use szInBuffer to as the size of the input buffer, and will
	return in szOutBuffer the number of bytes that actually correspond
	to the original plaintext message (might be different than the
	number of bytes written to bOut due to padding operations);	this
	function expects bOut to have enough room for the plaintext	message
	and extra padding; the size of the expected bOut buffer can be
	obtained by	calling the appropriate TKBCOutputSizeProc function; this
	function will return a Block Cipher Return Code with the given success
	level for the	operation }
	TKBCDecipherBufferProc = function( const bIn; var bOut; Key: Pointer; szInBuffer, szBlock, szKey: LongInt; var szOutBuffer: LongInt ): TKBCRetCode;

{ enciphers the plaintext message in bIn putting the corresponding
	ciphertext in bOut, using the key and block information passed; no
	padding and size calculations will be carried out; if szBlock is not
	a multiple of szKey, the return code will be BC_ERROR and no
	encryption will be carried out; the function will use szInBuffer
	as the size of the input buffer, and will	return in szOutBuffer
	the number of bytes actually written to bOut;	this function expects
	bOut to have enough room for the ciphertext	message; the size of the
	expected bOut buffer can be obtained by	calling the appropriate
	TKBCOutputSizeProc function; this function will return a Block
	Cipher Return Code with the given success level	for the	operation }
	TKBCEncipherBufferExProc = function( const bIn; var bOut; Key: Pointer; szInBuffer, szBlock, szKey: LongInt; var szOutBuffer: LongInt ): TKBCRetCode;

{ deciphers the ciphertext message in bIn putting the corresponding
	plaintext in bOut, using the key and block information passed; no
	padding and size calculations will be carried out; if szBlock is not
	a multiple of szKey, the return code will be BC_ERROR and no
	decryption will be carried out; the function will use szInBuffer
	as the size of the input buffer, and will return in szOutBuffer
	the number of bytes that actually correspond to the original
	plaintext message (might be different than the number of bytes
	written to bOut due to padding operations);	this function expects
	bOut to have enough room for the plaintext message; the size of
	the expected bOut buffer can be obtained by	calling the appropriate
	TKBCOutputSizeProc function; this function will return a Block Cipher
	Return Code with the given success level for the operation }
	TKBCDecipherBufferExProc = function( const bIn; var bOut; Key: Pointer; szInBuffer, szBlock, szKey: LongInt; var szOutBuffer: LongInt ): TKBCRetCode;

{ Blowfish definitions }


type
	TKBlowFishIterations = BF_MIN_ITERATIONS..BF_MAX_ITERATIONS;
	TKBlowFishKeySize = 1..BF_MAX_KEY_BLOCK_BYTES;

	TKBlowFishSBox = array[0..255] of {$IFDEF DELPHI4}Cardinal{$ELSE}LongInt{$ENDIF};

	PKBlowFishControl = ^TKBlowFishControl;
	TKBlowFishControl = record
		PArray: array[0..17] of {$IFDEF DELPHI4}Cardinal{$ELSE}LongInt{$ENDIF};
			SBox: array[0..03] of TKBlowFishSBox;
	end;

  {Array of byte to create a BlowFish Pointer Key with any another data type}
	PBFKey = ^TBFKey;
	TBFKey = array[0..BF_MAX_KEY_BLOCK_BYTES-1] of Byte;
