...
type
  TCaptureSmallIconForm = class( TForm )
    Timer1: TTimer;
...
var
  CaptureSmallIconForm: TCaptureSmallIconForm;
  Counter: Byte;
  CounterStart: Byte;
  Looper: LongInt;
...
{ Animated Rubbanding }
procedure TCaptureSmallIconForm.RemoveTheRect;
var
  R: TRect;
begin
...
{ Animated Rubbanding }
procedure TCaptureSmallIconForm.DrawTheRect;
begin
...
procedure TCaptureSmallIconForm.FormCreate( Sender: TObject );
var
  aDC: HDC;
  P1: TPoint;
...
procedure TCaptureSmallIconForm.FormMouseDown( Sender: TObject; Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer );
begin
  RemoveTheRect; // Erase any existing rectangle
  X1 := X;
...
procedure TCaptureSmallIconForm.FormMouseMove( Sender: TObject; Shift: TShiftState; X, Y: Integer );
begin
  RemoveTheRect; // Erase any existing rectangle
  X1 := X;
  Y1 := Y;
  ...
  begin
    ...
    fRect.Left := X;
    fRect.Top := Y;
    fRect.Right := X + 16; // Set the position of the rectangle to capture
    fRect.Bottom := Y + 16;
  end;
end;

procedure TCaptureSmallIconForm.FormMouseUp( Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer );
var
  ScreenDC: HDC;
  Bitmap: TBitmap;
begin
...
procedure TCaptureSmallIconForm.FormPaint( Sender: TObject );
begin
  Canvas.Draw( 0, 0, fBmp );
end;

procedure TCaptureSmallIconForm.FormDestroy( Sender: TObject );
begin
  fBmp.Free;
  RectBitmap.Free;
end;

procedure TCaptureSmallIconForm.FormKeyDown( Sender: TObject; var Key: Word; Shift: TShiftState );
var
  ScreenDC: HDC;
  Bitmap: TBitmap;
  iPoint: TPoint;
  ...
  case Key of
    ...
    VK_RETURN:
      begin
        ...
        CaptureSmallIconForm.ModalResult := mrOK;
      end;
    VK_F8:
...
procedure TCaptureSmallIconForm.WMEraseBkGnd( var Msg: TWMEraseBkGnd );
begin
  Msg.Result := 1;
end;

{ Animated Rubbanding }
procedure TCaptureSmallIconForm.Timer1Timer( Sender: TObject );
begin
