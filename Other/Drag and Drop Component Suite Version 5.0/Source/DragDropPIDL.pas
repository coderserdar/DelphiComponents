...
procedure ILFree(Buffer: PItemIDList); stdcall; {$ifdef VER15_PLUS} deprecated; {$endif}
...
begin
  ...
  if SHGetPathFromIDList(PItemIDList(PAnsiChar(PIDL)), Path) then
    Result := Path
  else
    Result := '';
end;
...
{$ifdef DD_WIDESTRINGLIST}
function TPIDLsToFilenamesStrings.GetWide(Index: Integer): UnicodeString;
var
  PIDL: string;
...
{$ifdef DD_WIDESTRINGLIST}
procedure TPIDLsToFilenamesStrings.PutWide(Index: Integer; const S: UnicodeString);
begin
end;
{$endif}
