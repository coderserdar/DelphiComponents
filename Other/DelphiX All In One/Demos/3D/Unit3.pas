...
type
  ...
  private
    ...
    TriangleVertices: array[1..6] of TD3DVertex;
...
var
  n0: TD3DVector; {Normálový (kolmý) vektor}
begin
  ...
  TriangleVertices [1] := MakeD3DVertex (
    MakeD3DVector (-1, -1, 0), n0, 0, 0);
  TriangleVertices [2] := MakeD3DVertex (
    MakeD3DVector (+1, -1, 0), n0, 1, 0);
  TriangleVertices [3] := MakeD3DVertex (
    MakeD3DVector ( 0, +1, 0), n0, 0.5, 1);

  TriangleVertices [4] := MakeD3DVertex (
    MakeD3DVector (+1, +1, 2), n0, 0, 0);
  TriangleVertices [5] := MakeD3DVertex (
    MakeD3DVector (-1, +1, 2), n0, 1, 0);
  TriangleVertices [6] := MakeD3DVertex (
    MakeD3DVector ( 0, -1, 2), n0, 0.5, 1);
end;
...
begin
  ...
  DXDraw1.D3DDevice7.SetRenderState(D3DRENDERSTATE_ALPHABLENDENABLE, 0);
  DXDraw1.D3DDevice7.SetRenderState(D3DRENDERSTATE_ZENABLE, Integer (D3DZB_TRUE));
  DXDraw1.D3DDevice7.SetRenderState(D3DRENDERSTATE_ZVISIBLE, 1);
  DXDraw1.D3DDevice7.SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, 1);

  {Tu bude vykreslovanie neprieh¾adných objektov}

  DXDraw1.D3DDevice7.SetRenderState(D3DRENDERSTATE_ALPHABLENDENABLE, 1);
  DXDraw1.D3DDevice7.SetRenderState(D3DRENDERSTATE_SRCBLEND,
    Integer (D3DBLEND_SRCALPHA));
  DXDraw1.D3DDevice7.SetRenderState(D3DRENDERSTATE_DESTBLEND,
    Integer (D3DBLEND_DESTALPHA));
  DXDraw1.D3DDevice7.SetRenderState(D3DRENDERSTATE_ZWRITEENABLE, 0);

  {Tu bude vykreslovanie prieh¾adných objektov}
  ...
  DXDraw1.D3DDevice7.DrawPrimitive(D3DPT_TRIANGLELIST,
    D3DFVF_VERTEX, TriangleVertices[1], 6, 0);
