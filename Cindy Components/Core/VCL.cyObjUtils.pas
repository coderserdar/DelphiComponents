...
type
  ...
  function RichEditStringLocate(aRichEdit: TRichEdit; const aText: string; const Options: TSearchTypes; const FromPos: Integer = 0; const SearchLength: Integer = 0; const AutoSelectText: Boolean = true): Integer;
...
begin
  if SearchLength < 1
  then NbCars := Length(aRichEdit.Text)
  else NbCars := SearchLength;

  Result := aRichEdit.FindText(aText, FromPos, NbCars, Options);

  if (Result <> -1) and AutoSelectText then
  begin
    aRichEdit.Lines.BeginUpdate;
    aRichEdit.SelStart  := Result;
...
begin
  ...
  try
    ...
    begin
      ...
      Result.Canvas.Lock;                  // Multithread drawing ...
      try
