...
{   Classes(s):
    TcyDbxAutoRefreshOptions, TcyDbxBaseTableUpdatesOptions, TcyDbxRecordLockOptions

    Description:
    Classes used by TcyDBXTable and TcyDbxSimpleTable
...
type
  ...
  TcyDbxSchemaOptions = class(TPersistent)
  private
    FOwner: TComponent;
...
procedure TcyDbxAutoRefreshOptions.DeactivateAll;
begin
  AfterDelete := false;
  AfterPostModifiedRecord := rfNone;
  AfterPostNewRecord := rfNone;
  MovingToFirst := false;
  MovingToLast := false;
end;
...
{ TcyDbxSchemaOptions }
constructor TcyDbxSchemaOptions.Create(AOwner: TComponent);
begin
  FOwner := AOwner;
...
procedure TcyDbxSchemaOptions.SetMySQL_UseInformationSchema(const Value: Boolean);
begin
  FMySQL_UseInformationSchema := Value;

  if (csDesigning in FOwner.ComponentState) and (not (csLoading in FOwner.ComponentState)) then
    ShowMessage('Note that connected user must have specific rights in order to access Information Schema !');
end;
