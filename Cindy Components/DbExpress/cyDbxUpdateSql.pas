...
uses Classes, Windows, SysUtils, SqlExpr, {$IFDEF DELPHI2007_OR_ABOVE} DBXCommon, {$ENDIF} Dialogs, cyDBX, cyDBXBaseTable;
...
begin
  ...
  try
    if cyDbxUpdateSql.FOptions.MySQL_UseInformationSchema then
    begin
      aQuery.SQL.Text := 'SELECT DISTINCT TABLE_NAME FROM information_schema.columns WHERE TABLE_SCHEMA = '
                          + SQLGetStringExpr(cyDbxUpdateSql.FSQLConnection.Params.Values['Database'], false, 0, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection))
                          + ' AND TABLE_NAME = ' + SQLGetStringExpr(FTableName, false, 0, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection));
    end
    else begin
      // Not working on MySQL prior 5.x and on Delphi 7:
      aQuery.SQL.Text := 'SHOW TABLES LIKE ' + SQLGetStringExpr(FTableName, false, 0, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection));
    end;
  ...
  try
    ...
    begin
      aQuery.SQL.Text := 'SELECT COLUMN_NAME FROM information_schema.columns WHERE TABLE_SCHEMA = '
                           + SQLGetStringExpr(cyDbxUpdateSql.FSQLConnection.Params.Values['Database'], false, 0, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection))
                           + ' AND TABLE_NAME = ' + SQLGetStringExpr(FTableName, false, 0, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection))
                           + ' AND COLUMN_NAME = ' + SQLGetStringExpr(FColumnName, false, 0, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection));
    end
    else begin
      // Not working on MySQL prior 5.x :
      aQuery.SQL.Text := 'SHOW COLUMNS FROM ' + FTableName + ' LIKE ' + SQLGetStringExpr(FColumnName, false, 0, GetDbxConnectionDriver(cyDbxUpdateSql.FSQLConnection));
    end;
...
procedure TcyDbxUpdateSql.SetOptions(const Value: TcyDbxSchemaOptions);
begin
  FOptions := Value;
end;
