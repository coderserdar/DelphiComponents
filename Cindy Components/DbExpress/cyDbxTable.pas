...
begin
  ...
  if not FCachedUpdates then
    if Active then
      if Assigned(FActiveConnection) then     // 2014-05-29
...
begin
  ...
  // Apply updates :
  if (not CachedUpdates) and (Assigned(FActiveConnection)) then     // 2014-05-29
    if ChangeCount <> 0 then
    begin
      FLastErrorCount := ApplyUpdates(FUpdatesOptions.MaxErrors);
...
begin
  ...
  if aTableName <> '' then
  begin
    aPkFieldName := SQLGetPrimaryKey(FActiveConnection, aTableName);
...
function TcyDbxTable.GetDbxConnectionDrv: TDbxConnectionDriver;
begin
  Result := cyDBX.GetDbxConnectionDriver(FActiveConnection);
end;
...
function TcyDbxTable.LockRecord: Boolean;
begin
  Result := false;
...
function TcyDbxTable.UnlockRecord: Boolean;
var
  SQLCmd: String;
...
begin
  ...
  try
    ...
    begin
      ...
      SQLCmd := 'SELECT RELEASE_LOCK(' + cyDBX.SQLGetStringExpr(GetInternalRecordLockId, false, 0, GetDbxConnectionDrv) + ')';

      // 2017-05-18 ...
      // FActiveConnection.ExecuteDirect(SQLCmd);
      Result := cyDBX.SQLReturnFieldValueAsInteger(FActiveConnection, SQLCmd) = 1;
    end;
  finally
