...
unit cyBaseWebBrowserD2007;

interface

uses
  Classes, Graphics, SHDocVw, SysUtils, StrUtils, Windows, ActiveX, Forms, MSHTML, Dialogs, OleServer;

type
  TEncoding = (EDefault, EUnicode, EUnicodeBigEndian, EUtf8);
  ...
  public
    ...
    function LoadFromString(aString: string; const TextContent: TTextContent = tcDocument): boolean;
    ...
    procedure SaveToFile(const FileName: string; const TextContent: TTextContent = tcDocument);
...
procedure TcyBaseWebBrowser.Loaded;        // !!! Not called for Dynamic controls !!!
begin
  inherited;
...
procedure TcyBaseWebBrowser.Navigate(const URL: WideString; var Flags,
...
begin
  ...
  if aCharset = 'utf-8' then aString := AddHtmlUnicodePrefix(aString);
  if aCharset = 'unicode' then aString := AddHtmlUnicodePrefix(aString);
  if aCharset = 'unicodefeff' then aString := AddHtmlUnicodePrefix(aString);
...
begin
  ...
  try
    if SaveToStream(StringStream, TextContent)
    then Result := StringStream.DataString
    else Result := '';
  finally
    StringStream.Free;
  end;
