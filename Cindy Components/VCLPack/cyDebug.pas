...
uses Classes, Windows, SysUtils, Forms, Grids, cyStrUtils;
...
type
  ...
  TLogOption = (loAddTitles, loAddDate, loAddTime, loAddAppHandle);
  ...
  public
    ...
    procedure AddToLog(aText: String; const ClearBefore: Boolean = false);
  ...
  published
    ...
    property CsvLogSeparation: Char read fCsvLogSeparation write fCsvLogSeparation default ';';
    property LogFile: String read FLogFile write FLogFile;
    property LogOptions: TLogOptions read FLogOptions write SetLogOptions default [loAddTitles, loAddDate, loAddTime, loAddAppHandle];
...
begin
  ...
  fCsvLogSeparation := ';';
  FLogOptions := [loAddTitles, loAddDate, loAddTime, loAddAppHandle];
  inherited;
end;
...
procedure TcyDebug.SetLogOptions(const Value: TLogOptions);
begin
  FLogOptions := Value;
end;
...
procedure TcyDebug.AddToLog(aText: String; const ClearBefore: Boolean = false);
var
  aTextfile: TextFile;
  FirstCol, isEmpty: Boolean;
  TextLine: string;

        procedure CsvLineAdd(const aStr: String);
        begin
          if FirstCol
          then FirstCol := false
          else TextLine := TextLine + fCsvLogSeparation;

          TextLine := TextLine + cyStrUtils.StringToCsvCell(aStr, fCsvLogSeparation);
        end;

begin
  try
    // 1) Add Application.Handle (will be handled by a property in future ... )
    // 2) Export as csv file ...
    aText := intToStr(Application.Handle) + ' ' + aText;

    AssignFile(aTextfile, FLogFile);

    if (not FileExists(FLogFile)) or (ClearBefore) then
    begin
      Rewrite(aTextfile);
      isEmpty := True;
    end
    ...
    // Add titles :
    if isEmpty then
      if loAddTitles in FLogOptions then
      begin
        TextLine := '';
        FirstCol := true;
        if loAddDate in FLogOptions then
          CsvLineAdd('Date');
        if loAddTime in FLogOptions then
          CsvLineAdd('Time');
        if loAddAppHandle in FLogOptions then
          CsvLineAdd('Application handle');
        CsvLineAdd('Message');

        Write(aTextfile, TextLine);
        isEmpty := false;
      end;

    if not isEmpty then
      Writeln(aTextfile, '');   // Open new line
    ...
    if loAddDate in FLogOptions then
      CsvLineAdd(FormatDateTime('dd/mm/yyyy', Now));
    if loAddTime in FLogOptions then
      CsvLineAdd(FormatDateTime('hh:nn:ss:zzz', Now));
    if loAddAppHandle in FLogOptions then
      CsvLineAdd(intToStr(Application.Handle));
    CsvLineAdd(aText);

    Write(aTextfile, TextLine);
    // Old ... Write(aTextfile, aText);
