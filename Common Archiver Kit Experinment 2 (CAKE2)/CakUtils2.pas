...
uses
  CakDefs2, Controls, Windows, SysUtils, Classes, Forms, Dialogs, StdCtrls, ExtCtrls,
  FileCtrl, Registry, Inifiles, Shellapi, ShlObj, Graphics, MAPI, Floppy,
  Vwin32, Links, Jpeg, Syncobjs, ActiveX, Comobj, StrUtils, Cakdir2, Menus, Masks, ZipRepair;
...
function Createprggrpshortcut(shortcut, caption, prggrp : string) : string;
...
procedure Archive_Convert(Cakdir2 : TCakdir2; filename : string; totype : supporttype);
...
procedure FocusComponent(Control : TWinControl);
procedure RightAllignMenu(MainMenu : TMainMenu; Help1 : TMenuItem);

function Match(filter,text : string) : boolean;

function MinimizeFileName(const FileName : TFileName; Font : TFont; MaxLen : integer) : TFileName;

procedure repairZip(SourceName, DestName : string);
function FormatSizeWithSpaces(totalsize : integer; input : string) : string;
implementation
...
//  Directory := MyReg.ReadString('Shell Folders','Desktop','');
   ...
   Directory := MyReg.ReadString('Shell Folders','Start Menu','')+ '\Programs\' +  prggrp;
   CreateDir(Directory);

  WFileName := Directory+'\'+caption+'.lnk';
  result := WFilename;
...
var
  ...
  i:       Integer;
begin
  MakeDirectory(Extractfilepath(Target));
...
begin
        ...
        CakDir2.New(Removefileext(filename) + Cakdir2.GetarcString(totype));
...
begin
  ...
  c := Control.Parent;
  while not (c is TForm) and (c.Parent <> nil) do
    begin
      pt := c.ClientToParent(pt);
      c  := c.Parent;
    end;
  if (c is TForm) then
  Pt := c.ClientToScreen(Pt) else exit;
...
begin
  ...
  if GetMenuItemInfo(MainMenu.Handle,Help1.MenuIndex,True, I) then begin
    I.fType := I.fType or MFT_RIGHTJUSTIFY;
    if SetMenuItemInfo(MainMenu.Handle,Help1.MenuIndex, True, I) then
...
begin
  ...
  filterList.CommaText := filter;
  result := false;
  for i := 0 to filterlist.Count -1 do
    if not Result then
    begin
    Mask := TMask.Create(filterlist.Strings[i]);
    if Mask.Matches(text) then
      Result := true;
    Mask.Free;
    end;
  FilterList.Free;
...
function MinimizeFileName(const FileName : TFileName; Font : TFont; MaxLen : integer) : TFileName;
      ...
      begin
        ...
        Result := Image.Canvas.TextWidth(Text);
        Image.Canvas.Free;
      end;
  ...
  begin
    ...
    if (Length(Dir) >= 2) and (Dir[2] = '\') then
      begin
        Drive := Copy(Dir, 1, 2 + Pos('\', Copy(Dir, 3,Length(Dir))));
        Delete(Dir, 1, Length(Drive));
      end else  Drive := '';
    while ((Dir <> '') or (Drive <> '')) and (GetWidth(Result) > MaxLen) do
      begin
        if Dir = '\...\' then
          begin
            Drive := '';
    ...
    if GetWidth(Result) > MaxLen then
      Result := '';
    end;
...
begin
  ...
  Result := input;
  try
    sizeOfSpace := Image.Canvas.TextWidth(' ');
    sizeOfInput := Image.Canvas.TextWidth(input);
    if totalsize-sizeOfInput > 0 then
      Result := Format('%-'+inttostr((totalsize-sizeOfInput) div sizeOfSpace+1)+'s',[Result]);
  finally
    Image.Free;
  end;
end;
