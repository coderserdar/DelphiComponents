...
begin
    if (Uppercase(Extractfileext(Cakdir.Archivename)) = '.UUE') or
       (Uppercase(Extractfileext(Cakdir.Archivename)) = '.UU')
    then
       ProcessUUE(doWhat) else
     if (Uppercase(Extractfileext(Cakdir.Archivename)) = '.XXE') or
       (Uppercase(Extractfileext(Cakdir.Archivename)) = '.XX')
     then
     ...
     begin
      Total_Contents := 0;
       begin
        ...
        CloseFile(tf);
        IDUUDecoder1.Free;
        CakDir.AddContents(ArchiveName,'',fn,fz,fz,false,Now,'FFFFFFFF');
       end;
     end;
    wtExtract: 
     begin
