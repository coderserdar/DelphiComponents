...
type
  TKADaoTablesCheckListBox = class(TCheckListBox)
  private
...
Constructor TKADaoTablesCheckListBox.Create(AOwner: TComponent);
Begin
...
Destructor TKADaoTablesCheckListBox.Destroy;
Begin
 F_DataLink.Free;
 F_DataLink := nil;
 inherited Destroy;
...
procedure TKADaoTablesCheckListBox.Notification(AComponent: TComponent; Operation: TOperation);
begin
...
Procedure TKADaoTablesCheckListBox.Loaded;
Begin
 Inherited Loaded;
 Items.Clear;
End;

Procedure TKADaoTablesCheckListBox.Click;
Var
  TT : Integer;
Begin
  Inherited Click;
...
Procedure TKADaoTablesCheckListBox.ActiveChange(Sender: TObject);
Var
  II  : Integer;
  Txt : String;
Begin
...
Procedure TKADaoTablesCheckListBox.F_Set_ViewType(Value : TViewObjects);
Begin
  F_ViewType   := Value;
  ActiveChange(Self);
End;

Function TKADaoTablesCheckListBox.F_Get_DataSource: TDataSource;
begin
  Result := F_DataLink.DataSource;
end;

Procedure TKADaoTablesCheckListBox.F_Set_DataSource(Value: TDataSource);
begin
  F_DataLink.DataSource := Value;
  if Value <> nil then Value.FreeNotification(Self);
end;

Procedure TKADaoTablesCheckListBox.F_Set_AutoOpen(Value: Boolean);
Begin
 F_AutoOpen := Value;
 Items.Clear;
 ActiveChange(Self);
...
procedure Register;
begin
  RegisterComponents('KADao Controls', [TKADaoTablesCheckListBox]);
end;
