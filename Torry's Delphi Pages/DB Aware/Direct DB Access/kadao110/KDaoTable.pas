...
protected
        ...
        Function        InternalFillRecordData(RS: OleVariant; MainTable : Boolean; Buffer:PChar):Boolean;
         ...
         Begin
          ...
          if Boolean(F_UpdatableFields.Items[X]) Then
             Begin
               if S='' Then
                  Begin
                    if (FF.DataType=ftString) or (FF.DataType=ftMemo) Then
                       Begin
                  ...
                  Begin
                    ...
                    Else
                       Begin
                         ...
                         Else
                            Begin
                              if (F_HasEncoder) And (FF.DataType=ftString) Then
                                 Begin
                                   //*******************************************
                                   // Perform Encoding here
                                   //*******************************************
...
Function TKADaoTable.GetCurrentRecord(Buffer: PChar): Boolean;
Var
  AB : PChar;
begin
  Result := F_Active;
...
{$IFDEF D4UP}
Function  TKADaoTable.Translate(Src, Dest: PChar; ToOem: Boolean):Integer;
{$ELSE}
Procedure TKADaoTable.Translate(Src, Dest: PChar; ToOem: Boolean);
{$ENDIF}
Begin
  {$IFDEF D4UP}
  ...
  if F_Translation Then
     Begin
      ...
      if (Src <> nil) and (Src <> Dest) then
         Begin
           if ToOem Then CharToOem(Src, Dest) Else OemToChar(Src,Dest);
           {$IFDEF D4UP}
           Result := StrLen(Dest);
           {$ENDIF}
         End;
...
Function TKADaoTable.InternalFillRecordData(RS: OleVariant; MainTable : Boolean; Buffer:PChar):Boolean;
Var
 X             : Integer;
 RD            : Variant;
 DTS           : TTimeStamp;
