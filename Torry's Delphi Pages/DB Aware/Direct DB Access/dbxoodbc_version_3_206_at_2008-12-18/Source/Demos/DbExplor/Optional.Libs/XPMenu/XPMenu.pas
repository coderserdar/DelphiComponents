...
//+
function CreateXPMenu(AOwner: TComponent): TXPMenu;
//+.
...
function CreateXPMenu(AOwner: TComponent): TXPMenu;
begin
  Result := XPMenu.TXPMenu.Create(AOwner);
  Result.Active := False;
  Result.xpContainers := [xccForm, xccFrame, xccPanel, xccScrollBox, xccTabSheet, xccPageScroller];
  Result.xpControls := [xcMainMenu, xcPopupMenu, xcListBox, xcEdit, xcMaskEdit, xcMemo, xcRichEdit,
...
begin
  ...
  {+}
  if (Comp is TMenuItem) and (xcMainMenu in XPControls) and (TMainMenu(TMenuItem(Comp).Owner).Tag <> 999) then
  begin
...
var
  X1, X2: integer;
//+
  SelectColor, BorderColor, FillColor: TColor;
const
 dx = 2;
 dy = 1;
//+.
      ...
      //+
        ...
        begin
          ...
          begin
            ...
            //else
            SelectColor := $00ECD9D9;
            BorderColor := FFSelectBorderColor;
          end;
        end
        else
        begin
          FillColor := FFMenuBarColor ;
          SelectColor := FFMenuBarColor;
          BorderColor := FDisabledColor;
        end;

        ACanvas.Brush.Color := SelectColor;
        ACanvas.Pen.Color := BorderColor;

        X1 := (CheckedRect.Right - CheckedRect.Left);
        X2 := (CheckedRect.Bottom - CheckedRect.Top);
        X1 := X2 - X1;
        if X1 > 0 then
          Inc(CheckedRect.Left, X1)
        else
          Inc(CheckedRect.Bottom, -X1);
        ...
        begin
          ...
          ACanvas.Rectangle(CheckedRect.Left+3, CheckedRect.Top+3, CheckedRect.Right-3, CheckedRect.Bottom-3);
        end;
        InflateRect(CheckedRect, -2, -2);
        ACanvas.Pen.Color := BorderColor;
        ACanvas.Brush.Color := SelectColor;
        ...
        begin
            ...
            begin
              ...
              ACanvas.Pen.color := FFDisabledColor;
            ACanvas.Polyline([
               Point(CheckedRect.Right - dx - 7, CheckedRect.Bottom - dy - 5),
               Point(CheckedRect.Right - dx - 5, CheckedRect.Bottom - dy - 3),
               Point(CheckedRect.Right - dx + 1, CheckedRect.Bottom - dy - 8 - 1)
            ]);

            ACanvas.Polyline([
               Point(CheckedRect.Right - dx - 7, CheckedRect.Bottom - dy - 4 ),
               Point(CheckedRect.Right - dx - 5, CheckedRect.Bottom - dy - 2),
               Point(CheckedRect.Right - dx + 1, CheckedRect.Bottom - dy - 7 - 1)
            ]);
            ACanvas.Polyline([
               Point(CheckedRect.Right - dx - 7, CheckedRect.Bottom - dy - 3 ),
               Point(CheckedRect.Right - dx - 5, CheckedRect.Bottom - dy - 1),
               Point(CheckedRect.Right - dx + 1, CheckedRect.Bottom - dy - 6 - 1)
            ]);
        end;
      //+.
      end
...
//      SelectColor := XPMenu.FFSelectColor;
      //+ ??? via XP Skin
      //if xpMenu.UseSystemColors then
      //  SelectColor := xpMenu.FFSelectColor
      //else
        SelectColor := $0048C2FD;
      //+
...
//      SelectColor := XPMenu.FFSelectColor;
    ...
    InflateRect(R, 0, -3);
    //+
    // v_SM_CXHTHUMB := GetSystemMetrics(SM_CXHTHUMB); // ???
    v_SM_CXHTHUMB := 19;
    //+.
//    R.Top := R.Top + ((R.Bottom - R.Top - GetSystemMetrics(SM_CXHTHUMB)) div 2);
//    R.Bottom := R.Top + GetSystemMetrics(SM_CXHTHUMB);
    //+
    R.Top := R.Top + ((R.Bottom - R.Top - v_SM_CXHTHUMB) div 2);
    R.Bottom := R.Top + v_SM_CXHTHUMB;
    //+.
...
//      R.Left := R.Right - GetSystemMetrics(SM_CXHTHUMB) + 1
      //+
      begin
        R.Left := R.Right - v_SM_CXHTHUMB + 1 + 5;
        inc(R.Right,5);
      end
      //+.
    else
...
//      R.Right := R.Left + GetSystemMetrics(SM_CXHTHUMB) - 1;
     //+
      R.Right := R.Left + v_SM_CXHTHUMB - 1
     else
      R.Right := R.Left + v_SM_CXHTHUMB - 1
     ;
     //+.
...
//    C.Rectangle(R.Left, R.Top, R.Right, R.Bottom);
    //+
       C.Rectangle(R.Left, R.Top+2, R.Right-2, R.Bottom-2);
       //if FIsFocused then begin
             C.Brush.Color := clWindow;
             C.Pen.Color := clWindow;
           C.Rectangle(R.Left+3, R.Top+2+3, R.Right-2-3, R.Bottom-2-3);
             C.Pen.Color := BorderColor;
             C.Brush.Color := SelectColor;
       //end;
    //+.
...
//      DrawCheckMark(C, R.Left, R.Bottom )
      //+
        C.Polyline([
           Point(R.Right - dx - 7, R.Bottom - dy - 5),
           Point(R.Right - dx - 5, R.Bottom - dy - 3),
           Point(R.Right - dx,     R.Bottom - dy - 8)
        ]);
        C.Polyline([
           Point(R.Right - dx - 7, R.Bottom - dy - 4 ),
           Point(R.Right - dx - 5, R.Bottom - dy - 2),
           Point(R.Right - dx,     R.Bottom - dy - 7)
        ]);
        C.Polyline([
           Point(R.Right - dx - 7, R.Bottom - dy - 3 ),
           Point(R.Right - dx - 5, R.Bottom - dy - 1),
           Point(R.Right - dx,     R.Bottom - dy - 6)
        ]);
      //+.
    end;
