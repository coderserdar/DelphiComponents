...
uses
  ...
  WideStrings,
  {.$IFEND}
  dbx_dbf_connect, DbxOpenOdbcCallback, DbxOpenOdbcFuncs;
...
procedure TForm1.SQLConnectionAfterConnect(Sender: TObject);
  ...
  begin
    ...
    if cbx_query.Items.Count = 0 then
      cbx_query.Items.Text := 'select * from table_demo';
...
begin
  ...
  SQLQuery.Close;
  DbxOpenOdbcFuncs.LoadStringFromFileW('..\DB\sql_text_unicode.txt', ws);
  SQLQuery.SQL.Text := '';
