...
type
  ...
  TDataSetClass = class of TDataSetPro;
  TObjectProc = procedure of object;

procedure TDataSetPro.CloseCursor;
begin
  inherited;
end;
{$IFEND}
...
begin
  ...
  {
  Result := nil;
  // calculate method: TDataSet.CloseCursor
  vClass := nil;
  xClass := Self.ClassParent;
  while Assigned(xClass) do
  begin
    if xClass = TDataSet then
    begin
       vClass := TDataSetClass(xClass);
       Break;
    end;
  ...
  {}
  vMethod.Code := @TDataSetPro.CloseCursor;// @vClass.CloseCursor;
  vMethod.Data := Self;
  {$ELSE}
  SetState(dsInactive);
  CloseCursor;
