...
begin
  {+}
  WriteLn('Make JCL debug data command line utility. (c) 2007.04.25 Project JEDI "2.00.fixed"');
  {+.}
  S1 := ParamStr(1);
  JdbgFlag := AnsiSameText(S1, '-J');
  {+}
  if not JdbgFlag then
  begin
    InsertResToExeFlag := AnsiSameText(S1, '-R');
    if InsertResToExeFlag then
      InsertToExeFlag := True
    else
      InsertToExeFlag := AnsiSameText(S1, '-E');
  end;
  {+.}
  if (ParamCount <> 2) or not (JdbgFlag xor InsertToExeFlag) then
  begin
    WriteLn('Usage: MAKEJCLDBG -<J|E|R> <map filenames>');
  ...
  begin
    S1 := ParamStr(2);
    S1 := ChangeFileExt(S1, '.map');
    if not MakeDebugData(S1) then
      Halt(1);
  end;
  {+.}
end.
