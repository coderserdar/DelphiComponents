...
//
// $Id: TBXLists.pas 77 2005-09-19 13:38:48Z Alex $
...
uses
  ...
  TBXThemes, TBXUxThemes, TBXUtils, ImgList, TBXStrUtils;
...
type
  ...
  protected
    ...
    procedure PaintButton(DC: HDC; Rect: TRect; Direction: Integer; Pushed, Enabled: Boolean);
    procedure PaintHandle(DC: HDC; Rect: TRect; Pushed, Enabled: Boolean);
    procedure PaintTrack(DC: HDC; Rect: TRect; IsNextZone, Pushed, Enabled: Boolean);
  ...
  protected
    ...
    function  GetItemText(Index: Integer): WideString; virtual; abstract;
  ...
  protected
    ...
    procedure DrawItems(Canvas: TCanvas; const ClientAreaRect: TRect);
  ...
  protected
    function GetItemText(Index: Integer): WideString; override;
  ...
  published
    ...
    property MaxWidth;
    property MinWidth;
    property Lines: TStringListW read FLines write SetLines;
...
procedure TTBXScrollBar.PaintButton(DC: HDC; Rect: TRect; Direction: Integer; Pushed, Enabled: Boolean);
const
  DirectionFlags: array [0..3] of Cardinal = (DFCS_SCROLLLEFT, DFCS_SCROLLUP, DFCS_SCROLLRIGHT, DFCS_SCROLLDOWN);
  EnabledFlags: array [Boolean] of Cardinal = (DFCS_INACTIVE, 0);
  PushedFlags: array [Boolean] of Cardinal = (0, DFCS_PUSHED or DFCS_FLAT);
  DirectionXPFlags: array [0..3] of Cardinal = (ABS_LEFTNORMAL, ABS_UPNORMAL, ABS_RIGHTNORMAL, ABS_DOWNNORMAL);
var
  StateFlags: Cardinal;
begin
  if USE_THEMES then
  begin
    ...
    DrawThemeBackground(SCROLLBAR_THEME, DC, SBP_ARROWBTN, StateFlags, Rect, nil);
  end
  else
    DrawFrameControl(DC, Rect, DFC_SCROLL, DirectionFlags[Direction] or EnabledFlags[Enabled] or PushedFlags[Pushed]);
end;

procedure TTBXScrollBar.PaintHandle(DC: HDC; Rect: TRect; Pushed, Enabled: Boolean);
const
...
begin
  ...
  begin
    ...
    DrawThemeBackground(SCROLLBAR_THEME, DC, ThumbXPFlags[Kind], StateFlags, Rect, nil);
    if (GetThemeMargins(SCROLLBAR_THEME, DC, ThumbXPFlags[Kind],
      StateFlags, TMT_CONTENTMARGINS, nil, ContentMarg) = S_OK) and
        (GetThemePartSize(SCROLLBAR_THEME, DC, GripperXPFlags[Kind],
      ...
      with Rect, ContentMarg, GripperSize do
        if ((Right- Left)- (cxLeftWidth+ cxRightWidth) >= cx) and
          ((Bottom- Top)- (cyTopHeight+ cyBottomHeight) >= cy) then
            DrawThemeBackground(SCROLLBAR_THEME, DC,
              GripperXPFlags[Kind], StateFlags, Rect, nil);
    end;
  end
  else
  begin
    DrawEdge(DC, Rect, EDGE_RAISED, BF_RECT or BF_ADJUST);
    FillRectEx(DC, Rect, clBtnFace);
  end;
end;

procedure TTBXScrollBar.PaintTo(DC: HDC);
var
  R: TRect;
  E, IsVert: Boolean;
  I: Integer;
...
begin
  ...
  OffsetWindowOrgEx(DC, -Bounds.Left, -Bounds.Top, Dummy);
  try

    if IsVert then I := 1 else I := 0;
    PaintButton(DC, Zones[sbzPrev], I, MouseDownZone = sbzPrev, E);
    PaintButton(DC, Zones[sbzNext], I + 2, MouseDownZone = sbzNext, E);

    if not IsRectEmpty(Zones[sbzEmpty]) then FillRectEx(DC, Zones[sbzEmpty], clScrollBar);

    if not IsRectEmpty(Zones[sbzPagePrev]) or not IsRectEmpty(Zones[sbzPageNext]) then
    begin
      R := Zones[sbzPagePrev];
      PaintTrack(DC, R, False, MouseDownZone = sbzPagePrev, E);
      R := Zones[sbzPageNext];
      PaintTrack(DC, R, True, MouseDownZone = sbzPageNext, E);
    end;

    if not IsRectEmpty(Zones[sbzHandle]) then
      PaintHandle(DC, Zones[sbzHandle], MouseDownZone = sbzHandle, E);
...
procedure TTBXScrollBar.PaintTrack(DC: HDC; Rect: TRect; IsNextZone, Pushed, Enabled: Boolean);
const
...
begin
  ...
  begin
    ...
    DrawThemeBackground(SCROLLBAR_THEME, DC, PartXPFlags[IsNextZone, Kind], StateFlags, Rect, nil);
  end
  else
  begin
    if Pushed then FillRectEx(DC, Rect, cl3DDkShadow)
    else DitherRect(DC, Rect, clBtnHighlight, clScrollBar);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      else DrawTBXImage(ACanvas.Handle, R2, ImgList, GetImageIndex(AIndex), ISF_DISABLED);
    ...
    if Length(S) > 0 then
    begin
      DrawTextRW(ACanvas.Handle, S, R, DT_SINGLELINE or DT_VCENTER or DT_NOPREFIX, TextColors[AIndex = AHoverIndex]);
    end;
  end;
end;
...
procedure TTBXCustomListViewer.DrawItems(Canvas: TCanvas; const ClientAreaRect: TRect);
var
  I: Integer;
  R: TRect;
begin
  ...
  begin
    ...
    FillRectEx(Canvas.Handle, R, clWindow);
  end;
end;
...
begin
  ...
  begin
    S := GetItemText(Index);
    Result := GetTextWidthW(ACanvas.Handle, S, False);
    if ShowImages then
    begin
      ImgList := GetImageList;
      if ImgList <> nil then
...
begin
  ...
  begin
    ...
    FScrollBar.PaintTo(Canvas.Handle);
  end;
end;
...
function TTBXStringList.GetItemText(Index: Integer): WideString;
begin
  Result := FLines[Index];
end;

procedure TTBXStringList.SetLines(Value: TStringListW);
begin
  FLines.Assign(Value);
end;
...
begin
  ...
  begin
    ...
    _DrawTextW(ACanvas.Handle, PWideChar(S), Length(S), R, DT_SINGLELINE or DT_VCENTER);
