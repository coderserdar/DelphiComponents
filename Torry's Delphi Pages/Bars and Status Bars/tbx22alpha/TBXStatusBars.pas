...
//
// $Id: TBXStatusBars.pas 99 2005-09-23 19:34:50Z Alex $
...
type
  ...
  private
    ...
    procedure ReadSimpleTextProperty(Reader: TReader);
    procedure WriteSimpleTextProperty(Writer: TWriter);
  protected
  ...
  public
    ...
    property SimpleText: WideString read FSimpleText write SetSimpleText stored False;
...
uses TBXUtils, TB2Common, TB2Dock;
...
begin
  if Length(Caption) > 0 then Result := ACPScrambleWideString(Caption) else SetLength(Result, 0);
...
procedure TTBXStatusPanel.SetCaption(const Value: WideString);
begin
...
procedure TTBXStatusPanel.DefineProperties(Filer: TFiler);
begin
  inherited;
  Filer.DefineProperty('Caption', ReadCaptionProperty, WriteCaptionProperty, Length(Caption) > 0);
  Filer.DefineProperty('Hint', ReadHintProperty, WriteHintProperty, Length(Hint) > 0);
end;

procedure TTBXStatusPanel.ReadCaptionProperty(Reader: TReader);
begin
  Caption := FilerReadWideString(Reader);
end;

procedure TTBXStatusPanel.ReadHintProperty(Reader: TReader);
begin
  Hint := FilerReadWideString(Reader);
end;

procedure TTBXStatusPanel.WriteCaptionProperty(Writer: TWriter);
begin
  FilerWriteWideString(Writer, Caption);
end;

procedure TTBXStatusPanel.WriteHintProperty(Writer: TWriter);
begin
...
begin
  ...
  if Panel <> nil then
  begin
    Message.HintInfo.HintStr := Utf8Encode(Panel.Hint);
    Message.HintInfo.CursorRect := Panel.BoundsRect;
    Message.HintInfo.HintWindowClass := TTBHintWindow;
  end;
end;
...
procedure TTBXCustomStatusBar.DefineProperties(Filer: TFiler);
begin
  inherited;
  Filer.DefineProperty('SimpleText', ReadSimpleTextProperty, WriteSimpleTextProperty, Length(SimpleText) > 0);
end;
...
begin
  inherited;
  CR := ClientRect;
  CurrentTheme.PaintStatusBar(Canvas.Handle, CR, SBP_BODY);
  Inc(CR.Top, 2);
  if SimplePanel then
  begin
    ...
    begin
      ...
      InflateRect(CR, -4, 0);
      DrawTBXCaption(Canvas.Handle, CR, SimpleText, Flags, ISF_STATUSCOLOR or CEnabledState[Enabled]);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          CurrentTheme.PaintStatusBar(Canvas.Handle, R, PartID);
        end;
        if UseSystemFont then Canvas.Font := ToolbarFont
        else Canvas.Font := Self.Font;
  ...
  if IsSizeGripVisible then
    CurrentTheme.PaintStatusBar(Canvas.Handle, GetGripperRect, SBP_GRIPPER);
end;
...
begin
  ...
  begin
    ...
    else DrawTBXImage(Canvas.Handle, R, Images, APanel.ImageIndex, ISF_DISABLED);
  end;
  Canvas.Brush.Style := bsClear;
  Flags := DT_SINGLELINE or DT_VCENTER or Alignments[APanel.Alignment] or Truncations[APanel.TextTruncation];
  DrawTBXCaption(Canvas.Handle, ARect, APanel.Caption, Flags, ISF_STATUSCOLOR or EnabledState[APanel.Enabled]);
  Canvas.Brush.Style := bsSolid;
end;

procedure TTBXCustomStatusBar.ReadSimpleTextProperty(Reader: TReader);
begin
  SimpleText := FilerReadWideString(Reader);
end;
...
procedure TTBXCustomStatusBar.SetSimpleText(const Value: WideString);
begin
...
procedure TTBXCustomStatusBar.WriteSimpleTextProperty(Writer: TWriter);
begin
  FilerWriteWideString(Writer, SimpleText);
end;
