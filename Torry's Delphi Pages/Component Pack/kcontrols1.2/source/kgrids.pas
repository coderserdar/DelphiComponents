...
{$ENDIF}
  ...
  {$ENDIF}
    ...
    function PaintCells(ACanvas: TCanvas; CellBitmap: TBitmap;
      ...
      MaxY: Integer; Printing, PaintSelection: Boolean; const BlockRect: TRect): TPoint;
...
var
{$IFDEF USE_THEMES}
 {$IF DEFINED(COMPILER9_UP) OR DEFINED(FPC)}
  {$IFDEF FPC}
  SelectionTheme: TThemedElement;
  Details: TThemedElementDetails;
  {$ELSE}
...
begin
  ...
  if EditorMode and CellInGridRect(Col, Row, FSelection) then
    FEditor.Invalidate;
end;
...
begin
  R := InternalExpandGridRect(FSelection);
  NormalizeGridRect(R);
    ...
    begin
      ...
      try
        ...
        if ABlockRect <> nil then
          ...
          TmpBlockRect := TmpRect;          
        if CellBitmap <> nil then
          OffsetRect(TmpBlockRect, -R.Left, -R.Top);
        if (ACanvas = nil) or (ACanvas = Canvas) then
        begin
          Info := GetAxisInfoBoth([aiFixedParams]);
          SelectClipRect(Canvas.Handle, Rect(Info.Horz.FixedBoundary, Info.Vert.FixedBoundary, Info.Horz.ClientExtent, Info.Vert.ClientExtent));
        end;
...
//    CellBitmap := nil;
  try
    GridRectToRect(Selection, TmpRect, True, goRangeSelect in FOptions);
...
begin
  ...
  try
    ...
    try
      ...
      IsSelection := GridRectToRect(Selection, BlockRect, True, goRangeSelect in FOptions);
      if FOptions * [goFixedHorzLine, goHorzLine] = [goFixedHorzLine, goHorzLine] then
        Dec(BlockRect.Bottom, GetEffectiveRowSpacing(Max(Selection.Row1, Selection.Row2)));
      if FOptions * [goFixedVertLine, goVertLine] = [goFixedVertLine, goVertLine] then
        Dec(BlockRect.Right, GetEffectiveColSpacing(Max(Selection.Col1, Selection.Col2)));
      ...
      begin
        ...
        begin
          TmpExtent := PaintCells(ACanvas, CellBitmap, CurClipRgn, FTopLeft.Col, FColCount - 1, FTopLeft.Row, FRowCount - 1,
            Info.Horz.FixedBoundary - FScrollOffset.X, Info.Vert.FixedBoundary - FScrollOffset.Y, ClientW, ClientH, False, True, BlockRect);
        end;
      end;
      ...
      begin
        ...
        if ExtSelectClipRectEx(DC, TmpRect, RGN_AND, CurClipRgn, MainClipRgn) then
          TmpExtent := PaintCells(ACanvas, CellBitmap, CurClipRgn, FTopLeft.Col, FColCount - 1, 0, FFixedRows - 1,
            Info.Horz.FixedBoundary - FScrollOffset.X, 0, ClientW, ClientH, False, True, BlockRect);
      end;
      GridW := Max(GridW, TmpExtent.X); GridH := Max(GridH, TmpExtent.Y);
      ...
      begin
        ...
        if ExtSelectClipRectEx(DC, TmpRect, RGN_AND, CurClipRgn, MainClipRgn) then
          TmpExtent := PaintCells(ACanvas, CellBitmap, CurClipRgn, 0, FFixedCols - 1, FTopLeft.Row, FRowCount - 1, 0,
            Info.Vert.FixedBoundary - FScrollOffset.Y, ClientW, ClientH, False, True, BlockRect);
      end;
      GridW := Max(GridW, TmpExtent.X); GridH := Max(GridH, TmpExtent.Y);
      ...
      begin
        ...
        if ExtSelectClipRectEx(DC, TmpRect, RGN_AND, CurClipRgn, MainClipRgn) then
          ...
          0, 0, ClientW, ClientH, False, True, BlockRect);
      end;
      GridW := Max(GridW, TmpExtent.X); GridH := Max(GridH, TmpExtent.Y);
    finally
    ...
    if not (csDesigning in ComponentState) and (goDrawFocusSelected in FOptions) and
      (FOptions * [goRangeSelect, goRowSelect] <> []) and Focused and not EditorMode and 
      IsSelection then
    begin
      // to ensure coming DrawFocusRect will be painted correctly:
      SetBkColor(DC, $FFFFFF);
      SetTextColor(DC, 0);
      ACanvas.DrawFocusRect(BlockRect);
    end;
...
procedure TKCustomGrid.UpdateEditor(Show: Boolean);
  ...
  begin
    ...
    if (FEditorCell.Col >= FTopLeft.Col) and ((FScrollOffset.X = 0) or (FEditorCell.Col > FTopLeft.Col)) or
      (gxEditFixedCols in FOptionsEx) and (FEditorCell.Col < FFixedCols) or
      (FEditorCell.Row >= FTopLeft.Row) and ((FScrollOffset.Y = 0) or (FEditorCell.Row > FTopLeft.Row)) or
      (gxEditFixedRows in FOptionsEx) and (FEditorCell.Col < FFixedCols) and (FEditorCell.Row < FFixedRows) then
    begin
      if CallResize then
