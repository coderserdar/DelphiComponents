...
type
  TColorDBComboBox = class(TDBComboBox)
  private
...
procedure TColorDBComboBox.CMEnter(var Message: TCMEnter);
begin
  Invalidate;
  inherited;
end;

procedure TColorDBComboBox.CMExit(var Message: TCMExit);
begin
  Invalidate;  
  inherited;
end;

destructor TColorDBComboBox.Destroy;
begin
  if Assigned(ColorPresets) then ColorPresets.DeleteOwner(self);
  inherited;
end;

procedure TColorDBComboBox.DrawItem(Index: Integer; Rect: TRect;
...
function TColorDBComboBox.GetColorPresets: TColorPresets;
begin
   Result := fColorPresets;
end;

procedure TColorDBComboBox.Notification(AComponent: TComponent;
...
procedure TColorDBComboBox.SetColorPresets(const Value: TColorPresets);
begin
