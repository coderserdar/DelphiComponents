...
//**************************************************************************
//    FRecordCount := FLookupLink.RecordCount;// Commented by Alexey Kogosov (Scroll bar fix)
//***************************************************************************
    FRecordCount := FLookupLink.DataSet.RecordCount;// Added by Alex Alexey Kogosov (Vert Scroll bar Fix)
...
begin
  if FLookupLink.Active then begin
    ...
    Max := 0;
//***********************************************************************************
// if FRecordCount = (FRowCount - Ord(EmptyRowVisible)) then begin // Commented by Alexey Kogosov (Scroll bar fix)
//*******************************************************************************
    if FRecordCount >= (FRowCount - Ord(EmptyRowVisible)) then begin// Added by Alexey Kogosov (Scroll bar fix) 
      Max := 4;
...
// Commented by Alexey Kogosov (Scroll bar fix)*********************************
//    if Max <> MaxPos then SetScrollRange(Handle, SB_VERT, 0, Max, False);
//******************************************************************************
    if Max <> MaxPos then SetScrollRange(Handle, SB_VERT, 0, Max, True);// Added by Alexey Kogosov (Scroll Bar fix)
...
begin
  if not FListVisible and {FListActive} CanModify then begin
    ...
    Y := P.Y + Height;
// Commented by Alexey Kogosov (Drop down direction fix)************************
//    if Y + FDataList.Height = Screen.Height then
//******************************************************************************
    if Y + FDataList.Height >= Screen.Height-10 {Win XP} then // Added by Alexey Kogosov (Drop down direction fix)
