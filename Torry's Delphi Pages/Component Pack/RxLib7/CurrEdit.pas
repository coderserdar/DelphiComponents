...
type
 TOnValue=procedure(Sender: TObject;var Value: Extended) of Object;// Added by Alex
{ TCustomNumEdit }
  ...
  private
    ...
    fOnSetValue,fOnGetValue: TOnValue;
...
{$ENDIF}
  ...
  public
    ...
    property OnSetValue: TOnValue read fOnSetValue write fOnSetValue;
    property OnGetValue: TOnValue read fOnGetValue write fOnGetValue;// Added by Alex
  end;
...
begin
  ...
  if Assigned(OnGetValue) then fOnGetValue(Self,FValue);// Added by Alex    
  Result := FValue;
end;

procedure TCustomNumEdit.SetValue(AValue: Extended);
begin
  if Assigned(OnSetValue) then fOnSetValue(Self,AValue);// Added by Alex
