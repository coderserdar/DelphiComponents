...
begin
  ...
  begin
    //SAARIXX
    celx := GAMMA_TABLE[255-flrx{ xor 255}];
    cely := GAMMA_TABLE[255-flry{ xor 255}];
    ...
    //SAARIXX
    P^:=MergeRegEx(C, P^, DivTable[celx,cely]); Inc(P);
    P^:=MergeRegEx(C, P^, DivTable[flrx,cely]); Inc(P, FWidth);
    P^:=MergeRegEx(C, P^, DivTable[flrx,flry]); Dec(P);
    P^:=MergeRegEx(C, P^, DivTable[celx,flry]);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      if (X >= Left) and (Y >= Top) then
       P^:=MergeRegEx(C{MergeReg(C, P^)}, P^, DivTable[celx,cely]);
      Inc(P);
      if (X < Right - 1) and (Y >= Top) then
       P^:=MergeRegEx(C{MergeReg(C, P^)}, P^, DivTable[flrx,cely]);
      Inc(P, FWidth);
      if (X < Right - 1) and (Y < Bottom - 1) then
       P^:=MergeRegEx(C{MergeReg(C, P^)}, P^, DivTable[flrx,flry]);
      Dec(P);
      if (X >= Left) and (Y < Bottom - 1) then
       P^:=MergeRegEx(C{MergeReg(C, P^)}, P^, DivTable[celx,flry]);
    end;
  end;
end;
  ...
  try
    ...
    if L then
     begin
      if hyp>=$10000 then
       begin
        Inc(nx,nx div (hyp shr 16));
        Inc(ny,ny div (hyp shr 16));
       end;
      Inc(hyp, 65536);
     end;
...
procedure TBitmap32.LineToS(X, Y: Integer; L: Boolean=False); //SAARIXX
begin
  LineS(RasterX, RasterY, X, Y, PenColor, L);
  RasterX := X;
  RasterY := Y;
end;

procedure TBitmap32.LineToTS(X, Y: Integer; L: Boolean=False); //SAARIXX
begin
  LineTS(RasterX, RasterY, X, Y, PenColor, L);
  RasterX := X;
  RasterY := Y;
end;

procedure TBitmap32.LineToAS(X, Y: Integer; L: Boolean=False); //SAARIXX
begin
  LineAS(RasterX, RasterY, X, Y, PenColor, L);
  RasterX := X;
  RasterY := Y;
end;
...
procedure TBitmap32.LineToXS(X, Y: TFixed; L:Boolean=False); //SAARIXX
begin
  LineXS(RasterXF, RasterYF, X, Y, PenColor, L);
  RasterXF := X;
  RasterYF := Y;
end;

procedure TBitmap32.LineToFS(X, Y: Single; L: Boolean=False); //SAARIXX
begin
  LineToXS(Fixed(X), Fixed(Y), L);
end;

procedure TBitmap32.LineToXSP(X, Y: TFixed; L:Boolean=False); //SAARIXX
begin
  LineXSP(RasterXF, RasterYF, X, Y, L);
  RasterXF := X;
  RasterYF := Y;
end;

procedure TBitmap32.LineToFSP(X, Y: Single; L: Boolean=False); //SAARIXX
begin
  LineToXSP(Fixed(X), Fixed(Y), L);
end;
