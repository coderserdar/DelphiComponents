...
type
  ...
  // Loading from stream (Structure and data would be loaded from stream 'as is'  - with no respect to current structure).
  ...
  try
    ...
    if(tmpCancel) then begin
      ...
      if(AExceptionClass = ERxMemoryDataReadError) then
        tmp := ' Loading ';
      raise AExceptionClass.Create(tmp + ' of data was aborted.' (* '???????' + tmp + '???????.'*) );
    end;
  finally
    tmp := '';
  end;
...
begin
 ...
 // Loading from stream (Structure and data would be loaded from stream 'as is'  - with no respect to current structure). // Exceptions are rased for errors.
