    ...
    if Result then
      repeat
        if (SearchRec.Name <> '.') and (SearchRec.Name <> '..') and
          {$IFNDEF WIN32}(SearchRec.Attr and faVolumeID <> faVolumeID) and {$ENDIF}
...
//      !!! BUG !!!
      if (FileInfo.Name <> '.') and (FileInfo.Name <> '..') {$IFNDEF WIN32}and (FileInfo.Attr <> faVolumeID){$ENDIF} then
      begin
        if (FileInfo.Attr and faDirectory = faDirectory) then
          Result := ClearDir(NormalDir(Path) + FileInfo.Name, Delete) and Result
        else {$IFNDEF WIN32}if (FileInfo.Attr and faVolumeID <> faVolumeID) then{$ENDIF}
        begin
