...
//	    ChangeBitmapColor(TmpImage,ColorToRGB(}TransparentColor),clBtnFace);
    ...
    with TmpImage.Canvas do
      ...
      begin
        ...
        if iTileBitmapOption = ftoPropStretch then begin
...
begin
  ...
  begin
    case AutoTrColor of
      ftpfLeftTopPixel: begin x := 0; y := 0; end;
      ftpfLeftBottomPixel: begin x := 0; y := Bitmap.Height - 1; end;
      ftpfRightTopPixel: begin x := Bitmap.Width - 1; y := 0; end;
...
begin
  ...
  if FHighlightButton then Cursor := {$IFDEF RX_D3}crHandPoint{$ELSE}crUpArrow{$ENDIF} else Cursor := crDefault;
  Repaint;
end;
...
begin
  ...
  FCombineMode := Value; DoUpdateRegion;
end;
