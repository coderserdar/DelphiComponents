...
uses Classes, SysUtils, DB, RxHintProp, ColnEdit,
...
uses TypInfo, RXResConst, RXDBCtrl, RXLookup, RxLogin, RXDBComb, RxVCLUtils,
  {$IFNDEF RX_D3} DBTables, {$ENDIF} {$IFDEF DCS} RxSelDSFrm, {$ENDIF}
  {$IFDEF RX_D3} RxMemDS, {$ENDIF} {$IFDEF WIN32} RxDBRichEd, {$ENDIF}
...
begin
  if (GetComponent(0) is TRow) then begin
...
{ Data aware components and controls }
  RegisterComponents(srRXDBAware, [
    ...
    TDBStatusLabel, TRxDBComboBox{$IFDEF RX_D4}, TRxDBRecordGrid{$ENDIF}]);
  RegisterComponents(RxLoadStr(srRXTools), [TRxLoginDialog]);
{$IFDEF RX_D3}
