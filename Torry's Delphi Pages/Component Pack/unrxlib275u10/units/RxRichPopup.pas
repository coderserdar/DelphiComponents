...
uses
  Classes, Graphics, Dialogs, Menus, ComCtrls, RxRichEd, ExtDlgs;
...
uses {RxDBRichEd, DBCtrls, DB, }Clipbrd, Windows, Messages{, jpeg, RxGIF};
...
begin
  ...
  if not (csDesigning in Componentstate) then begin
    ...
    if not Assigned(OpenPictureDialog) then begin
      OpenPictureDialog := TOpenPictureDialog.Create(self);
    end;

    if Assigned(OpenPictureDialog) then begin
...
begin
  canEdit := true;
{$IFNDEF RX_D3}
  if PopupComponent is TRXDBRichEdit
    then RichEdit := TRxDBRichEdit(PopupComponent)
  else RichEdit := TRxRichEdit(PopupComponent);
  if RichEdit is TRXDBRichEdit then
    with TRXDBRichEdit(RichEdit)
      do
    begin
...
{$ELSE}
  if not Assigned(RichEdit)
    then RichEdit := TRxRichEdit(PopupComponent);
{$ENDIF}
