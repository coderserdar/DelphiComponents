...
{                                                       }
{ Adopted from Delphi VCL Library                       }
...
const
  StringProps   = [{$IFDEF UNICODE}tkWString, tkUString{$ELSE}tkString, tkLString{$ENDIF}];
  ClassProps    = [tkClass];
  DefDelimeter  = '.';
...
begin
  ...
  if (ListIndexOf(FTransList, Instance) >= 0) or (Instance is TComponent)
    and (Instance <> FRoot) and (ListIndexOf(FTransList, TComponent(Instance).Owner) < 0)
  then
    Exit;
