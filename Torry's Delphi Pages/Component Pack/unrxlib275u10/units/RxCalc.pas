...
uses {$IFNDEF WIN32} RxStr16, {$ENDIF} RxVCLUtils, RxMaxMin, RxToolEdit,
...
begin
  ...
  try
    ...
    else if Kind = cbDcm then Caption := {$IFDEF RX_D15}FormatSettings.{$ENDIF}DecimalSeparator
...
begin
  ...
  if CharInSet(Key, [{$IFDEF RX_D15}FormatSettings.{$ENDIF}DecimalSeparator, '.', ',']) then begin
    CheckFirst;
...
var
  ...
  BtnTag: Longint;
begin
  if CharInSet(Key, [{$IFDEF RX_D15}FormatSettings.{$ENDIF}DecimalSeparator, '.', ',']) then Key := '.'
