...
uses Consts, {$IFNDEF WIN32} RxStr16, {$ENDIF} RxVCLUtils, RxAniFile, RxConst, RxStrUtils,
...
begin
  ...
  SetLength(S, 3);
  Stream.Read(S[1], 3);
  if CompareText(GIFSignature, string(S)) <> 0 then GifError(RxLoadStr(SGIFVersion));
  SetLength(S, 3);
  Stream.Read(S[1], 3);
  for I := Low(TGIFVersion) to High(TGIFVersion) do
    if CompareText(string(S), string(StrPas(GIFVersionStr[I]))) = 0 then begin
      FVersion := I;
      Break;
    end;
