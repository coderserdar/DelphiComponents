...
uses SysUtils, Dialogs, {$IFDEF WIN32} CommCtrl, {$ELSE} RxStr16, {$ENDIF}
...
procedure TRxProgress.Paint;
  function DiameterColor(Color1, Color2: TColor): TColor;
  var  //JB.
    r, g, b: Byte;
  begin             
...
//            col := col shl 8;
          ...
          Brush.Color := Color;
          BrushCopy(rect(0, 0, i + FPos, h), bmpText,
            rect(0, 0, i + FPos, h), col);
          Brush.Color := Font.Color;
          BrushCopy(rect(i + FPos, 0, w, h), bmpText,
            rect(i + FPos, 0, w, h), col);
        finally
          bmpText.Free;
        end;
      end
    ...
    { show resultat }
    Canvas.Copyrect(rect(0, 0, w + 1, h + 1), lBMP.Canvas,
      rect(0, 0, w + 1, h + 1));
  finally
    lBMP.Free;
  end;
...
{$ENDIF}
 ...
 {$IfDef win32} InsertHintCell(Result); {$EndIf}
end;
...
{$ENDIF}
  ...
  {$IfDef win32} InsertHintCell(Index); {$EndIf}
end;
...
begin
  ListBox.ResetContent;  {$IfDef win32} DropHints; {$EndIf}
end;
...
begin
  ...
  {$IfDef win32} <> LB_ERR then if Fitems is TRxListBoxstrings
    then (FItems as TRxListBoxstrings ).DeleteHintCell(Index) {$EndIf} ;
end;
...
{$IfDef Win32} TempHint1, TempHint2 :string;  {$EndIf}
begin
...
{$IfDef Win32} TempHint :string;  {$EndIf}
begin
  with TRxCheckListBox(ListBox) do begin
{$IfDef Win32} TempHint := ItemHint[CurIndex]; {$EndIf}
    TempState := State[CurIndex];
    TempEnabled := EnabledItem[CurIndex];
    inherited Move(CurIndex, NewIndex);
    State[NewIndex] := TempState;
    EnabledItem[NewIndex] := TempEnabled;
{$IfDef Win32} ItemHint[NewIndex] := TempHint; {$EndIf}
  end;
end;
