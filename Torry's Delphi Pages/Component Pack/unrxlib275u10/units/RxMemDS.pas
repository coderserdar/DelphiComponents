...
uses Windows, SysUtils, Classes, Controls, DB, RxDBUtils;
...
begin
  ...
  for I := 0 to BlobFieldCount - 1 do
    {$IFDEF RX_D12}TBlobDataArray{$ELSE}PMemBlobArray{$ENDIF}(Buffer + FBlobOfs)[I] := {$IFDEF RX_D12}nil{$ELSE}''{$ENDIF};
end;
...
begin
  ...
  fieldNameEnd := false;
  i := 1; s := '';
  while (i <= Length(fSortFields)) do begin
    ...
    if ((i + 1) > length(fSortFields)) or fieldNameEnd then begin
   {s = s.strip(HString::both).to_lower();}
      if s <> '' then begin
        if LowerCase(s) = whatToSearch then begin
          result := true;
          exit;
        end
      end;
...
//type TDataSetState = (dsInactive, dsBrowse, dsEdit, dsInsert, dsSetKey, dsCalcFields, dsFilter, dsNewValue, dsOldValue, dsCurValue, dsBlockRead, dsInternalCalc, dsOpening);
  ...
  if Filtered and Active then begin
    i := 0;
