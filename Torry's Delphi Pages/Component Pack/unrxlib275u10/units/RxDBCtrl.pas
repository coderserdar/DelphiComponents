...
{$ENDIF}
  ...
  if ((Key = VK_NEXT) and not DataLink.DataSet.EOF)
    or ((Key = VK_PRIOR) and not DataLink.DataSet.BOF)
  then
    Invalidate;
end;
  ...
  if not Result then
    if (not FInReset) and FDataLink.Editing then try
      if (Value <> NullDate) then begin
        ...
        then raise Exception.CreateFmt(RxLoadStr(SDateOutOfRange),[FormatDateTime(GetDateFormat, Value),FormatDateTime(GetDateFormat, MinDate), FormatDateTime(GetDateFormat, MaxDate)])
        else if ((MinDate <> NullDate) and (Value < MinDate))
          then  raise Exception.CreateFmt(RxLoadStr(SDateOutOfMin),[FormatDateTime(GetDateFormat, Value),FormatDateTime(GetDateFormat,MinDate)])
          else if ((MaxDate <> NullDate) and (Value > MaxDate))
            then  raise Exception.CreateFmt(RxLoadStr(SDateOutOfMax),[FormatDateTime(GetDateFormat, Value),FormatDateTime(GetDateFormat,MaxDate)]);
      end;
      Result := True;
    except
      Reset;
...
begin
  ...
  else Result := RxLoadStr(StrIds[Kind]);
end;
