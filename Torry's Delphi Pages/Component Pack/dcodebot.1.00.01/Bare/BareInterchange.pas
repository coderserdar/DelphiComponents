...
type
  ...
  public
    ...
    function Next(Parent: PInterchangeNode; const Tag: string): PInterchangeNode;
    procedure Select(Node: PInterchangeNode);
...
function ReadVint4(Stream: TStream): Longword;
function ReadUint4(Stream: TStream): Longword;
function ReadUint2Flags(Stream: TStream): Word;
...
function ReadTag(Stream: TStream; const Tag: string): Boolean; overload;
function ReadTag(Stream: TStream): string; overload;
...
begin
  CreateFromStream(TFileStream.Create(FileName, fmOpenRead), True);
end;
...
begin
  ...
  FRoot.Size := ReadUint4(FStream) - 4;
...
begin
  ...
  try
    ...
    begin
      ...
      Child.Index := 0;
      InterchangeCheck(FStream.Seek(Child.Size, soFromCurrent) = Position + Child.Size);
      Inc(Position, Child.Size);
    end;
  except
    RemoveChildren(Node);
    raise;
...
function TInterchangeFile.Next(Parent: PInterchangeNode; const Tag: string): PInterchangeNode;
var
  S: string;
  I: Integer;
begin
  Result := nil;
  S := UpperCase(Tag);
  for I := Parent.Index to Length(Parent.Children) do
    if Parent.Index = Length(Parent.Children) then
    begin
      Parent.Index := I;
      Break;
    end
    else if Parent.Children[Parent.Index].Tag = S then
    begin
      Result := Parent.Children[Parent.Index];
      Parent.Index := I + 1;
      Break;
    end
    else
...
procedure TInterchangeFile.Select(Node: PInterchangeNode);
begin
  FStream.Seek(Node.Offset, soFromBeginning);
  Node.Index := 0;
end;
...
begin
  ...
  if A < $FF00 then
    ...
    Result := (A and $FF) shl $10 or ReadUint2(Stream);
end;
...
function ReadTag(Stream: TStream): string;
begin
  Result := UpperCase(ReadText(Stream, TagLength));
end;
