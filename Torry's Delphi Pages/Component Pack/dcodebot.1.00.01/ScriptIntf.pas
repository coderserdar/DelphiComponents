...
type
   IActiveScriptError = interface(IUnknown)
     ...
     function GetSourceLineText(SourceLine: PWideChar): HResult; stdcall;
   end;
...
{ IActiveScriptSite interface }
  ...
  IActiveScriptSite = interface(IUnknown)
    ...
    function OnScriptTerminate(const varResult: Variant;
...
{ IActiveScriptSiteWindow interface }
  ...
  protected
    ...
    function OnScriptTerminate(const varResult: Variant;
      const ExcepInfo: TExcepInfo): HResult; stdcall;
    function OnStateChange(ScriptState: TScriptState): HResult; virtual; stdcall;
    function OnScriptError(ScriptError: IActiveScriptError): HResult; virtual; stdcall;
...
constructor TScriptSite.Create(Wnd: HWND);
begin
  inherited Create;
  FWnd := Wnd;
end;
...
function TScriptSite.GetItemInfo(Name: PWideChar; ReturnMask: DWORD;
  out unkItem: IUnknown; out Info: ITypeInfo): HResult;
begin
  Result := E_INVALIDARG;
end;
...
function TScriptSite.OnScriptTerminate(const varResult: Variant;
  const ExcepInfo: TExcepInfo): HResult;
begin
  Result := S_OK;
end;

function TScriptSite.OnStateChange(ScriptState: TScriptState): HResult;
begin
  Result := S_OK;
end;
...
begin
  EnableWindow(FWnd, Enable);
  Result := S_OK;
end;

{procedure TForm1.FormCreate(Sender: TObject);
begin
  FScript := CreateComObject(CLSID_VBScript) as IActiveScript;
  FScript.SetScriptSite(TScriptSite.Create(Handle) as IActiveScriptSite);
end;
...
begin
  FScript.QueryInterface(IActiveScriptParse, Parser);
  Parser.InitNew;
  Parser.ParseScriptText('MsgBox "It is cold outside"'#13#10, nil, nil, nil, 0,
    0, 0, nil, nil);
  FScript.SetScriptState(SCRIPTSTATE_CONNECTED);
end;}
