// ------------------------------------------------------------------------------
// DPF.Android.DesignTime helper functions
//
...
procedure BitmapAsBorder (Control:TDPFANDBaseControl; ABitmap:TBitmap; BorderSize:integer; const FillColor:TAlphaColor=TAlphaColors.Null);
procedure BitmapAsBorderToRect (Control:TDPFANDBaseControl; ABitmap:TBitmap; BorderSize:integer; ARect:TRectF; const FillColor:TAlphaColor=TAlphaColors.Null);
procedure BitmapAsBackground (Control:TDPFANDBaseControl; ABitmap:TBitmap);
procedure BitmapToPosition (Control:TDPFANDBaseControl; ABitmap:TBitmap; X, Y:single);
procedure BitmapToRect (Control:TDPFANDBaseControl; ABitmap:TBitmap; R:TRectF);
procedure PaintCaption (Control:TDPFANDBaseControl; Caption:string; CaptionRect:TRectF; LineBreak:TDPFLineBreak; NumberOfLines:integer; Alignment: TTextAlign);
//function  GetWidthOfToolBarButton (Button:TUniversalToolBarItem; IncludeMargin:boolean):single;
//procedure PaintToolBar (Control:TDPFANDBaseControl; BarItems:TArrayOfUniversalToolBarItem);
  ...
  begin
    ...
    if FillColor<>TAlphaColors.Null then
      begin
        ...
        Control.Canvas.Fill.Kind:=TBrushKind.bkSolid;
        Control.Canvas.Fill.Color:=FillColor;
        Control.Canvas.FillRect(R,0,0,AllCorners,1,TCornerType.ctRound);
      end;
  end;
    ...
    case LineBreak of
      lbWordWrap, lbCharacterWrap:
        begin
          ...
          Control.Canvas.FillText( CaptionRect, Caption, NumberOfLines > 1, 1, [], Alignment, TTextAlign.taCenter );
        end;
      lbClip:
        begin
          if NumberOfLines=0 then //No line limit
            Control.Canvas.FillText( CaptionRect, Caption, true, 1, [], Alignment, TTextAlign.taCenter );
          if NumberOfLines=1 then //1 line
            Control.Canvas.FillText( CaptionRect, Caption, false, 1, [], Alignment, TTextAlign.taCenter );
      ...
      lbHeadTruncation:
        begin
          ...
          Control.Canvas.FillText( CaptionRect, s, NumberOfLines<>1, 1, [], Alignment, TTextAlign.taCenter );
        end;
      lbTailTruncation:
        begin
          s := Caption;
...
function FMTextAlign(TextAlign:TDPFTextAlignment):TTextAlign;
  begin
    ...
    case TextAlign of
      taLeft:   Result:=TTextAlign.taLeading;
      taCenter: Result:=TTextAlign.taCenter;
      taRight:  Result:=TTextAlign.taTrailing;
    end;
  end;
initialization
{$IFNDEF ANDROID}
