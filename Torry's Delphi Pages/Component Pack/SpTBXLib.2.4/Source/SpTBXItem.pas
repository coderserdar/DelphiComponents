...
To Do:
  ...
  - Toolbar.Stretch property doesn't work, I don't know how to fix it
    without changin the sources of TB2K.
...
type
  ...
  protected
    ...
    procedure InternalDrawBackground(ACanvas: TCanvas; ARect: TRect);
...
procedure SpDrawXPStatusBar(ACanvas: TCanvas; ARect, AGripRect: TRect; SkinType: TSpTBXSkinType; UsingOfficeTheme: Boolean);
...
procedure SpDrawXPToolbar(W: TTBCustomDockableWindow; ACanvas: TCanvas; ARect: TRect; PaintOnNCArea: Boolean; PaintBorders: Boolean = True; SkinComponent: TSpTBXSkinComponentsType = skncToolbar);
...
const
  DefaultToolbarBorderSize = 2;       // Default size of Floating Toolbar borders
...
begin
  ...
  IsPushed := IsHoverItem and (IsOpen or (View.MouseOverSelected and View.Capture and IV.IsToolbarStyle));
  ...
  ItemInfo.ImageOrCheckShown := ImageIsShown or (not IV.IsToolbarStyle and Item.Checked);
  ...
  if Assigned(View.ParentItem) then begin
    if View.ParentItem is TSpTBXCustomItem then
      ItemInfo.IsOnToolbox := TSpTBXCustomItem(View.ParentItem).ToolBoxPopup
    else
      if View.ParentItem is TSpTBXRootItem then
        ItemInfo.IsOnToolbox := TSpTBXRootItem(View.ParentItem).ToolBoxPopup;
  end;
  ...
  ItemInfo.ToolbarStyle := IV.IsToolbarStyle;
...
procedure SpDrawXPMenuItem(ACanvas: TCanvas; ARect: TRect; ItemInfo: TSpTBXMenuItemInfo);
  ...
  begin
    ForceRectBorders := [];
    if ItemInfo.IsSplit then
      if ItemInfo.ComboPart = cpSplitLeft then
        ForceRectBorders := [akRight]
      else
        ForceRectBorders := [akLeft];      

    case ItemInfo.SkinType of
      sknNone:
        if ItemInfo.Checked then begin
          if not ItemInfo.HotTrack or (not ItemInfo.Enabled and ItemInfo.ToolbarStyle) then begin
      ...
      sknWindows:
        if ItemInfo.IsDesigning then
          ...
          if ItemInfo.IsOnMenuBar then begin
            ...
            DrawThemeBackground(ThemeServices.Theme[teToolBar], ACanvas.Handle, XPPart[ItemInfo.ComboPart], Flags, ARect, nil);
          end;
      sknSkin:
        if ItemInfo.IsOpen and CurrentSkin.OfficePopup then begin
          ...
          if ItemInfo.IsDesigning then
            ...
            if ItemInfo.IsOnMenuBar then
              ...
              CurrentSkin.PaintBackground(ACanvas, ARect, skncToolbarItem, ItemInfo.State, True, True, False, ForceRectBorders);
    end;
  end;

  procedure MenuItemDraw(ARect: TRect);
  var
    MenuState: TSpTBXSkinStatesType;
  begin
    MenuState := sknsHotTrack; // other states are not painted
    ...
    case ItemInfo.SkinType of
      ...
      sknWindows:
        if SpIsWinVista then begin
          ...
          DrawThemeBackground(ThemeServices.Theme[teMenu], ACanvas.Handle, MENU_POPUPITEM, MPI_HOT, ARect, nil);
        end
        else
          SpFillRect(ACanvas, ARect, clHighlight);
      sknSkin:
        CurrentSkin.PaintBackground(ACanvas, ARect, skncMenuItem, MenuState, True, True);
    end;
  end;
...
begin
  if ItemInfo.ToolbarStyle then begin // Toolbar Item
    ...
    if (ItemInfo.SkinType = sknSkin) or (SpIsWinVista and (ItemInfo.SkinType = sknWindows)) then
    ...
    // Draw the item selection rectangle
    ...
    if DrawCheckBoxFrame then begin
      ...
      if SpIsWinVista and (ItemInfo.SkinType = sknWindows) then begin
        // Use the new API on Windows Vista
...
begin
  ...
  case SkinManager.GetSkinType of
    ...
    sknWindows:
      if MenuItemStyle and SpIsWinVista then begin
...
begin
  ...
  if (SkinType = sknWindows) and not SpIsWinVista then
    SkinType := sknNone;
    ...
    sknWindows: // Only Windows Vista painting, XP just fills the background
      begin
        ...
        if DrawGutter then begin
          ...
          GutterR.Right := GutterR.Left + MarginsInfo.GutterSize + MarginsInfo.LeftCaptionMargin + 3; // +3 because the popup has 2 pixel border and the gutter separator has 2 pixel
          DrawThemeBackground(ThemeServices.Theme[teMenu], ACanvas.Handle, MENU_POPUPGUTTER, 0, GutterR, nil);
        end;
      end;
    sknSkin:
      begin
        ...
        if DrawGutter and not CurrentSkin.Options(skncGutter, sknsNormal).IsEmpty then begin
          ...
          CurrentSkin.PaintBackground(ACanvas, GutterR, skncGutter, sknsNormal, True, True);
        end;
      end;
  end;
end;
  ...
  case SpTBXSkinType(SkinType) of
    ...
    sknSkin:
      begin
        ...
        if UsingOfficeTheme and not IsRectEmpty(ARect) then begin
          Inc(ARect.Top);
...
var
  R: TRect;
  PaintDefault, DrawSkinBody, DrawSkinBorders: Boolean;
  TD: TSpTBXDock;
  Toolbar: TTBCustomDockableWindowAccess;
  IsMenuBar, IsFloatingNoBody, IsVertical: Boolean;
begin
  ...
  DrawSkinBody := True;
  DrawSkinBorders := True;

  if Toolbar.Docked then begin
    // Try to use OnDrawBackground to paint the background
    if Toolbar.CurrentDock is TSpTBXDock then begin
      TD := TSpTBXDock(Toolbar.CurrentDock);
      if Assigned(TD.OnDrawBackground) then begin
        PaintDefault := True;
        TD.OnDrawBackground(TD, ACanvas, ARect, pstPrePaint, PaintDefault);
        if not PaintDefault then
          Exit;
      end;
    end;
    ...
    if not PaintOnNCArea then begin
      OffsetRect(R, -DefaultToolbarBorderSize, -DefaultToolbarBorderSize);
      if IsVertical then
        Dec(R.Top, SpGetDragHandleSize(Toolbar))
      else
        Dec(R.Left, SpGetDragHandleSize(Toolbar));
    end;
    if (Toolbar.CurrentDock is TSpTBXDock) or SpIsDockUsingBitmap(Toolbar.CurrentDock) then
      TTBDockAccess(Toolbar.CurrentDock).DrawBackground(ACanvas.Handle, R);
  ...
  PaintDefault := True;
  IsMenuBar := False;
  IsFloatingNoBody := False;
  if W is TSpTBXToolbar then begin
    IsMenuBar := TSpTBXToolbar(W).MenuBar;
    if IsMenuBar then
      IsFloatingNoBody := Toolbar.Floating and CurrentSkin.Options(skncMenuBar, sknsNormal).Body.IsEmpty
    else
      IsFloatingNoBody := Toolbar.Floating and CurrentSkin.Options(SkinComponent, sknsNormal).Body.IsEmpty;
    TSpTBXToolbar(W).DoDrawBackground(ACanvas, ARect, pstPrePaint, PaintDefault);
  end
  else
    if W is TSpTBXCustomToolWindow then begin
      IsFloatingNoBody := Toolbar.Floating and CurrentSkin.Options(skncToolbar, sknsNormal).Body.IsEmpty;
  ...
  if PaintDefault then begin
    ...
    if Toolbar.Color <> clNone then begin
      ...
      case SkinManager.GetSkinType of
        sknNone:
          if PaintBorders and Toolbar.Docked then
            Windows.DrawEdge(ACanvas.Handle, R, BDR_RAISEDINNER, BF_RECT or BF_ADJUST);
        sknWindows:
          if PaintBorders and Toolbar.Docked then
            SpDrawRectangle(ACanvas, R, 0, SpLighten(clBtnFace, 24), SpLighten(clBtnFace, -32));
        sknSkin:
          if Toolbar.Docked or Toolbar.Floating then begin
            if IsFloatingNoBody then begin
              CurrentSkin.PaintBackground(ACanvas, R, skncDock, sknsNormal, True, False);
            end
            else
              if IsMenuBar then
                CurrentSkin.PaintBackground(ACanvas, R, skncMenuBar, sknsNormal, DrawSkinBody, DrawSkinBorders and PaintBorders, IsVertical)
              else
                CurrentSkin.PaintBackground(ACanvas, R, SkinComponent, sknsNormal, DrawSkinBody, DrawSkinBorders and PaintBorders, IsVertical);
          end;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    if ItemInfo.ToolbarStyle then begin
      ...
      if tbisClicksTransparent in Item.ItemStyle then begin
        ...
        // tbisClicksTransparent itemstyle (like a SpTBXLabelItem)
        TextInfo.TextFlags := DT_SINGLELINE or DT_CENTER or DT_VCENTER;
        CaptionRect := R;
      end
      else begin
  ...
  if ItemInfo.ToolbarStyle and (ItemInfo.RightImageSize.cx > 0) and (ItemInfo.RightImageSize.cy > 0) then begin
    if IsRectEmpty(RightImageRect) then begin
      ...
      RightImageRect.Top := (R.Top + R.Bottom - ItemInfo.RightImageSize.cy) div 2;
...
begin
  if not IsToolbarStyle then begin
    if CurrentSkin.OfficeMenuSeparator then
    ...
    case SkinManager.GetSkinType of
      ...
      sknWindows, sknSkin:
        begin
          ...
          if not (tboToolbarStyle in Item.EffectiveOptions) then
            if SpIsWinVista or not CurrentSkin.Options(skncGutter, sknsNormal).IsEmpty then begin
              if View.Window is TSpTBXPopupWindow then
...
begin
  Form := GetSystemMenuParentForm;
  if Assigned(Form) then
    SendMessage(Form.Handle, WM_SYSCOMMAND, TTBCustomItem(Sender).Tag, GetMessagePos);
end;
...
begin
  ...
  try
    ...
    if PaintDefault then
      InternalDrawBackground(ACanvas, DrawRect);
...
procedure TSpTBXDock.InternalDrawBackground(ACanvas: TCanvas; ARect: TRect);
begin
  if (csDestroying in ComponentState) then Exit;

  // Paint the Bitmap if it's assigned, or use the skin, or use the Color
  if UsingBitmap then
    inherited
  else
    if Color = clNone then begin
      case SkinManager.GetSkinType of
        ...
        sknSkin:
          CurrentSkin.PaintBackground(ACanvas, ARect, skncDock, sknsNormal, True, True, Position in [dpLeft, dpRight]);
      end;
    end
    else begin
...
begin
  ...
  // To check if the Dock is using a Bitmap use SpIsUsingBitmap instead
  Result := True;
end;
...
begin
  ...
  try
    View.ValidatePositions;
    IsRotated := SpIsVerticalToolbar(Self);
...
begin
  ...
  if (PaintStage = pstPrePaint) and PaintDefault then begin
    ...
    SpDrawXPStatusBar(ACanvas, ARect, G, SkinType, OfficeSeparators);
      ...
      if PaintDefault then begin
        ...
        if Active then begin
          ...
          if Assigned(FDock) and not Maximized and (SkinManager.GetSkinType = sknWindows) then begin
            DockAreaR := ARect;
            DockAreaR.Right := FDock.Width + (FloatingBorderSize.X * 2);
