...
type
  ...
  protected
    ...
    procedure Autofit;
    procedure BeginUpdateHidden;
    procedure EndUpdateHidden;
...
begin
  if Visible then begin
    ...
    if CanTabClose then begin
      ...
      NextTab := nil;
      if Checked and GetTabToolbar(T) then begin
        NextTab := GetNextTab(True, sivtInmediate);
        if not Assigned(NextTab) then
          NextTab := GetNextTab(False, sivtInmediate);
      end;
      Visible := False;
      DoTabClose;
      if CloseAndFree then
        Free;  // Removes the item from the paren, calls tbicDeleting notification and frees the item
...
begin
  ...
  if Action = tbicInvalidateAndResize then begin
    ...
    if (FUpdateHidden = 0) and not IsItemMoving then
      InvalidateNC;
  end;
end;

procedure TSpTBXTabToolbar.BeginUpdateHidden;
begin
  if FUpdateHidden < 0 then FUpdateHidden := 0;
  if FUpdateHidden = 0 then
    SendMessage(Handle, WM_SETREDRAW, 0, 0);
  Inc(FUpdateHidden);
end;

procedure TSpTBXTabToolbar.EndUpdateHidden;
begin
  Dec(FUpdateHidden);
  if FUpdateHidden < 0 then FUpdateHidden := 0;
  if FUpdateHidden = 0 then begin
...
begin
  if (FUpdateHidden = 0) and not (tstResizing in FState) and HandleAllocated then
...
var
  ...
  R: TRect;
begin
  if not FTabAutofit or (FUpdateHidden > 0) or (Items.Count = 0) then Exit;
...
begin
  if (FUpdateHidden > 0) or (csDestroying in ComponentState) or (tstRightAligning in FState) or
    not Assigned(CurrentDock) or (Items.Count <= 0) or
    not Stretch or (ShrinkMode <> tbsmNone) or
    (CurrentDock.Width <= 0) or (CurrentDock.Height <= 0) then
      Exit;
  ...
  try
    ...
    if not IsFirstPartiallyVisible then begin
      ...
      IV := SpFindItemViewer(View, ActiveTab);
      if Assigned(IV) and (IV.Item.Visible) then begin
        iStart := IV.Index;
        iEnd := IV.Index;
      end
      else begin
      ...
      // Show items from left side of the ActiveTab
      ...
      // Show items from right side of the ActiveTab
    ...
    if Items.Count = 1 then
      ...
      if I > -1 then
        if Toolbar.FHiddenTabs.IndexOf(Items[I]) > -1 then begin
          IV := View.Find(Item);
          if Assigned(IV) then begin
            Toolbar.BeginUpdateHidden;
            try
              Toolbar.FHiddenTabs.Add(Item);
              Item.Visible := False;
            finally
              Toolbar.EndUpdateHidden;
            end;
          end;
        end;
    end;
