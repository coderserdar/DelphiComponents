...
type
  ...
  published
    ...
    property OnClosePopup: TSpTBXRollUpEvent read FClosePopup write FClosePopup;
...
begin
  ...
  try
    ...
    if PaintDefault then begin
      GR := Rect(0, 0, 0, 0);
      SpDrawXPStatusBar(ACanvas, R, GR, sknSkin, False);
    end;
    ...
    if PaintDefault then begin
      ...
      if C1 = clNone then C1 := clBtnShadow;
      C2 := SkinManager.CurrentSkin.Options(skncStatusBarGrip).Body.Color2;
      if C2 = clNone then C2 := clBtnHighlight;
    ...
    WM_LBUTTONDOWN, WM_RBUTTONDOWN, WM_MBUTTONDOWN:
      ...
      if (GetCapture = 0) and (Msg.hwnd <> Handle) and not Windows.IsChild(Handle, Msg.hwnd) then
        if Assigned(FPopupControl) and not (FPopupControl is TCustomForm) and (FPopupControl is TWinControl) then begin
          if (Msg.hwnd <> TWinControl(FPopupControl).Handle) and not Windows.IsChild(TWinControl(FPopupControl).Handle, Msg.hwnd) then begin
            Handled := True;
            RollUp(False);
          end;
        end
...
//    PostMessage(Handle, WM_SPTBX_POPUPROLLUP, 0, 1);
    ...
    PostMessage(Handle, WM_SPTBX_POPUPINVALIDATE, 0, 0);  // Post the invalidate message to avoid flicker
  end;
end;
...
begin
  SpDrawXPMenuPopupWindow(ACanvas, ARect, Rect(0, 0, 0, 0), False, 0, FFormPopupMenu.SkinType);
end;
...
begin
  ...
  if Assigned(FClosePopup) then FClosePopup(Self, Selected);
