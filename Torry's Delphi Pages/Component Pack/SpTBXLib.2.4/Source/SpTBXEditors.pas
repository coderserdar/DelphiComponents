  ...
  if BorderStyle <> bsNone then
    if SpTBXSkinType(FSkinType) <> sknNone then begin
      if FHotTrack then
        ...
        HotTrackFrame := False;
      SpDrawXPEditFrame(Self, HotTrackFrame, FSkinType, False);
    end;
end;
  ...
  begin
    ...
    try
      ...
      if (Integer(itemID) >= 0) and (Integer(itemID) < Items.Count) then begin
        ...
        SknState := CurrentSkin.GetState(not (odDisabled in State), False, odSelected in State, False);
...
begin
  ...
  try
    ...
    if Style = csSimple then
      ...
      ButtonWidth := GetSystemMetrics(SM_CXHSCROLL);
    ButtonR := R;
    ButtonR.Left := ButtonR.Right - ButtonWidth;
    ...
    try
      ...
      // OS draw the frame
      VistaNewComCtrls := not (csDesigning in ComponentState) and (T = sknWindows) and SpIsWinVista and ThemeServices.ThemesEnabled;
      ...
      if PaintDefault and (T <> sknNone) and not VistaNewComCtrls then begin
        ...
        case T of
          sknNone:
            begin
              InflateRect(ButtonR, 0, -1);
              OffsetRect(ButtonR, -1, 0);
            end;
          sknWindows:
            begin
              InflateRect(ButtonR, 0, -1);
          ...
          sknSkin:
            begin
              ...
              InflateRect(ButtonR, 0, -2);
              OffsetRect(ButtonR, -2, 0);
            end;
        end;
...
begin
  inherited;
  if BorderStyle <> bsNone then
    if SpTBXSkinType(FSkinType) <> sknNone then
