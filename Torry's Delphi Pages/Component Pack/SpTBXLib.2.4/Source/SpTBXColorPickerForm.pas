...
type
  ...
  TSpTBXColorPickerForm = class(TForm)
    ...
    tColorBtn: TSpTBXItem;
    tLabel: TSpTBXRightAlignSpacerItem;
    tColorNone: TSpTBXItem;
    tColorDialog: TSpTBXItem;
    ...
    procedure imgPaletteMouseUp(Sender: TObject; Button: TMouseButton;
    ...
    procedure tColorBtnDrawItem(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
      ItemInfo: TSpTBXMenuItemInfo; const PaintStage: TSpTBXPaintStage;
      var PaintDefault: Boolean);
    procedure tColorNoneClick(Sender: TObject);
    procedure tColorDialogClick(Sender: TObject);
    procedure tColorNoneSelect(Sender: TTBCustomItem; Viewer: TTBItemViewer;
      Selecting: Boolean);
    procedure tColorDialogSelect(Sender: TTBCustomItem; Viewer: TTBItemViewer;
      Selecting: Boolean);
  protected
...
begin
  ...
  // Don't move SourceR outside of the screen visible area
  if SourceR.Left < 0 then SourceR.Left := 0
  else if SourceR.Left + zoomW > Screen.Width then SourceR.Left := Screen.Width - zoomW;
  if SourceR.Top < 0 then SourceR.Top := 0
  else if SourceR.Top + zoomH > Screen.Height then SourceR.Top := Screen.Height - zoomH;
...
begin
  SpTBXTabSheet1.DoubleBuffered := True;
...
procedure TSpTBXColorPickerForm.ColorListBox1Click(Sender: TObject);
begin
  SetSelectedColor(ColorListBox1.Selected);
end;
...
procedure TSpTBXColorPickerForm.imgPaletteMouseUp(Sender: TObject;
...
procedure TSpTBXColorPickerForm.tColorDialogClick(Sender: TObject);
var
...
procedure TSpTBXColorPickerForm.tColorDialogSelect(Sender: TTBCustomItem;
  Viewer: TTBItemViewer; Selecting: Boolean);
begin
  tLabel.Caption := SSpTBXColorPicker;
end;

procedure TSpTBXColorPickerForm.tColorBtnDrawItem(Sender: TObject;
...
begin
  if PaintStage = pstPrePaint then begin
    ...
    if tColorBtn.CaptionGlowColor = clNone then
      SpDrawChequeredBackground(ACanvas, ARect)
    else begin
      ACanvas.Brush.Color := tColorBtn.CaptionGlowColor;
...
procedure TSpTBXColorPickerForm.tColorNoneClick(Sender: TObject);
begin
  SetSelectedColor(clNone);
end;

procedure TSpTBXColorPickerForm.tColorNoneSelect(Sender: TTBCustomItem;
  Viewer: TTBItemViewer; Selecting: Boolean);
begin
  UpdateColorLabel(clNone);
  tLabel.Caption := SSpTBXTransparentColor;
end;
...
procedure TSpTBXColorPickerForm.btnColorPickerEndDrag(Sender, Target: TObject; X,
  Y: Integer);
begin
  if Assigned(FColorPickerDragObject) then
    if FColorPickerDragObject.Dropped then
      SetSelectedColor(tColorBtn.CaptionGlowColor);
  FreeAndNil(FColorPickerDragObject);
end;
...
begin
  tColorBtn.CaptionGlowColor := AColor;
  tLabel.Caption := SpColorToHTML(AColor);
end;
