...
type
  ...
  public
    ...
    constructor Create(const aFrequency : DWord;
                             aTimerEvent : TfsThreadProcessEvent;
  ...
  { This is the base class for threads associated with pools.  The pool's
    ...
    this class.  It then calls the TfsPooledThread.Process method. }
  TfsPooledThread = class(TfsThread)
  protected  { private }
    ...
    FProcessEvent : TfsThreadProcessEvent;
  ...
  public
    ...
    constructor Create(threadPool : TFSThreadPool); virtual;
    ...
    procedure Process(aProcessEvent : TfsThreadProcessEvent;
  ...
  TFSThreadPool = class(TfsLoggableComponent)                          {!!.06}
  private

    FActive : TFSNormalList;
      {-List of acquired threads. When a thread becomes inactive it is moved
        to FInactive. }

    FInactive : TFSNormalList;
  ...
  protected
    ...
    function thpGetThreadFromPool : TfsPooledThread;
    ...
    procedure thpPutInQueue(aProcessEvent : TfsThreadProcessEvent;
                            aProcessCookie: longInt);
    ...
    procedure thpReturnThreadToPool(aThread : TfsPooledThread);
  ...
  public
    ...
    procedure ProcessThreaded(aProcessEvent : TfsThreadProcessEvent;
                              aProcessCookie: longInt);
  ...
  { This type is used to store pending requests in the TFSThreadPool. }
  TfsThreadRequestItem = class(TfsSelfListItem)
  protected
    FProcessCookie : longInt;
    FProcessEvent : TfsThreadProcessEvent;
  public
    constructor Create(anEvent : TfsThreadProcessEvent;
                       aCookie : longInt);

    property ProcessCookie : longInt read FProcessCookie;
    property ProcessEvent : TfsThreadProcessEvent read FProcessEvent;
  end;
...
{===TfsTimerThread===================================================}
constructor TfsTimerThread.Create(const aFrequency : DWord;
                                        aTimerEvent : TfsThreadProcessEvent;
                                  const aTimerEventCookie : longInt;
...
begin
  ...
  if not assigned(aTimerEvent) then
    fsRaiseSCErrorCodeFmt(fssce_ParameterRequired,
  ...
  FDieEvent := TFSNormalEvent.Create;
...
{--------}
destructor TfsTimerThread.Destroy;
begin
  FDieEvent.Free;
  inherited Destroy;
end;
{--------}
procedure TfsTimerThread.DieDieDie;
begin
  Terminate;
  FDieEvent.SignalEvent;
end;
{--------}
procedure TfsTimerThread.Execute;
var
  aResult : DWORD;
begin
...
{===TfsPooledThread==================================================}
constructor TfsPooledThread.Create(threadPool : TFSThreadPool);
  { Use this method to create the thread and associate it with a thread
    pool. }
begin
  ...
  FWorkEvent := TFSNormalEvent.Create;
  FThreadEventHandles[0] := FWorkEvent.Handle;
  FThreadEventHandles[1] := FDieEvent.Handle;
  FreeOnTerminate := False;  { Freed in TFSThreadPool.destroy }
end;
{--------}
destructor TfsPooledThread.Destroy;
begin
  FDieEvent.Free;
  FWorkEvent.Free;
  inherited Destroy;
...
{--------}
procedure TfsPooledThread.DieDieDie;
begin
  Terminate;
  FDieEvent.SignalEvent;
end;
{--------}
procedure TfsPooledThread.Execute;
var
  WaitResult : DWORD;
begin
...
{Begin !!.06}
      try
        ...
        on E:Exception do
          FThreadPool.lcLog('Exception caught in TfsPooledThread.Execute: ' +
                            E.Message);
      end;
{End !!.06}
...
{--------}
procedure TfsPooledThread.Process(aProcessEvent : TfsThreadProcessEvent;
                                  aProcessCookie: longInt);
  { This method is called by the thread pool to perform work.  It saves
...
{--------}
procedure TfsPooledThread.ptReturnToPool;
begin
...
{===TFSThreadPool====================================================}
constructor TFSThreadPool.Create(aOwner : TComponent);
begin
  inherited Create(aOwner);
  FLock := TfsPadlock.Create;
  FActive := TFSNormalList.Create;
  FActive.Sorted := False;
  FInactive := TFSNormalList.Create;
  FInactive.Sorted := False;
  FInitialCount := 5;
  FInitialized := False;
  FMaxCount := 16;
  FPendingQueue := TfsThreadQueue.Create;
  FSkipInitial := False;
end;
{--------}
destructor TFSThreadPool.Destroy;
var
  anIndex : longInt;
  aThread : TfsPooledThread;
  HandleList : TFSSpecList;  { list of thread handles }
  PHandleArray : pointer;
begin
  FFNotifyDependents(ffn_Destroy);                                     {!!.11}
  FLock.Lock;
  try
    HandleList := TFSSpecList.Create;
    try
      if assigned(FActive) then begin
        ...
        for anIndex := pred(FActive.Count) downto 0 do begin
          aThread := TfsPooledThread(TfsIntListItem(FActive[anIndex]).KeyAsInt);
      ...
      if assigned(FInactive) then begin
        ...
        for anIndex := pred(FInactive.Count) downto 0 do begin
          aThread := TfsPooledThread(TfsIntListItem(FInactive[anIndex]).KeyAsInt);
          HandleList.Append(aThread.Handle);
          aThread.DieDieDie;
        end;
      end;
    ...
    if assigned(FActive) then
      for anIndex := pred(FActive.Count) downto 0 do
        TfsPooledThread(TfsIntListItem(FActive[anIndex]).KeyAsInt).Free;

    if assigned(FInactive) then
      for anIndex := pred(FInactive.Count) downto 0 do
        TfsPooledThread(TfsIntListItem(FInactive[anIndex]).KeyAsInt).Free;
...
{--------}
procedure TFSThreadPool.Flush(NumToRetain : integer);
var
  anIndex : integer;
  aThread : TfsPooledThread;
begin
...
{--------}
procedure TFSThreadPool.ProcessThreaded(aProcessEvent : TfsThreadProcessEvent;
                                        aProcessCookie: longInt);
var
  aThread : TfsPooledThread;
begin
...
{--------}
function TFSThreadPool.thpGetActiveCount : integer;
begin
  FLock.Lock;
  try
...
{--------}
function TFSThreadPool.thpGetFreeCount : integer;
begin
...
{--------}
function TFSThreadPool.thpGetInactiveCount : integer;
begin
  FLock.Lock;
  try
...
{--------}
function TFSThreadPool.thpGetThreadFromPool : TfsPooledThread;
var
...
begin
  ...
  try
    ...
    if anIndex >= 0 then begin
      ...
      aListItem := TfsIntListItem(FInactive[anIndex]);
      FInactive.RemoveAt(anIndex);
      Result := TfsPooledThread(aListItem.KeyAsInt);
    end;
    ...
    if not assigned(Result) then
      if thpGetFreeCount > 0 then begin
        Result := TfsPooledThread.Create(Self);
        aListItem := TfsIntListItem.Create(longInt(Result));
      end;
...
{--------}
procedure TFSThreadPool.thpPutInQueue(aProcessEvent : TfsThreadProcessEvent;
                                      aProcessCookie: longInt);
var
  anItem : TfsThreadRequestItem;
begin
  anItem := TfsThreadRequestItem.Create(aProcessEvent, aProcessCookie);
...
{--------}
procedure TFSThreadPool.thpReturnThreadToPool(aThread : TfsPooledThread);
var
  aCookie: longInt;
  anEvent : TfsThreadProcessEvent;
  anItem : TfsThreadRequestItem;
  aListItem : TfsIntListItem;
...
begin
    ...
    with FPendingQueue.BeginWrite do
      try
        ...
        if PendingRequest then begin
          anItem := TfsThreadRequestItem(FPendingQueue.Dequeue);
  ...
  if PendingRequest then
    ...
    FLock.Lock;
    try
      aListItem := TfsIntListItem(FActive[FActive.Index(longInt(aThread))]);
...
{--------}
procedure TFSThreadPool.thpSetInitialCount(const aCount : integer);
var
  anIndex : integer;
  ...
  if not (csDesigning in ComponentState) and (not FInitialized) and
    ...
    try
      ...
      for anIndex := 1 to aCount do begin
        aThread := TfsPooledThread.Create(Self);
        anItem := TfsIntListItem.Create(longInt(aThread));
...
{--------}
procedure TFSThreadPool.thpSetMaxCount(const aCount : integer);
var
  anIndex : integer;
  aThread : TfsPooledThread;
  currCount : integer;
      ...
      try
        for anIndex := 1 to delCount do
          ...
          if FInactive.Count > 0 then begin
            aThread := TfsPooledThread(TfsIntListItem(FInactive[0]).KeyAsInt);
            aThread.DieDieDie;
...
{===TfsThreadRequestItem=============================================}
constructor TfsThreadRequestItem.Create(anEvent : TfsThreadProcessEvent;
                                        aCookie : longInt);
begin
  inherited Create;
