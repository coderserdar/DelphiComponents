...
begin
  result := Pos(String(SubStr), String(Str));
end;
...
begin
  result := ALMakeResultString(Pchar(copy(String(Str), Index, Count)));
end;
...
begin
  ...
  result := ALMakeResultString(pchar(stringreplace(String(s),String(OldPattern),String(NewPattern),aFlags)));
end;

{*******************************************************************************
take care if you put more than 32k instead of 16k in CSTRING params
http://tech.groups.yahoo.com/group/firebird-support/message/107453
...
DECLARE EXTERNAL FUNCTION ALUDF_POS
  CSTRING(16384) CHARACTER SET ISO8859_1,
  CSTRING(16384) CHARACTER SET ISO8859_1
  RETURNS INTEGER BY VALUE
...
DECLARE EXTERNAL FUNCTION ALUDF_COPY
  ...
  SMALLINT,
  SMALLINT
  RETURNS CSTRING(16384) CHARACTER SET ISO8859_1 FREE_IT
