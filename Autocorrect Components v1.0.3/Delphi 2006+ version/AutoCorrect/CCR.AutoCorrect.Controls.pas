...
type
  ...
  strict private
    FAutoCorrectEngine: TAutoCorrectEngine;
    procedure SetAutoCorrectEngine(Value: TAutoCorrectEngine);
  protected
  ...
  published
    property AutoCorrectEngine: TAutoCorrectEngine read FAutoCorrectEngine
...
{$IFNDEF UNICODE} //D2009 finally uses riched20.dll, which 'only' has the dashes bug
...
begin
  inherited;
  if (AComponent = FAutoCorrectEngine) and (Operation = opRemove) then
    FAutoCorrectEngine := nil;
end;

procedure TComboBoxWithAutoCorrect.SetAutoCorrectEngine(Value: TAutoCorrectEngine);
begin
  if FAutoCorrectEngine <> nil then FAutoCorrectEngine.RemoveFreeNotification(Self);
  FAutoCorrectEngine := Value;
  if FAutoCorrectEngine <> nil then FAutoCorrectEngine.FreeNotification(Self);
end;
...
procedure TEditWithAutoCorrect.SetAutoCorrectEngine(Value: TAutoCorrectEngine);
begin
  if FAutoCorrectEngine <> nil then FAutoCorrectEngine.RemoveFreeNotification(Self);
  FAutoCorrectEngine := Value;
  if FAutoCorrectEngine <> nil then FAutoCorrectEngine.FreeNotification(Self);
...
procedure TMemoWithAutoCorrect.SetAutoCorrectEngine(Value: TAutoCorrectEngine);
begin
  if FAutoCorrectEngine <> nil then FAutoCorrectEngine.RemoveFreeNotification(Self);
  FAutoCorrectEngine := Value;
  if FAutoCorrectEngine <> nil then FAutoCorrectEngine.FreeNotification(Self);
...
procedure TRichEditWithAutoCorrect.SetAutoCorrectEngine(Value: TAutoCorrectEngine);
begin
  if FAutoCorrectEngine <> nil then FAutoCorrectEngine.RemoveFreeNotification(Self);
  FAutoCorrectEngine := Value;
  if FAutoCorrectEngine <> nil then FAutoCorrectEngine.FreeNotification(Self);
...
var
  ...
  TryFix: Boolean;
begin
  TryFix := (Message.Options and SF_RTF <> 0) and (Message.EditStream <> nil) and
    Assigned(Message.EditStream.pfnCallback);
  if TryFix then
  begin
  ...
  if TryFix then
  begin
    Pointer(Message.EditStream.dwCookie) := Cookie.SourceCookie;
    Pointer(@Message.EditStream.pfnCallback) := @Cookie.SourceCallBack;
  end;
end;
