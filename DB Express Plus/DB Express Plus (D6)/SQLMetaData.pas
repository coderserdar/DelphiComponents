...
uses
  Windows, Classes, Forms, Dialogs, SysUtils, DBXpress, SQLExpr, DBLocalS,
...
type
  ...
  public
    ...
    function GetTableMetaData(const ATableName: string; AFieldIndex: Integer): TFieldMetaData;
...
function TSQLMetaData.GetTableMetaData(const ATableName: string; AFieldIndex: Integer): TFieldMetaData;
begin
...
begin
  ...
  for FieldCount := 0 to FMetaDataFieldCount - 1 do begin
    if FieldCount = FMetaDataFieldCount - 1 then
      ASQL.Add(GetTableMetaData(ATableName, FieldCount).ColumnName)
    else
      ASQL.Add(GetTableMetaData(ATableName, FieldCount).ColumnName + ' ,');
  end;
  ASQL.Add(' ) VALUES ( ');
  for FieldCount := 0 to FMetaDataFieldCount - 1 do begin
    if FieldCount = FMetaDataFieldCount - 1 then
      ASQL.Add('value_for_' + GetTableMetaData(ATableName, FieldCount).ColumnName + ' )')
    else
      ASQL.Add('value_for_' + GetTableMetaData(ATableName, FieldCount).ColumnName + ' ,');
  end;
...
begin
  ...
  for FieldCount := 0 to FMetaDataFieldCount - 1 do begin
    if FieldCount = FMetaDataFieldCount - 1 then
      ASQL.Add(GetTableMetaData(ATableName, FieldCount).ColumnName +
         ' = ' + GetTableMetaData(ATableName, FieldCount).ColumnTypeName)
    else
      ASQL.Add(GetTableMetaData(ATableName, FieldCount).ColumnName +
      ' = ' + GetTableMetaData(ATableName, FieldCount).ColumnTypeName + ' ,');
  end;
